...
type
  TDMAjustesItensFiscal = class(TDMManutencao)
...
var
  DMAjustesItensFiscal: TDMAjustesItensFiscal;
...
procedure TDMAjustesItensFiscal.AlterarAlmox;
Begin
  C_TabelaEstoque.value := EstoqueAlmox(C_TabelaITEM.Value, C_TabelaAlmoxarifado.Value);
  C_TabelaITEMChange(C_TabelaITEM);
End;

function TDMAjustesItensFiscal.EstoqueAlmox(Item, Almox: Integer): Real;
Var Qtd: Real;
Begin
  Qtd:=0;
  With Q_Sql do Begin
...
procedure TDMAjustesItensFiscal.DataModuleCreate(Sender: TObject);
begin
  inherited;
  DMAjustesItensFiscal := self;
  dData := DMProjeto.dDataSistema;
  bMultiAlmox := DMProjeto.Parametro('MultiAlmox') = 'S';
end;

procedure TDMAjustesItensFiscal.C_TabelaNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TDMAjustesItensFiscal.C_TabelaITEMChange(Sender: TField);
begin
  inherited;
  if Sender.asVariant <> null then begin
    ...
    Else
       C_TabelaEstoque.value      := DMProjeto.C_LocalizarItens.FieldByName('EstoqueFiscal').AsFloat;
...
procedure TDMAjustesItensFiscal.C_TabelaQUANTIDADEChange(Sender: TField);
var
  nNovoEst : double;
begin
  inherited;
...
procedure TDMAjustesItensFiscal.C_TabelaEstoqueNovoChange(Sender: TField);
var
  nQtd : double;
begin
  inherited;
...
procedure TDMAjustesItensFiscal.DMComponentGravar1_Iniciar(Sender: TObject;
  var bSkip: Boolean);
begin
  inherited;
...
function TDMAjustesItensFiscal.DadosCompleto;
begin
  result := true;
...
function TDMAjustesItensFiscal.ContabilidadeAtivado;
begin
  result := DMContabil <> nil;
end;

procedure TDMAjustesItensFiscal.ContabilidadeDesativado;
begin
  DMContabil.FechaDatasets;
  DMContabil.Free;
end;

procedure TDMAjustesItensFiscal.CriaContabilidade;
begin
  DMContabil := TDMContabilidade.Create(self);
end;

procedure TDMAjustesItensFiscal.Contabiliza;
begin
  DMContabil.ContabilizaAjustesItens(DMAjustesItensFiscal.C_TabelaIDMestre.Value,DMAjustesItensFiscal.C_Tabela);
end;

procedure TDMAjustesItensFiscal.MostraContabilidade;
begin
...
procedure TDMAjustesItensFiscal.DMComponentGravar3_AposApply(Sender: TObject);
begin
  inherited;
  Contabiliza;
...
procedure TDMAjustesItensFiscal.DMComponentModoInclusao1_Iniciar(
  Sender: TObject);
begin
  inherited;
...
procedure TDMAjustesItensFiscal.DMComponentApagar_AntesApply(Sender: TObject);
begin
  inherited;
  DMProjeto.CancelarLancamentos(C_TabelaIDMESTRE.asstring);
end;

procedure TDMAjustesItensFiscal.C_TabelaUNIDADEChange(Sender: TField);
var
  nFator : double;
begin
  inherited;
...
procedure TDMAjustesItensFiscal.C_TabelaCalcFields(DataSet: TDataSet);
var
  nFator : double;
begin
  inherited;
...
procedure TDMAjustesItensFiscal.C_TabelaQUANTIDADESetText(Sender: TField;
  const Text: String);
begin
  inherited;
...
procedure TDMAjustesItensFiscal.C_TabelaicEstoqueNovoSetText(Sender: TField;
  const Text: String);
begin
  inherited;
  //Se a unidade for inteira, truncar a quantidade
...
procedure TDMAjustesItensFiscal.C_TabelaUNIDADEValidate(Sender: TField);
begin
  inherited;
