...
begin
    ...
    if (DMItens.C_TabelaicCST.Value = '020') then
        begin
            lbTipoTributacao.Caption := sTributacao + ', Redução Base Calculo.';
            dfMVA.Value := 0;
            dfrMVA.Value := 0;
            lbMVA.Enabled := false;
        ...
        begin
            lbTipoTributacao.Caption := sTributacao + ', Substituição Tributaria.';
            dfREducaoBC.Value := 0;
        ...
        begin
            lbTipoTributacao.Caption := sTributacao + ', Isento, Não Incidência.';
...
begin
    ...
    if (DMProjeto.sTipoTributEmpresa = 'N') then
        begin
            try
                //Simples Nacional - Calculo para produto com redução de Base de Calculo
                if (DMItens.C_TabelaicCST.Value = '020') then
                    begin
...
//                            nCustoContabil := nCustoContabil + (dfPrecoNota.Value * (DMItens.C_TabelaCompensaCusto.Value / 100.00));
                    ...
                    begin
                        //Simples Nacional - Calculo para Produto com Substituição
...
//                            nCustoContabil := nCustoContabil + (dfPrecoNota.Value * (DMItens.C_TabelaCompensaCusto.Value / 100.00));
                    ...
                    begin
                        //Simples Nacional - Isento Ou Não Tributadd
                        nICMSAplicEntrada := 0.00;
                        baseCalculoST := dfPrecoNota.Value
...
//                            nCustoContabil := nCustoContabil + (dfPrecoNota.Value * (DMItens.C_TabelaCompensaCusto.Value / 100.00));
                ...
                {Simples Nacional - Definindo o Preço de Venda}
    ...
    if (DMProjeto.sTipoTributEmpresa = 'P') then
        begin
            try
                //Lucro Presumido- Calculo para produto com redução de Base de Calculo
                if (DMItens.C_TabelaicCST.Value = '020') then
                    begin
...
//                            nCustoContabil := nCustoContabil + (dfPrecoNota.Value * (DMItens.C_TabelaCompensaCusto.Value / 100.00));
                    ...
                    begin
                        //Lucro Presumido-- Isento Ou Não Tributadd
                        nICMSAplicEntrada := 0.00;
...
//                            nCustoContabil := nCustoContabil + (dfPrecoNota.Value * (DMItens.C_TabelaCompensaCusto.Value / 100.00));
                    ...
                    begin
                        //Lucro Presumido - Calculo para Produto com Substituição
                        nICMSAplicEntrada := 0.00;
                        nICMSAplicSaida := 0.00;
                        baseCalculoST := dfPrecoNota.Value
                            + (dfPrecoNota.Value * (dfIPICompra.Value / 100.00))
...
//                            nCustoContabil := nCustoContabil + (dfPrecoNota.Value * (DMItens.C_TabelaCompensaCusto.Value / 100.00));
                ...
                {Lucro Presumido  - Definindo o Preço de Venda}
                nSemLucro := nICMSAplicSaida //Já está considerando Reducao, Substituicao, Isensão, etc.
                + dfIPIVenda.Value
                    + dfImpostosVenda.value
                    + dfDespesaFixa.Value
                    + dfCartoes.Value
    ...
    if (DMProjeto.sTipoTributEmpresa = 'R') then
        begin
            try
                //Lucro Real - Calculo para produto com redução de Base de Calculo
...
//                            nCustoContabil := nCustoContabil + (dfPrecoNota.Value * (DMItens.C_TabelaCompensaCusto.Value / 100.00));
                    ...
                    begin
                        //Lucro Real - Calculo para Produto com Substituição
                        nICMSAplicEntrada := 0.00;
                        nICMSAplicSaida := dfICMSAplicado.Value;
                        baseCalculoST := dfPrecoNota.Value
                            + (dfPrecoNota.Value * (dfIPICompra.Value / 100.00))
...
//                            nCustoContabil := nCustoContabil + (dfPrecoNota.Value * (DMItens.C_TabelaCompensaCusto.Value / 100.00));
                    ...
                    begin
                        //Lucro Real - Calculo Para o Produto Isento ou Não Tributado
                        nICMSAplicEntrada := 0.00;
                        nICMSAplicSaida := 0.00;
                        nCustoContabil := (dfPrecoNota.Value)
                            + (dfFrete.Value) //Digitado em R$
                ...
                {Lucro Real  - Definindo o Preço de Venda}
                nSemLucro := nICMSAplicSaida //Já está considerando Reducao, Substituicao, Isensão, etc.
                + dfIPIVenda.Value
                    + dfImpostosVenda.value
                    + dfDespesaFixa.Value
...
begin
    ...
    if DMItens.C_TabelaUltimaCompra.Value = 0 then
        with DMItens do
            begin //se não houve compra pode substituir campos
...
begin
    ...
    if (DMProjeto.sTipoTributEmpresa = null) or (DMProjeto.sTipoTributEmpresa = '') then
        begin
            ShowMessage('Falta configurar no cadastro de empresas a sua forma de tributação');
            Close();
        end;
end;
