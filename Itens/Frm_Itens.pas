...
type
    ...
    private
        ...
        TmpPage: TTS_PageControl; //Utilizado no Keydown para melhorar performance em não tê-lo local ao evento.
    ...
    with DMItens do
        begin
            ...
            case lbDescMax1.Font.Color of
                ...
                clTeal: lbDescMax1.Hint := 'Maior Desconto que os Usuários podem dar';
                clOlive: lbDescMax1.Hint := 'Não existe definição no sistema!' + #13 +
                    'Configure o Desconto Máximo nos Usuários ou para este Item.';
            end;
            ...
            case lbComissao1.Font.Color of
                ...
                clTeal: lbComissao1.Hint := 'Comissão Média definida no Cad. de Fatores de Custos';
                clOlive: lbComissao1.Hint := 'Não existe definição no sistema!' + #13 +
                    'Configure a Comissão Média defina uma para este Item.';
            end;
...
begin
    inherited;
    {Definindo se o Preço de compra pode ser informado}
            ...
            begin
                if C_TabelaicFatorLucro.Value = 0 then
                    begin
                        ...
                        imgpreco.Hint := 'Atenção, você não está tendo lucro! Seu Fator de Lucro está Zero.';
                    end
                    ...
                    begin
                        ...
                        imgpreco.Hint := 'Cuidado, você está tendo prejuízo! O preço de Venda está abaixo do Menor Preço de Venda calculado.';
                    end
                    ...
                    begin
                        ...
                        imgpreco.Hint := 'Atenção, você não está tendo lucro! O preço de Venda está igual ao Menor Preço de Venda calculado.';
                    end
                    ...
                    begin
                        ...
                        imgpreco.Hint := 'Seu Preço de Venda está com Lucro!';
                    end
                    ...
                    begin
                        ...
                        imgpreco.Hint := 'Dependendo do Desconto na hora da Venda, o item pode dar prejuízo!';
                    end
                else
                    begin
...
begin
    if DMProjeto.Parametro('PrecosAutomaticos') = 'S' then
        with DMItens do
            begin
                {Verificando se o Desconto Máximo está num limite aceitavel (com lucro ainda)}
...
begin
    ...
    lbPrecoVenda1.Caption := iif(bMultiUnd, 'Preço de Custo:', 'Preço de Venda:');
...
begin
    ...
    {Verificando se o Item ficou em promoção - O Flag promoçao é atualizado por triggers}
    if DMItens.bAlteracao then
...
begin
    ...
    {Alertando para possível distorção entre o preço de compra e o de venda. Provodado por ItensUnidades}
    with DMItens do
        begin
    ...
    {Verificando se o estoque foi informado e o custo não}
    ...
    if DMItens.C_TabelaTipoItem.Value = 1 then
        with DMItens do
            begin
                ...
                {Se todas as unidades são distintas (validação acima), o distinct dos fatores identifica se existem duas
                unidades diferentes com mesmo fator.}
                {Inicio Multiunidade}

                  // Para Possibilitar a inclusão de mais de uma unidade com o mesmo fator.
...
begin
  ...
  //  dfEstoque_b.SetReadOnly(false);
    ...
    if (DMProjeto.Parametro('CustoCalcPreco') = 'CC') then
        TS_Label29.Caption := 'Custo Contábil (C.C):'
    else
        TS_Label29.Caption := 'Custo Médio (C.M.):';
    ...
    case key of
        34: if not FormatoTabela.Checked and (Estado = fsInclusao) then
                begin
                    ...
                    {Procurando o próximo visivel que pode ser ativado}
...
begin
    ...
    if (DMItens.C_TabelaPreco.asCurrency <> DMItens.C_TabelaicPrecoCalc.asCurrency) then
        begin
            confirmaPrecoCalculado := MessageDlg('Preço calculado: R$ ' + DMItens.C_TabelaicPrecoCalc.AsString + #13 + 'esta diferente do ' + #13 + 'Preço do Produto: R$ ' + DMItens.C_TabelaPreco.AsString + '. ' + #13 + 'Deseja Salvar?', mtConfirmation, [mbYes, mbNo], 0);
    ...
    with DMItens do
        begin
            ...
            if C_Tabela.State in [dsInsert, dsEdit] then
                ...
                C_Tabela.Edit; //Já ocorre o calcfields;
        end;
    ...
    with DMItens do
        begin
            ...
            if nCusto = 0 then
                ...
                C_TabelaicFATORLUCRO.Value := -500; //100% de prejuízo sobre o custo do item;
...
begin
    inherited;
    {Calculando o Fator de Lucro para o Preço de Venda digitado}
    if Key = #13 then
        begin
    ...
    if LastDataObject.TableName = 'GRUPOS' then
        with DMItens do
            begin
                //          C_Tabela.Refresh;
                C_Tabela.Edit;
                C_TabelaGrupo.Value := lastdataobject.objectkey; //Para forcar o calcfields e a atualização dos campos ic...;
            end;
...
begin
    ...
    if DMProjeto.Parametro('AtualizaPeloGrupo') = 'S' then
        begin
            if Application.MessageBox('Deseja atualizar os dados fiscais deste item com informações correspondente ao grupo escolhido?', 'Atenção', mb_yesno) <> id_yes then
                exit;
...
begin
    ...
    AnoMes := InputBox('Ano/Mês Referência:', 'Informe o ano e o mês', AnoMes);
    //Fabricante := cmbFabricante.LookupKeyValue;
    DMITens.ExportacaoMabel(AnoMes, 0);
    Application.MessageBox('Arquivos Gerados com Êxito!', 'AVISO!', mb_ok);
end;
    ...
    if (nFixarTamanhoCodigoItem > 0) then
        begin
            if (Length(dfCodigo.Text) <> nFixarTamanhoCodigoItem) then
                begin
                    DlgMsg.ShowMsg(50, [pchar('O tamanho do campo código deve ser igual a ' + inttoStr(nFixarTamanhoCodigoItem) + ' caracteres.')]);
                    ActiveControl := dfCodigo;
                end;
        end;
    if SoNumeros(dfCodigo.Text, Length(Trim(dfCodigo.Text))) <> Trim(dfCodigo.Text) then
        begin
            DlgMsg.ShowMsg(50, [pchar('O Campo Código só permite caracteres númericos.')]);
            ActiveControl := dfCodigo;
        end;
end;
...
begin
    ...
    if SoNumeros(dfCodBarras.Text, Length(Trim(dfCodBarras.Text))) <> Trim(dfCodBarras.Text) then
        begin
            DlgMsg.ShowMsg(50, [pchar('O Campo Código de Barras só permite caracteres númericos.')]);
