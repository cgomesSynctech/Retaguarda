...
type
  TDMAjustesVolumesItens = class(TDMManutencao)
    ...
    C_TabelaQUANTIDADEVOLUME: TBCDField;
...
var
  DMAjustesVolumesItens: TDMAjustesVolumesItens;
...
procedure TDMAjustesVolumesItens.AlterarAlmox;
Begin
...
function TDMAjustesVolumesItens.EstoqueAlmox(Item, Almox: Integer): Real;
Var Qtd: Real;
Begin
  Qtd:=0;
  With Q_Sql do Begin
...
procedure TDMAjustesVolumesItens.DataModuleCreate(Sender: TObject);
begin
  inherited;
  DMAjustesVolumesItens := self;
  dData := DMProjeto.dDataSistema;
  bMultiAlmox := DMProjeto.Parametro('MultiAlmox') = 'S';
end;

procedure TDMAjustesVolumesItens.C_TabelaNewRecord(DataSet: TDataSet);
begin
  inherited;                                             
...
procedure TDMAjustesVolumesItens.C_TabelaITEMChange(Sender: TField);
begin
  inherited;
  if Sender.asVariant <> null then begin
    ...
    C_TabelaQUANTIDADEVOLUME.Value := DMProjeto.C_LocalizarItensESTOQUEVOLUME.Value ;
...
procedure TDMAjustesVolumesItens.C_TabelaQUANTIDADEChange(Sender: TField);
var
  nNovoEst : double;
begin
  inherited;
...
procedure TDMAjustesVolumesItens.C_TabelaEstoqueNovoChange(Sender: TField);
var
  nQtd : double;
begin
  inherited;
...
procedure TDMAjustesVolumesItens.DMComponentGravar1_Iniciar(Sender: TObject;
  var bSkip: Boolean);
begin
  inherited;
...
function TDMAjustesVolumesItens.DadosCompleto;
begin
  result := true;
...
function TDMAjustesVolumesItens.ContabilidadeAtivado;
begin
  result := DMContabil <> nil;
end;

procedure TDMAjustesVolumesItens.ContabilidadeDesativado;
begin
  DMContabil.FechaDatasets;
  DMContabil.Free;
end;

procedure TDMAjustesVolumesItens.CriaContabilidade;
begin
  DMContabil := TDMContabilidade.Create(self);
end;

procedure TDMAjustesVolumesItens.Contabiliza;
begin
 // DMContabil.ContabilizaAjustesVolumesItens(DMAjustesVolumesItens.C_TabelaIDMestre.Value,DMAjustesVolumesItens.C_Tabela);
end;

procedure TDMAjustesVolumesItens.MostraContabilidade;
begin
...
procedure TDMAjustesVolumesItens.DMComponentGravar3_AposApply(Sender: TObject);
begin
  inherited;
  Contabiliza;
...
procedure TDMAjustesVolumesItens.DMComponentModoInclusao1_Iniciar(
  Sender: TObject);
begin
  inherited;
...
procedure TDMAjustesVolumesItens.DMComponentApagar_AntesApply(Sender: TObject);
begin
  inherited;
  DMProjeto.CancelarLancamentos(C_TabelaIDMESTRE.asstring);
end;

procedure TDMAjustesVolumesItens.C_TabelaUNIDADEChange(Sender: TField);
var
  nFator : double;
begin
  inherited;
...
procedure TDMAjustesVolumesItens.C_TabelaCalcFields(DataSet: TDataSet);
var
  nFator : double;
begin
  inherited;
  ...
  C_TabelaicEstoqueNovo.Value := C_TabelaQUANTIDADEVOLUME.Value + C_TabelaQUANTIDADE.Value ;
...
procedure TDMAjustesVolumesItens.C_TabelaQUANTIDADESetText(Sender: TField;
  const Text: String);
begin
  inherited;
...
procedure TDMAjustesVolumesItens.C_TabelaicEstoqueNovoSetText(Sender: TField;
  const Text: String);
begin
  inherited;
  //Se a unidade for inteira, truncar a quantidade
...
procedure TDMAjustesVolumesItens.C_TabelaUNIDADEValidate(Sender: TField);
begin
  inherited;
