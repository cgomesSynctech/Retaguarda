...
type
  TFrmAjustesVolumesItens = class(TFrmPadrao)
    TS_Label1: TTS_Label;
    TS_Label2: TTS_Label;
...
var
  FrmAjustesVolumesItens: TFrmAjustesVolumesItens;

implementation
  uses DM_AjustesVolumesItens, DM_Projeto, funcoes, variants;

{$R *.DFM}

procedure TFrmAjustesVolumesItens.TS_SpeedButton1Click(Sender: TObject);
begin
  inherited;
  if not DMProjeto.dlgAutorizacao.ExecuteX(Self.Name,'CONTABILID') then
    exit;

  if not DMAjustesVolumesItens.DadosCompleto then
    exit;

  // Adriano
  DMAjustesVolumesItens.Contabiliza;

  DMAjustesVolumesItens.MostraContabilidade;
  ////////////////

{  DMContabil.ContabilizaAjustesVolumesItens(DMAjustesVolumesItens.C_TabelaIDMestre.value,DMAjustesVolumesItens.C_Tabela);
...
procedure TFrmAjustesVolumesItens.TS_DBPopupEdit1InitPopup(Sender: TObject);
begin
  inherited;
  DlgPopup.SetEventos(TS_DBPopupEdit1);
end;

procedure TFrmAjustesVolumesItens.FormCreate(Sender: TObject);
begin
  // Adriano
  DMAjustesVolumesItens.CriaContabilidade;
  inherited;
  DlgPopup := TDlgPopupContas.Create(self,DMAjustesVolumesItens.C_ContasDS,nil,true);
...
procedure TFrmAjustesVolumesItens.FormActivate(Sender: TObject);
begin
  inherited;
  TS_DBPopupEdit1.PopupControl := DlgPopup.pnPopup;
end;

procedure TFrmAjustesVolumesItens.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  inherited;
  DlgPopup.free;
  if DMAjustesVolumesItens.ContabilidadeAtivado then
    DMAjustesVolumesItens.ContabilidadeDesativado;
  ////////////////////
end;

procedure TFrmAjustesVolumesItens.FormComponentEstado_Inicial(Sender: TObject;
  var bSkip: Boolean);
begin
  inherited;
  DMAjustesVolumesItens.C_Unidades.Filtered := false;
...
procedure TFrmAjustesVolumesItens.FormComponentEstado_Navegacao(Sender: TObject;
  var bSkip: Boolean);
begin
  inherited;
...
procedure TFrmAjustesVolumesItens.dfItemSelecionou(Sender: TObject);
begin
  inherited;
//  lblLote.visible     := DMAjustesVolumesItens.C_TabelaControleValidade.value = 'S';
//  dfLote.visible      := DMAjustesVolumesItens.C_TabelaControleValidade.value = 'S';
//  lblValidade.visible := DMAjustesVolumesItens.C_TabelaControleValidade.value = 'S';
//  dfValidade.visible  := DMAjustesVolumesItens.C_TabelaControleValidade.value = 'S';

end;

procedure TFrmAjustesVolumesItens.FormComponentAfterScroll(Sender: TObject);
begin
  inherited;
//  lblLote.visible := DMAjustesVolumesItens.C_TabelaControleValidade.value = 'S';
//  dfLote.visible := DMAjustesVolumesItens.C_TabelaControleValidade.value = 'S';
...
procedure TFrmAjustesVolumesItens.FormComponentBeforeClearParams(Sender: TObject);
begin
  inherited;
...
procedure TFrmAjustesVolumesItens.cbLockDateChange(Sender: TObject);
begin
  inherited;
  if cbLockDate.Checked then
    DMAjustesVolumesItens.dData := DMAjustesVolumesItens.C_TabelaData.Value
  else
    DMAjustesVolumesItens.dData := DMProjeto.dDataSistema;
end;

procedure TFrmAjustesVolumesItens.TS_DBEditDate1Exit(Sender: TObject);
begin
  inherited;
  cbLockDateChange(Self);
end;

procedure TFrmAjustesVolumesItens.FormComponentRefresh(Sender: TObject;
  var bSkip: Boolean);
begin
  inherited;
...
procedure TFrmAjustesVolumesItens.FormKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  If Key in ['.',','] then
...
procedure TFrmAjustesVolumesItens.cmbAlmoxChange(Sender: TObject);
begin
  inherited;
  DMAjustesVolumesItens.AlterarAlmox;
end;
