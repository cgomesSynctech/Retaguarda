...
type
  TDlgEtiquetas = class(TFrmModeloCadastros)
    ...
    Q_PayRoll: TIBQuery;
    C_PayRoll: TClientDataSet;
    P_PayRoll: TDataSetProvider;
    C_PayRollDS: TDataSource;
    C_PayRollPAYROLL: TIntegerField;
    C_PayRollDESCRICAO: TStringField;
    ...
    procedure tsFuncionariosShow(Sender: TObject);
  ...
  with pgAssistente do begin
    ...
    {Cartas}
      ...
      if (rgClientes.Checked) and (rgSelecionarClientes.Checked) and (lbClientes.Items.Count=0) then begin
        ...
        Exit;
			end else if (rgFuncionarios.Checked) and (rgFuncionariosPayRoll.Checked) and (cmbPayRoll.Text='')  then begin
...
begin
  ...
  sNovaEtiqueta:='';
	...
	sSelectFuncionarios := 'select f.nome, f.endereco, f.cidade, f.uf, f.cep, f.fone1, f.fone2, f.contato, '+
              ...
              'ff.nome as Cliente, pi.descricao as ItemPayroll '+
  						'from favorecidos f '+
              'left join paises p on f.pais = p.pais '+
...
begin
  ...
  with DBEtiquetas do begin
    ...
    DefaultRowHeight := round((edAltura.value+(edEspVertical.value))*Conversor);
    sCampos  := '';
  	for i:=1 to ColumnCount-1 do
      if i <= Colunas then
      	Columns[i].Width	:= DBLayout.Width //round((edLargura.value+edEspHorizontal.value)*Conversor)
      else
          ...
          for j:=1 to Colunas do begin
            ...
            for k:=1 to ContaStrings(sCampos,'|') do //DBLayout.VisibleColumnCount do
         		   fieldbyname('coluna'+IntToStr(j)).AsString := fieldbyname('coluna'+IntToStr(j)).AsString+
																													getCaptionAntes(DBLayOut.FindColumnByFieldName(SeparaStrings(sCampos,'|',k)).Caption)+
               																						getCaption(DBLayOut.FindColumnByFieldName(SeparaStrings(sCampos,'|',k)).Caption)+
                    ...
                    if pos('<',DBLayOut.FindColumnByFieldName(SeparaStrings(sCampos,'|',j)).Caption)>0 then begin
                  	...
                  	sConteudo := formatacampo(q_sql.fieldbyname(SeparaStrings(sCampos,'|',j)));
          				fieldbyname('coluna'+IntToStr(k)).AsString := fieldbyname('coluna'+IntToStr(k)).AsString+
                                                              ...
                                                              //';'+DBLayOut.ColumnsRect.Values[SeparaStrings(sCampos,'|',j)]+
...
begin
  inherited;
  DBEtiquetas.DefaultRowHeight := Round((edAltura.value+edEspVertical.value)*Conversor);
end;
...
begin
  ...
  if rgClientes.Checked then begin
    ...
    sSelectCount := 'select count(favorecido) from favorecidos f '+sWhere;
      ...
      sOrderBy:= 'order by f.nome';
    	sSelect := sSelectFuncionarios+sWhere+iif(sOrdem='',sOrderBy,'order by f.'+sOrdem);
    end else if rgFuncionariosPayRoll.Checked then begin
      sWhere	:= 'where f.tipofavorecido = 3 and f.desativado = ''N'' '+
                 'and (exists( select payrollfunc from PayRollsFunc p where p.payroll = '+iif(VarIsNull(cmbPayRoll.LookUpKeyValue),'0',VarToStr(cmbPayRoll.LookUpKeyValue))+' and p.funcionario = f.favorecido )) ';
      sOrderBy:= 'order by f.nome';
    	sSelect := sSelectFuncionarios+sWhere+iif(sOrdem='',sOrderBy,'order by f.'+sOrdem);
    end else if rgFuncionariosCliente.Checked then begin
      sWhere	:= 'where f.tipofavorecido = 3 and f.desativado = ''N'' '+
                 'and (exists( select contrato from ContratosFav c, FuncsPayItens p where c.favorecido = '+IntToStr(edCliente.ID)+' and c.funcpayitem = p.funcpayitem and p.funcionario = f.favorecido )) ';
      sOrderBy:= 'order by f.nome';
    	sSelect := sSelectFuncionarios+sWhere+iif(sOrdem='',sOrderBy,'order by f.'+sOrdem);
...
procedure TDlgEtiquetas.tsFuncionariosShow(Sender: TObject);
begin
  inherited;
  C_Payroll.Open;
end;
    ...
    with Etiqueta do begin
      ...
      if rgClientes.Checked then
      	...
      	WriteString('Etiqueta', 'Condicao', iif(rgFuncionariosAtivos.Checked,'ATIVOS',
                                            iif(rgFuncionariosPayRoll.Checked,'PAYROLL;'+VarToStr(cmbPayRoll.LookUpKeyValue),
                                            iif(rgFuncionariosCliente.Checked,'CLIENTE;'+IntToStr(edCLiente.ID),'SELECIONADOS;'+sSelecionados))))
...
begin
    ...
    rgFuncionariosPayRoll.Checked := UpperCase(Condicao)='PAYROLL';
    rgFuncionariosCliente.Checked := UpperCase(SeparaStrings(Condicao,';',1))='CLIENTE';
    if rgFuncionariosCliente.Checked then
    	cmbPayRoll.LookupKeyValue 	:= StrToIntDef(SeparaStrings(Condicao,';',2),0);
...
begin
  ...
  while i <= ContaStrings(sTexto,'|') do begin
    ...
    ARight:= 0;
      ...
      // StrToIntDef(SeparaStrings(SeparaStrings(sTexto,'|',i),';',7),10)*100
      ...
      //Area   	:= StrToRect(SeparaStrings(SeparaStrings(sTexto,'|',i),';',4));
      nWidth  := StrToIntDef(SeparaStrings(SeparaStrings(sTexto,'|',i),';',4),400);                                                        
  		R 			:= Rect(ALeft+AClientRect.Left+(Trunc(edMT.IntValue*Conversor)*APixel), ATop+(Trunc(edST.IntValue*Conversor)*APixel), ALeft + AClientRect.Left + (nWidth*APixel), ATop + AlturaAnterior - 1 * APixel + (Trunc(edST.IntValue*Conversor)*APixel));
      ALeft  	:= ALeft + (nWidth*APixel); //Area.Right
