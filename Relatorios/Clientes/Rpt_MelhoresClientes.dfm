inherited RptMelhoresClientes: TRptMelhoresClientes
  Left = 35
  Top = 78
  Height = 460
  Caption = 'Clientes / Vendas'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnTitulo: TTS_MaxPanel
    Gradient.ColorStart = 14859922
    OnDblClick = Ajuda1Click
    inherited lbCaption: TdxfLabel
      Width = 293
      Caption = 'Relat'#243'rio de Melhores Clientes'
      Effect3D.ShadowedColor = 14859922
    end
    inherited btHelp: TTS_SpeedButton
      Top = 0
    end
  end
  inherited pnGrid: TPanel
    Height = 383
    inherited pnDados: TTS_Panel
      Height = 23
      Color = 16116702
      object TS_Label1: TTS_Label
        Left = 0
        Top = 4
        Width = 118
        Caption = 'Calcula a m'#233'dia para '
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label2: TTS_Label
        Left = 163
        Top = 4
        Width = 49
        Alignment = taLeftJustify
        Caption = 'meses'
        FormatoTabela = False
        LinkToResult = 0
      end
      object spCliente: TTS_SpeedButton
        Left = 258
        Top = 1
        Width = 127
        Height = 21
        Caption = 'Escolher Cliente(s)'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Glyph.Data = {
          
          AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA}
        ParentFont = False
        OnClick = spClienteClick2
        RepeatedClick = False
        Border = True
      end
      object EdMedia: TTS_SpinEdit
        Left = 121
        Top = 2
        Width = 37
        TabOrder = 0
        StyleController = DMProjeto.esClientes
        OnChange = EdMediaChange
        Value = 3
        Height = 19
      end
    end
    inherited dbgConsulta: TTS_QDBGrid
      Top = 23
      Height = 341
      Bands = <
        item
          Caption = 'Dados do Cliente'
          Width = 198
        end
        item
          Caption = 'Vendas'
          Width = 472
        end>
      KeyField = 'FAVORECIDO'
      BandColor = 15461355
      Filter.Criteria = {00000000}
      PreviewFieldName = 'PreviewEnd'
      ShowBands = True
      TS_SelectedColumn = 'NOME'
      object dbgConsultaFAVORECIDO: TdxDBGridMaskColumn
        DisableCustomizing = True
        Visible = False
        Width = 77
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FAVORECIDO'
      end
      object dbgConsultaCODIGO: TdxDBGridMaskColumn
        Visible = False
        Width = 70
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CODIGO'
      end
      object dbgConsultaNOME: TdxDBGridMaskColumn
        Sorted = csUp
        Width = 198
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOME'
      end
      object dbgConsultaFONE1: TdxDBGridMaskColumn
        Visible = False
        Width = 118
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FONE1'
      end
      object dbgConsultaTOTALATEHOJE: TdxDBGridMaskColumn
        Caption = 'At'#233' Hoje'
        Width = 68
        BandIndex = 1
        RowIndex = 0
        FieldName = 'TOTALATEHOJE'
      end
      object dbgConsultaTOTALESTEANO: TdxDBGridMaskColumn
        Caption = 'Este Ano'
        Width = 70
        BandIndex = 1
        RowIndex = 0
        FieldName = 'TOTALESTEANO'
      end
      object dbgConsultaMEDIACOMPRAAOMES: TdxDBGridMaskColumn
        Width = 68
        BandIndex = 1
        RowIndex = 0
        FieldName = 'MEDIACOMPRAAOMES'
      end
      object dbgConsultaMEDIACOMPRA: TdxDBGridMaskColumn
        Width = 88
        BandIndex = 1
        RowIndex = 0
        FieldName = 'MEDIACOMPRA'
      end
      object dbgConsultaVOLUME0_30: TdxDBGridMaskColumn
        Alignment = taRightJustify
        HeaderAlignment = taCenter
        Width = 60
        BandIndex = 1
        RowIndex = 0
        FieldName = 'VOLUME0_30'
      end
      object dbgConsultaVOLUME31_60: TdxDBGridMaskColumn
        Alignment = taRightJustify
        HeaderAlignment = taCenter
        Width = 63
        BandIndex = 1
        RowIndex = 0
        FieldName = 'VOLUME31_60'
      end
      object dbgConsultaVOLUME61_90: TdxDBGridMaskColumn
        Alignment = taRightJustify
        HeaderAlignment = taCenter
        Width = 52
        BandIndex = 1
        RowIndex = 0
        FieldName = 'VOLUME61_90'
      end
    end
    inherited PainelFullSelect: TTS_Panel
      Top = 364
      inherited cbFullSelect: TTS_CheckBox
        Style.ButtonStyle = btsSimple
        Height = 19
      end
    end
  end
  inherited TS_Panel1: TTS_Panel
    Height = 383
    inherited btSair: TTS_SpeedButton
      Top = 338
    end
    inherited btAtualizar: TTS_SpeedButton
      OnClick = btAtualizarClick
    end
  end
  inherited DlgMsg: TDlgMsg
    Left = 645
  end
  inherited ppmPadrao: TTS_PopupMenu
    Top = 112
    object Endereo1: TMenuItem [0]
      Caption = 'Endere'#231'o'
      OnClick = Endereo1Click
    end
    object N9: TMenuItem [1]
      Caption = '-'
    end
  end
  inherited Q_Consulta: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    SQL.Strings = (
      
        'select f.favorecido, f.codigo, f.nome, f.fone1, f.endereco, f.ci' +
        'dade, f.uf, f.cep, f.fax, f.fone2, '
      '    (select sum(total) from saidas where tipopadrao=1'
      '    and Situacao='#39'N'#39' and favorecido=f.favorecido)'
      '    as TotalAteHoje,'
      '    (select sum(total) from saidas where tipopadrao=1'
      '    and Situacao='#39'N'#39' and data between :datai and :dataf'
      '    and favorecido=f.favorecido)'
      '    as TotalEsteAno,'
      '    (select sum(s.total)  from saidas s'
      '    where s.tipopadrao = 1 and s.situacao = '#39'N'#39
      '     and s.data between  :dataprazoi30'
      '       and :dataprazof30'
      '    and s.favorecido = f.favorecido)'
      '    as volume0_30,'
      '    (select sum(s.total)  from saidas s'
      '    where s.tipopadrao = 1 and s.situacao = '#39'N'#39
      '     and s.data between :dataprazoi60'
      '       and :dataprazof60'
      '    and s.favorecido = f.favorecido)'
      '    as volume31_60,'
      '    (select sum(s.total)  from saidas s'
      '    where s.tipopadrao = 1 and s.situacao = '#39'N'#39
      '     and s.data between :dataprazoi90'
      '       and :dataprazof90'
      '    and s.favorecido = f.favorecido)'
      '    as volume61_90,'
      '    (select sum(s.total)/3  from saidas s'
      '    where s.tipopadrao = 1 and s.situacao = '#39'N'#39
      '    and s.data between :dataiMedia and :datafMedia'
      '   and s.favorecido = f.favorecido)'
      '   as MediaCompraAoMes,'
      '    (select avg(s.total)  from saidas s'
      '   where s.tipopadrao = 1 and s.situacao = '#39'N'#39
      '   and s.favorecido = f.favorecido)'
      '   as MediaCompra'
      'from favorecidos f'
      'where tipofavorecido=1 and  f.desativado = '#39'N'#39)
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'datai'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'dataf'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazoi30'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazof30'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazoi60'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazof60'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazoi90'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazof90'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'dataiMedia'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'datafMedia'
        ParamType = ptUnknown
      end>
  end
  inherited C_Consulta: TClientDataSet
    Params = <
      item
        DataType = ftUnknown
        Name = 'datai'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'dataf'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazoi30'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazof30'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazoi60'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazof60'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazoi90'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'dataprazof90'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'dataiMedia'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'datafMedia'
        ParamType = ptUnknown
      end>
    OnCalcFields = C_ConsultaCalcFields
    Top = 218
    object C_ConsultaCODIGO: TStringField
      DisplayLabel = 'C'#243'digo'
      FieldName = 'CODIGO'
    end
    object C_ConsultaFAVORECIDO: TIntegerField
      FieldName = 'FAVORECIDO'
      Required = True
    end
    object C_ConsultaNOME: TStringField
      DisplayLabel = 'Nome'
      FieldName = 'NOME'
      Size = 50
    end
    object C_ConsultaFONE1: TStringField
      DisplayLabel = 'Fone'
      FieldName = 'FONE1'
    end
    object C_ConsultaPreviewEnd: TStringField
      FieldKind = fkInternalCalc
      FieldName = 'PreviewEnd'
      Size = 80
    end
    object C_ConsultaENDERECO: TStringField
      FieldName = 'ENDERECO'
      Size = 50
    end
    object C_ConsultaCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 30
    end
    object C_ConsultaUF: TStringField
      FieldName = 'UF'
      Size = 2
    end
    object C_ConsultaCEP: TStringField
      FieldName = 'CEP'
      Size = 10
    end
    object C_ConsultaFAX: TStringField
      FieldName = 'FAX'
    end
    object C_ConsultaFONE2: TStringField
      FieldName = 'FONE2'
    end
    object C_ConsultaMEDIACOMPRAAOMES: TFloatField
      DisplayLabel = 'M'#233'dia/M'#234's'
      FieldName = 'MEDIACOMPRAAOMES'
      DisplayFormat = '###,###,##0.00'
    end
    object C_ConsultaMEDIACOMPRA: TFloatField
      DisplayLabel = 'M'#233'dia/Compra'
      FieldName = 'MEDIACOMPRA'
      DisplayFormat = '###,###,##0.00'
    end
    object C_ConsultaTOTALATEHOJE: TFloatField
      FieldName = 'TOTALATEHOJE'
    end
    object C_ConsultaTOTALESTEANO: TFloatField
      FieldName = 'TOTALESTEANO'
    end
    object C_ConsultaVOLUME0_30: TFloatField
      FieldName = 'VOLUME0_30'
    end
    object C_ConsultaVOLUME31_60: TFloatField
      FieldName = 'VOLUME31_60'
    end
    object C_ConsultaVOLUME61_90: TFloatField
      FieldName = 'VOLUME61_90'
    end
  end
  inherited ppmLayout: TTS_PopupMenu
    Left = 343
    Top = 115
  end
  inherited ppmHelp: TTS_PopupMenu
    inherited Ajuda1: TMenuItem
      HelpContext = 549
    end
  end
  inherited GridPrinter: TdxComponentPrinter
    PreviewOptions.PreviewBoundsRect = {00000000000000000000000000000000}
    inherited GridPrinterLink: TdxDBGridReportLink
      DateTime = 37242.6291927662
      PrinterPage._dxMeasurementUnits_ = 2
      PrinterPage._dxLastMU_ = 2
      BuiltInReportLink = True
    end
  end
  inherited ppmSelTemplate: TTS_PopupMenu
    Left = 624
    Top = 138
  end
  inherited FormsComponent: TFormsComponent
    ModuloStyle = DMProjeto.msClientes
    BarEndColor = 14859922
    CaptionShadow = 14859922
    FormColor = 16116702
    Modulo = 'Clientes / Vendas'
    Caption = 'Relat'#243'rio de Melhores Clientes'
    OnRefresh = FormsComponentRefresh
  end
end
