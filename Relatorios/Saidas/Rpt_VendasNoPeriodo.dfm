inherited RptVendasNoPeriodo: TRptVendasNoPeriodo
  Left = 246
  Top = 126
  Width = 953
  Height = 504
  Caption = 'Clientes / Vendas'
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel [0]
    Left = 224
    Top = 216
    Width = 46
    Height = 13
    Caption = 'ValorFinal'
  end
  inherited pnTitulo: TTS_MaxPanel
    Width = 945
    Gradient.ColorStart = 14859922
    inherited lbCaption: TdxfLabel
      Width = 178
      Caption = 'Vendas no Per'#237'odo'
      Effect3D.ShadowedColor = 14859922
    end
    inherited btHelp: TTS_SpeedButton
      Left = 911
    end
    inherited btTemplates: TTS_SpeedButton
      Left = 884
    end
  end
  inherited pnGrid: TPanel
    Width = 892
    Height = 439
    object Label2: TLabel [0]
      Left = 264
      Top = 216
      Width = 60
      Height = 13
      Caption = 'DESCCAIXA'
    end
    object Label3: TLabel [1]
      Left = 256
      Top = 136
      Width = 14
      Height = 13
      Caption = 'UF'
    end
    inherited pnDados: TTS_Panel
      Width = 892
      Height = 45
      Color = 16116702
      object TS_Label1: TTS_Label
        Left = 0
        Top = 5
        Width = 78
        Caption = 'No per'#237'odo de:'
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label2: TTS_Label
        Left = 151
        Top = 6
        Width = 18
        Caption = ' '#224' '
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label3: TTS_Label
        Left = -3
        Top = 24
        Width = 81
        Height = 15
        Caption = 'Tipo Opera'#231#227'o :'
        FormatoTabela = False
        LinkToResult = 0
      end
      object btClientes: TTS_SpeedButton
        Left = 0
        Top = 45
        Width = 82
        Height = 20
        Caption = '&Clientes'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Glyph.Data = {
          
          E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
        ParentFont = False
        Visible = False
        OnClick = btClientesClick
        RepeatedClick = False
        Border = True
      end
      object btVendedor: TTS_SpeedButton
        Left = 659
        Top = 4
        Width = 77
        Height = 21
        Caption = '&Vendedor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Glyph.Data = {
          
          E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
        ParentFont = False
        OnClick = btVendedorClick
        RepeatedClick = False
        Border = True
      end
      object DataI: TTS_DateTimePicker
        Left = 78
        Top = 3
        Width = 81
        TabOrder = 0
        AutoSize = False
        StyleController = DMProjeto.esClientes
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = DataIDateChange
        Height = 19
        StoredValues = 4
      end
      object DataF: TTS_DateTimePicker
        Left = 169
        Top = 3
        Width = 81
        TabOrder = 1
        AutoSize = False
        StyleController = DMProjeto.esClientes
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = DataIDateChange
        Height = 19
        StoredValues = 4
      end
      object cbCanceladas: TTS_CheckBox
        Left = 655
        Top = 23
        Width = 82
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 2
        Caption = 'Canceladas'
        StyleController = DMProjeto.esClientes
        OnChange = cbCanceladasChange
        DisableEdit = False
        Height = 19
      end
      object ppeTipoOperacao: TTS_PopupEdit
        Left = 80
        Top = 22
        Width = 170
        TabOrder = 3
        AutoSelect = False
        StyleController = DMProjeto.esClientes
        HideEditCursor = True
        PopupControl = clbTipoOperacao
        PopupFormBorderStyle = pbsSysPanel
        OnCloseUp = ppeTipoOperacaoCloseUp
        LookupKeyValue = 0
        Height = 19
      end
      object popupFavorecido: TTS_PopupEdit
        Left = 33
        Top = 48
        Width = 115
        TabOrder = 4
        Visible = False
        AutoSelect = False
        StyleController = DMProjeto.esClientes
        Text = 'Filtrar Cliente ...'
        HideEditCursor = True
        PopupControl = FrmFiltroFavorecidos.pnPrincipal
        PopupFormBorderStyle = pbsSysPanel
        OnCloseUp = popupFavorecidoCloseUp
        LookupKeyValue = 0
        Height = 19
      end
      object cbFonte: TTS_CheckBox
        Left = 270
        Top = 4
        Width = 151
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 5
        Caption = 'Apenas os Clientes Fontes'
        StyleController = DMProjeto.esClientes
        NullStyle = nsUnchecked
        DisableEdit = False
        Height = 19
      end
      object cbNormal: TTS_CheckBox
        Left = 270
        Top = 23
        Width = 173
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 6
        Caption = 'Apenas os Clientes N'#227'o Fontes'
        StyleController = DMProjeto.esClientes
        NullStyle = nsUnchecked
        DisableEdit = False
        Height = 19
      end
      object cbISENTOS: TTS_CheckBox
        Left = 455
        Top = 22
        Width = 179
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 7
        Caption = 'Apenas os Clientes Isentos'
        StyleController = DMProjeto.esClientes
        NullStyle = nsUnchecked
        DisableEdit = False
        Height = 16
      end
      object cbSemInscricao: TTS_CheckBox
        Left = 452
        Top = 4
        Width = 189
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 8
        Caption = 'Apenas os Clientes Sem Inscri'#231#227'o'
        StyleController = DMProjeto.esClientes
        NullStyle = nsUnchecked
        DisableEdit = False
        Height = 17
      end
    end
    inherited dbgConsulta: TTS_QDBGrid
      Top = 45
      Width = 892
      Height = 375
      KeyField = 'SAIDA'
      SummaryGroups = <
        item
          DefaultGroup = True
          SummaryItems = <
            item
              SummaryField = 'Data'
              SummaryFormat = '>Data=###0'
              SummaryType = cstCount
            end
            item
              SummaryField = 'valorfinal'
              SummaryFormat = '>valorfinal=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'TOTAL'
              SummaryFormat = '>TOTAL=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'TOTALITENS'
              SummaryFormat = '>TOTALITENS=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'DESCONTO'
              SummaryFormat = '>DESCONTO=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'DEVOLUCAO'
              SummaryFormat = '>DEVOLUCAO=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'valorcomissao'
              SummaryFormat = '>valorcomissao=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'valoricms'
              SummaryFormat = '>valoricms=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'valoricmssubst'
              SummaryFormat = '>valoricmssubst=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'basecalcicms'
              SummaryFormat = '>basecalcicms=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'basecalcsubst'
              SummaryFormat = '>basecalcsubst=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'VENDEDOR'
              SummaryFormat = '>NOMEVENDEDOR=###0'
              SummaryType = cstCount
            end>
          Name = 'Default'
        end>
      Filter.Criteria = {00000000}
      TS_SelectedColumn = 'NUMERO'
      TS_SummaryFields.Strings = (
        'Data;Count'
        'valorfinal;Sum'
        'TOTAL;SUM'
        'TOTALITENS;SUM'
        'DESCONTO;SUM'
        'DEVOLUCAO;SUM'
        'Imposto;Sum'
        'valorcomissao;Sum'
        'Comissao;Sum'
        'valoricms;Sum'
        'valoricmssubst;Sum'
        'basecalcicms;SUM'
        'basecalcsubst;SUM')
      object dbgConsultaSAIDA: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 33
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SAIDA'
      end
      object dbgConsultaNUMERO: TdxDBGridMaskColumn
        Caption = 'N'#250'mero'
        HeaderAlignment = taCenter
        Width = 68
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NUMERO'
      end
      object dbgConsultaDATA: TdxDBGridDateColumn
        Caption = 'Data'
        HeaderAlignment = taCenter
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATA'
        SummaryFooterType = cstCount
        SummaryFooterFormat = '###0'
      end
      object dbgConsultaHora: TdxDBGridTimeColumn
        Caption = 'Hora Vnd.'
        Width = 60
        BandIndex = 0
        RowIndex = 0
        FieldName = 'HORA'
      end
      object dbgConsultaMesAno: TdxDBGridColumn
        Caption = 'M'#234's/Ano'
        HeaderAlignment = taCenter
        Visible = False
        Width = 79
        BandIndex = 0
        RowIndex = 0
        FieldName = 'MESANO'
      end
      object dbgConsultaNOMECLIENTE: TdxDBGridMaskColumn
        Caption = 'Cliente'
        HeaderAlignment = taCenter
        Width = 134
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMECLIENTE'
      end
      object dbgConsultaFAVORECIDO: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 37
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FAVORECIDO'
      end
      object dbgConsultaGRUPOCLIENTE: TdxDBGridMaskColumn
        Caption = 'Grupo Cliente'
        HeaderAlignment = taCenter
        Visible = False
        Width = 71
        BandIndex = 0
        RowIndex = 0
        FieldName = 'GRUPOCLIENTE'
      end
      object dbgConsultaTOTALITEM: TdxDBGridColumn
        Caption = 'Vlr. Venda'
        HeaderAlignment = taCenter
        Width = 72
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TOTALITENS'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaDESCONTO: TdxDBGridMaskColumn
        Caption = 'Desconto'
        HeaderAlignment = taCenter
        Width = 63
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCONTO'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaTotal: TdxDBGridColumn
        Caption = 'Total'
        HeaderAlignment = taCenter
        Width = 118
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TOTAL'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaValorFinal: TdxDBGridMaskColumn
        Caption = 'Vlr. Final'
        HeaderAlignment = taCenter
        Visible = False
        Width = 75
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ValorFinal'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaDevolucao: TdxDBGridColumn
        Caption = 'Devolu'#231#227'o'
        HeaderAlignment = taCenter
        Width = 75
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DEVOLUCAO'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaNOMEVENDEDOR: TdxDBGridMaskColumn
        Caption = 'Vendedor'
        HeaderAlignment = taCenter
        Sorted = csUp
        Width = 81
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMEVENDEDOR'
        SummaryFooterType = cstCount
        SummaryFooterFormat = '###0'
      end
      object dbgConsultaVALORCOMISSAO: TdxDBGridMaskColumn
        Caption = 'Comiss'#227'o'
        HeaderAlignment = taCenter
        Width = 67
        BandIndex = 0
        RowIndex = 0
        FieldName = 'VALORCOMISSAO'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaDATAENTREGA: TdxDBGridDateColumn
        Caption = 'Data Entrega'
        HeaderAlignment = taCenter
        Visible = False
        Width = 37
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATAENTREGA'
      end
      object dbgConsultaDESCPLANOPAGAMENTO: TdxDBGridMaskColumn
        Caption = 'Plano Pagamento'
        HeaderAlignment = taCenter
        Width = 94
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCPLANOPAGAMENTO'
      end
      object dbgConsultaDESCCAIXA: TdxDBGridMaskColumn
        Caption = 'Caixa'
        HeaderAlignment = taCenter
        Visible = False
        Width = 217
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCCAIXA'
      end
      object dbgConsultaDESCSTATUS: TdxDBGridMaskColumn
        Caption = 'Status'
        HeaderAlignment = taCenter
        Visible = False
        Width = 81
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCSTATUS'
      end
      object dbgConsultaOBS: TdxDBGridMaskColumn
        Caption = 'Observa'#231#245'es'
        HeaderAlignment = taCenter
        Visible = False
        Width = 717
        BandIndex = 0
        RowIndex = 0
        FieldName = 'OBS'
      end
      object dbgConsultaJUROS: TdxDBGridMaskColumn
        Caption = 'Juros'
        HeaderAlignment = taCenter
        Visible = False
        Width = 53
        BandIndex = 0
        RowIndex = 0
        FieldName = 'JUROS'
      end
      object dbgConsultaFRETE: TdxDBGridCurrencyColumn
        Caption = 'Frete'
        HeaderAlignment = taCenter
        Visible = False
        Width = 53
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FRETE'
        DisplayFormat = ',0.00;-,0.00'
        Nullable = False
      end
      object dbgConsultaColumn20: TdxDBGridColumn
        Caption = 'Tabela de Pre'#231'o'
        HeaderAlignment = taCenter
        Visible = False
        Width = 100
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCTABELAPRECO'
      end
      object dbgConsultaBASECALCICMS: TdxDBGridColumn
        Caption = 'Base ICMS'
        HeaderAlignment = taCenter
        Width = 118
        BandIndex = 0
        RowIndex = 0
        FieldName = 'BASECALCICMS'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaBaseSubT: TdxDBGridColumn
        Caption = 'Base Subst.'
        HeaderAlignment = taCenter
        Width = 118
        BandIndex = 0
        RowIndex = 0
        FieldName = 'BASECALCSUBST'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaValorICMS: TdxDBGridColumn
        Caption = 'Valor ICMS'
        HeaderAlignment = taCenter
        Width = 118
        BandIndex = 0
        RowIndex = 0
        FieldName = 'VALORICMS'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaValorSubT: TdxDBGridColumn
        Caption = 'Valor Subst.'
        HeaderAlignment = taCenter
        Width = 118
        BandIndex = 0
        RowIndex = 0
        FieldName = 'VALORICMSSUBST'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaSubstituicao: TdxDBGridColumn
        Caption = 'Substitui'#231#227'o'
        HeaderAlignment = taCenter
        Width = 76
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SUBSTTRIB'
      end
      object dbgConsultaDESCTIPOCOBRANCA: TdxDBGridMaskColumn
        Caption = 'Tipo de Cobran'#231'a'
        HeaderAlignment = taCenter
        Visible = False
        Width = 131
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCTIPOCOBRANCA'
      end
      object dbgConsultaInscricao_Est: TdxDBGridColumn
        Caption = 'Insc. Est.'
        BandIndex = 0
        RowIndex = 0
        FieldName = 'INSCRICAO_EST'
      end
      object dbgConsultaENDERECO: TdxDBGridColumn
        Caption = 'Endere'#231'o'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ENDERECO'
      end
      object dbgConsultaBairro: TdxDBGridColumn
        Caption = 'Bairro'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'BAIRRO'
      end
      object dbgConsultaCIDADE: TdxDBGridColumn
        Caption = 'Cidade'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CIDADE'
      end
      object dbgConsultaUF: TdxDBGridColumn
        Caption = 'UF.'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'UF'
      end
      object dbgConsultaLOGINNAME: TdxDBGridColumn
        Caption = 'Usuario Canc.'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'LOGINNAME'
      end
      object dbgConsultaMOTIVOCANCELAMENTO: TdxDBGridColumn
        Caption = 'Motivo Canc.'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'MOTIVOCANCELAMENTO'
      end
      object dbgConsultaCEP: TdxDBGridColumn
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CEP'
      end
      object dbgConsultaCAMPO01: TdxDBGridColumn
        Caption = 'Cust01'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CAMPO01'
      end
      object dbgConsultaCAMPO02: TdxDBGridColumn
        Caption = 'Cust02'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CAMPO02'
      end
      object dbgConsultaCAMPO03: TdxDBGridColumn
        Caption = 'Cust03'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CAMPO03'
      end
      object dbgConsultaCAMPO04: TdxDBGridColumn
        Caption = 'Cust04'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CAMPO04'
      end
      object dbgConsultaProfissional: TdxDBGridColumn
        Caption = 'Profissional'
        BandIndex = 0
        RowIndex = 0
        FieldName = 'PROFISSIONAL'
      end
      object dbgConsultaTIPOPESSOA: TdxDBGridMaskColumn
        Caption = 'Tipo Pessoa'
        HeaderAlignment = taCenter
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TIPOPESSOA'
      end
      object dbgConsultaNumRemessa: TdxDBGridColumn
        Caption = 'Num. Remessa'
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NUMREMESSA'
      end
      object dbgConsultaColumn46: TdxDBGridColumn
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CPF_CNPJ'
      end
    end
    inherited PainelFullSelect: TTS_Panel
      Top = 420
      Width = 892
      inherited cbFullSelect: TTS_CheckBox
        Style.ButtonStyle = btsSimple
        Height = 19
      end
    end
    object clbTipoOperacao: TTS_CheckListBox
      Left = 64
      Top = 64
      Width = 150
      Height = 123
      BorderStyle = bsNone
      Ctl3D = False
      ItemHeight = 13
      ParentCtl3D = False
      TabOrder = 4
      Visible = False
      KeyField = 'TIPOMOVIMENTO'
      ListField = 'DESCRICAO'
      Transparent = False
    end
  end
  inherited TS_Panel1: TTS_Panel
    Height = 439
    inherited btSair: TTS_SpeedButton
      Top = 394
    end
    inherited btAtualizar: TTS_SpeedButton
      OnClick = btAtualizarClick
    end
  end
  inherited DlgMsg: TDlgMsg
    Left = 605
    Top = 345
  end
  inherited LastDataObject: TTS_LastDataObject
    Left = 539
    Top = 287
  end
  inherited ppmPadrao: TTS_PopupMenu
    Left = 162
  end
  inherited FormStorage: TFormStorage
    Left = 120
  end
  inherited Q_Consulta: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    SQL.Strings = (
      
        'select s.saida, s.data, s.DTMODIFICACAO, s.totalitens, s.Devoluc' +
        'ao, s.total - s.devolucao as Total, s.imposto, s.numero, s.datae' +
        'ntrega, s.valorcomissao,  s.frete, s.obs, s.desconto, s.juros, s' +
        '.vendedor,  s.valoricms, s.valoricmssubst, s.basecalcicms, s.bas' +
        'ecalcsubst, s.substtrib, s.favorecido, S.tipomovimento, f.tipofa' +
        'vorecido, f.nome as nomecliente,  f.cpf_cnpj, f.cidade, f.bairro' +
        ', f.cep, f.endereco, f.uf, f.fone1, v.nome as nomevendedor,  gc.' +
        'descricao as grupocliente,  p.descricao as descplanopagamento, c' +
        '.descricao as desccaixa, st.descricao as descstatus,  tp.descric' +
        'ao as desctabelapreco,  tc.descricao as desctipocobranca, f.ENDE' +
        'RECO, f.bairro, f.cidade, f.uf, s.HORA, f.inscricao_est,u.loginn' +
        'ame, s.motivocancelamento,s.campo01, s.campo02, s.campo03, s.cam' +
        'po04, m.nome as Profissional, case when upper(f.pessoa_fj) = '#39'F'#39 +
        ' then '#39'F'#237'sica'#39' when upper(f.pessoa_fj) = '#39'J'#39' then '#39'Jur'#237'dica'#39' els' +
        'e '#39'N'#227'o Definido'#39' end as tipopessoa, NumRemessa'
      'from saidas s'
      'inner join favorecidos f on f.favorecido = s.favorecido'
      'left join favorecidos v on v.favorecido = s.vendedor'
      'left join usuarios u on u.usuario = s.funccancelamento'
      'left join gruposcliente gc on gc.grupocliente = f.grupocliente'
      
        'left join planospagamento p on p.planopagamento = s.planopagamen' +
        'to'
      'left join statussaidas st on st.status = s.status'
      'left join contas c on c.conta = s.caixa'
      'left join tabelaspreco tp on tp.tabelapreco = s.tabelapadrao'
      'left join tiposcobranca tc on tc.tipocobranca = s.cobranca'
      'left join favorecidos m on m.favorecido = s.medico')
    Left = 340
    Top = 113
  end
  inherited C_ConsultaDS: TDataSource
    Left = 339
  end
  inherited P_Consulta: TDataSetProvider
    Left = 341
  end
  inherited C_Consulta: TClientDataSet
    OnCalcFields = C_ConsultaCalcFields
    Left = 342
    Top = 217
    object C_ConsultaValorFinal: TCurrencyField
      FieldKind = fkInternalCalc
      FieldName = 'ValorFinal'
    end
    object C_ConsultaMesAno: TStringField
      FieldKind = fkInternalCalc
      FieldName = 'MesAno'
      Size = 8
    end
    object C_ConsultaSAIDA: TIntegerField
      FieldName = 'SAIDA'
      Required = True
    end
    object C_ConsultaDATA: TDateField
      FieldName = 'DATA'
    end
    object C_ConsultaDTMODIFICACAO: TDateTimeField
      FieldName = 'DTMODIFICACAO'
    end
    object C_ConsultaNUMERO: TStringField
      FieldName = 'NUMERO'
      Size = 12
    end
    object C_ConsultaDATAENTREGA: TDateField
      FieldName = 'DATAENTREGA'
    end
    object C_ConsultaOBS: TStringField
      FieldName = 'OBS'
      Size = 500
    end
    object C_ConsultaVENDEDOR: TIntegerField
      FieldName = 'VENDEDOR'
    end
    object C_ConsultaFAVORECIDO: TIntegerField
      FieldName = 'FAVORECIDO'
    end
    object C_ConsultaTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
    end
    object C_ConsultaTIPOFAVORECIDO: TIntegerField
      FieldName = 'TIPOFAVORECIDO'
    end
    object C_ConsultaNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Size = 50
    end
    object C_ConsultaCPF_CNPJ: TStringField
      FieldName = 'CPF_CNPJ'
      Size = 14
    end
    object C_ConsultaCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 30
    end
    object C_ConsultaBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Size = 30
    end
    object C_ConsultaCEP: TStringField
      FieldName = 'CEP'
      Size = 10
    end
    object C_ConsultaENDERECO: TStringField
      FieldName = 'ENDERECO'
      Size = 50
    end
    object C_ConsultaUF: TStringField
      FieldName = 'UF'
      Size = 2
    end
    object C_ConsultaFONE1: TStringField
      FieldName = 'FONE1'
    end
    object C_ConsultaNOMEVENDEDOR: TStringField
      FieldName = 'NOMEVENDEDOR'
      Size = 50
    end
    object C_ConsultaGRUPOCLIENTE: TStringField
      FieldName = 'GRUPOCLIENTE'
      Size = 30
    end
    object C_ConsultaDESCPLANOPAGAMENTO: TStringField
      FieldName = 'DESCPLANOPAGAMENTO'
      Size = 30
    end
    object C_ConsultaDESCCAIXA: TStringField
      FieldName = 'DESCCAIXA'
      Size = 50
    end
    object C_ConsultaDESCSTATUS: TStringField
      FieldName = 'DESCSTATUS'
      Size = 30
    end
    object C_ConsultaDESCTABELAPRECO: TStringField
      FieldName = 'DESCTABELAPRECO'
      Size = 25
    end
    object C_ConsultaDESCTIPOCOBRANCA: TStringField
      FieldName = 'DESCTIPOCOBRANCA'
      Size = 50
    end
    object C_ConsultaHORA: TTimeField
      FieldName = 'HORA'
    end
    object C_ConsultaINSCRICAO_EST: TStringField
      FieldName = 'INSCRICAO_EST'
      Size = 14
    end
    object C_ConsultaLOGINNAME: TStringField
      FieldName = 'LOGINNAME'
      Size = 30
    end
    object C_ConsultaMOTIVOCANCELAMENTO: TStringField
      FieldName = 'MOTIVOCANCELAMENTO'
      Size = 80
    end
    object C_ConsultaCAMPO01: TStringField
      FieldName = 'CAMPO01'
      Size = 50
    end
    object C_ConsultaCAMPO02: TStringField
      FieldName = 'CAMPO02'
      Size = 50
    end
    object C_ConsultaCAMPO03: TStringField
      FieldName = 'CAMPO03'
      Size = 50
    end
    object C_ConsultaCAMPO04: TStringField
      FieldName = 'CAMPO04'
      Size = 50
    end
    object C_ConsultaPROFISSIONAL: TStringField
      FieldName = 'PROFISSIONAL'
      Size = 50
    end
    object C_ConsultaTOTALITENS: TFloatField
      FieldName = 'TOTALITENS'
    end
    object C_ConsultaDEVOLUCAO: TFloatField
      FieldName = 'DEVOLUCAO'
    end
    object C_ConsultaTOTAL: TFloatField
      FieldName = 'TOTAL'
    end
    object C_ConsultaIMPOSTO: TFloatField
      FieldName = 'IMPOSTO'
    end
    object C_ConsultaVALORCOMISSAO: TFloatField
      FieldName = 'VALORCOMISSAO'
    end
    object C_ConsultaFRETE: TFloatField
      FieldName = 'FRETE'
    end
    object C_ConsultaDESCONTO: TFloatField
      FieldName = 'DESCONTO'
    end
    object C_ConsultaJUROS: TFloatField
      FieldName = 'JUROS'
    end
    object C_ConsultaVALORICMS: TFloatField
      FieldName = 'VALORICMS'
    end
    object C_ConsultaVALORICMSSUBST: TFloatField
      FieldName = 'VALORICMSSUBST'
    end
    object C_ConsultaBASECALCICMS: TFloatField
      FieldName = 'BASECALCICMS'
    end
    object C_ConsultaBASECALCSUBST: TFloatField
      FieldName = 'BASECALCSUBST'
    end
    object C_ConsultaSUBSTTRIB: TStringField
      FieldName = 'SUBSTTRIB'
      Size = 1
    end
    object C_ConsultaTIPOPESSOA: TStringField
      FieldName = 'TIPOPESSOA'
      FixedChar = True
      Size = 12
    end
    object C_ConsultaNUMREMESSA: TStringField
      FieldName = 'NUMREMESSA'
      Size = 10
    end
  end
  inherited ppmDados: TTS_PopupMenu
    Left = 219
    Top = 145
  end
  inherited ppmAgrupamento: TTS_PopupMenu
    Left = 225
    Top = 198
  end
  inherited ppmLayout: TTS_PopupMenu
    Left = 232
    Top = 95
  end
  inherited ppmImprimir: TTS_PopupMenu
    Left = 228
    Top = 296
  end
  inherited ppmSumarizacao: TTS_PopupMenu
    Left = 227
    Top = 249
  end
  inherited ppmExportacao: TTS_PopupMenu
    Left = 231
    Top = 349
    object LayoutVendasSellOUT1: TMenuItem
      Caption = 'Layout Vendas (Sell OUT)'
      OnClick = LayoutVendasSellOUT1Click
    end
  end
  inherited ppmHelp: TTS_PopupMenu
    Left = 665
  end
  inherited GridPrinter: TdxComponentPrinter
    PreviewOptions.PreviewBoundsRect = {00000000000000000000000000000000}
    Left = 87
    Top = 244
    inherited GridPrinterLink: TdxDBGridReportLink
      DateTime = 39181.3361838079
      PrinterPage._dxMeasurementUnits_ = 2
      PrinterPage._dxLastMU_ = 2
      BuiltInReportLink = True
    end
  end
  inherited dxPSEngineController1: TdxPSEngineController
    Left = 104
    Top = 298
  end
  inherited ppmTemplates: TTS_PopupMenu
    Left = 148
    Top = 366
  end
  inherited Q_SQLReport: TIBQuery
    Left = 664
  end
  inherited ppmSelTemplate: TTS_PopupMenu
    Left = 664
    Top = 88
  end
  inherited FormsComponent: TFormsComponent
    BeforeClearParams = FormsComponentBeforeClearParams
    ModuloStyle = DMProjeto.msClientes
    BarEndColor = 14859922
    CaptionShadow = 14859922
    FormColor = 16116702
    Modulo = 'Clientes / Vendas'
    Caption = 'Vendas no Per'#237'odo'
    Left = 663
  end
  object Q_TiposMovimentos: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'Select TIPOMOVIMENTO, DESCRICAO'
      
        'from TIPOSMOVIMENTO where   TIPOPADRAO in (1,4,5,8,40) order by ' +
        'TIPOPADRAO ,DESCRICAO')
    Left = 456
    Top = 111
    object Q_TiposMovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'TIPOSMOVIMENTO.TIPOMOVIMENTO'
      Required = True
    end
    object Q_TiposMovimentosDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = 'TIPOSMOVIMENTO.DESCRICAO'
      Size = 50
    end
  end
  object P_TiposMovimentos: TDataSetProvider
    DataSet = Q_TiposMovimentos
    Constraints = True
    Left = 456
    Top = 168
  end
  object C_TiposMovimentos: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'TIPOMOVIMENTO'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'DESCRICAO'
        DataType = ftString
        Size = 30
      end>
    IndexDefs = <
      item
        Name = 'DEFAULT_ORDER'
      end
      item
        Name = 'CHANGEINDEX'
      end>
    Params = <>
    ProviderName = 'P_TiposMovimentos'
    StoreDefs = True
    Left = 456
    Top = 224
    object C_TiposMovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'TIPOSMOVIMENTO.TIPOMOVIMENTO'
      Required = True
    end
    object C_TiposMovimentosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 30
    end
  end
  object C_TipoMovimentoDS: TDataSource
    DataSet = C_TiposMovimentos
    Left = 456
    Top = 280
  end
end
