...
{ Inclução de impressão para pedido de 1 Via em saidas César 10-10-2017}
        else if (CB_Documento = 'S') and (sModelo = 'P!V') then
            begin
                if Template <> 0 then
                    begin
...
//                subComposicaoItens4.Visible := (C_InvoiceCB_ComposicaoItens.Value = 'S');
       ...
       {inicio pedido com grade -- copia do pedido uma via com  alteracao da quantidade de itens na propriedade do relatorio  }

       else if (CB_Documento = 'S') and (sModelo = 'PCG') then
            begin
                if Template <> 0 then
                    begin
                        rbPedidoGrade.Template.FileName := sArquivo;
                        if fileexists(rbPedidoGrade.Template.FileName) then
                            rbPedidoGrade.Template.LoadFromFile
                        else
                            rbPedidoGrade.Template.FileName := '';
                    end
                else
                    rbPedidoGrade.Template.FileName := '';
...
//                subComposicaoItens4.Visible := (C_InvoiceCB_ComposicaoItens.Value = 'S');
                ...
                if (impressaoPadrao = 'S') then
                    begin
                        rbPedidoGrade.PrinterSetup.PrinterName := nomeImpressora;
                        rbPedidoGrade.PrinterSetup.PaperName := papelImpressora;
                    end;

                if ImpressaoPara = 'S' then
                    begin
                        rbPedidoGrade.DeviceType := 'Printer';
                        rbPedidoGrade.Print;
                    end
                else if ImpressaoPara = 'C' then
                    begin
                    ...
                    begin
                        rbPedidoGrade.DeviceType := 'Screen';
                        rbPedidoGrade.Print;
                    end
                else
                    EnviarEmail(rbPedidoGrade, DMProjeto.RelPath + C_InvoiceDescricao.asString + ' ' + C_InvoiceInvoice.asstring + '.jpg');
            end


       { final do pedido com grade }
