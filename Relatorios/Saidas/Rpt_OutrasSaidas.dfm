inherited RptOutrasSaidas: TRptOutrasSaidas
  Left = -4
  Top = -4
  Width = 1608
  Height = 864
  Caption = 'Clientes / Outras Saidas'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnTitulo: TTS_MaxPanel
    Width = 1600
    Gradient.ColorStart = 14859922
    inherited lbCaption: TdxfLabel
      Width = 138
      Caption = 'Outras Saidas.'
      Effect3D.ShadowedColor = 14859922
    end
    inherited btHelp: TTS_SpeedButton
      Left = 1569
    end
    inherited btTemplates: TTS_SpeedButton
      Left = 1542
    end
  end
  inherited pnGrid: TPanel
    Width = 1547
    Height = 798
    inherited pnDados: TTS_Panel
      Width = 1547
      Height = 25
      Color = 16116702
      object TS_Label1: TTS_Label
        Left = -2
        Top = 5
        Width = 78
        Caption = 'No per'#237'odo de:'
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label2: TTS_Label
        Left = 151
        Top = 6
        Width = 18
        Caption = ' '#224' '
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label3: TTS_Label
        Left = 251
        Top = 6
        Width = 81
        Height = 15
        Caption = 'Tipo Opera'#231#227'o :'
        FormatoTabela = False
        LinkToResult = 0
      end
      object btClientes: TTS_SpeedButton
        Left = 456
        Top = 2
        Width = 82
        Height = 20
        Caption = '&Clientes'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Glyph.Data = {
          
          E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
        ParentFont = False
        OnClick = btClientesClick
        RepeatedClick = False
        Border = True
      end
      object btVendedor: TTS_SpeedButton
        Left = 543
        Top = 2
        Width = 77
        Height = 20
        Caption = '&Vendedor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Glyph.Data = {
          
          E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
        ParentFont = False
        OnClick = btVendedorClick
        RepeatedClick = False
        Border = True
      end
      object DataI: TTS_DateTimePicker
        Left = 78
        Top = 3
        Width = 81
        TabOrder = 0
        AutoSize = False
        StyleController = DMProjeto.esClientes
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = DataIDateChange
        Height = 19
        StoredValues = 4
      end
      object DataF: TTS_DateTimePicker
        Left = 169
        Top = 3
        Width = 81
        TabOrder = 1
        AutoSize = False
        StyleController = DMProjeto.esClientes
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = DataIDateChange
        Height = 19
        StoredValues = 4
      end
      object cbCanceladas: TTS_CheckBox
        Left = 625
        Top = 3
        Width = 82
        TabOrder = 2
        Caption = 'Canceladas'
        OnChange = cbCanceladasChange
        DisableEdit = False
        Height = 19
      end
      object TS_PopupEdit1: TTS_PopupEdit
        Left = 336
        Top = 4
        Width = 115
        TabOrder = 3
        AutoSelect = False
        StyleController = DMProjeto.esClientes
        HideEditCursor = True
        PopupControl = clbTipoOperacao
        PopupFormBorderStyle = pbsSysPanel
        OnCloseUp = TS_PopupEdit1CloseUp
        LookupKeyValue = 0
        Height = 19
      end
    end
    inherited dbgConsulta: TTS_QDBGrid
      Top = 25
      Width = 1547
      Height = 754
      KeyField = 'SAIDA'
      SummaryGroups = <
        item
          DefaultGroup = True
          SummaryItems = <
            item
              SummaryField = 'Data'
              SummaryFormat = '>Data=###0'
              SummaryType = cstCount
            end
            item
              SummaryField = 'Total'
              SummaryFormat = '>Total=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'TOTALITENS'
              SummaryFormat = '>TOTALITENS=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'DESCONTO'
              SummaryFormat = '>DESCONTO=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'basecalcicms'
              SummaryFormat = '>basecalcicms=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'valoricms'
              SummaryFormat = '>valoricms=#,###,##0.00'
              SummaryType = cstSum
            end>
          Name = 'Default'
        end>
      Filter.Criteria = {00000000}
      TS_SelectedColumn = 'DATA'
      TS_SummaryFields.Strings = (
        'Data;Count'
        'Total;Sum'
        'TOTALITENS;SUM'
        'DESCONTO;SUM'
        'Imposto;Sum'
        'valorcomissao;Sum'
        'Comissao;Sum'
        'basecalcicms;Sum'
        'valoricms;Sum')
      object dbgConsultaSAIDA: TdxDBGridMaskColumn
        DisableCustomizing = True
        Visible = False
        Width = 38
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SAIDA'
      end
      object dbgConsultaDATA: TdxDBGridDateColumn
        HeaderAlignment = taCenter
        Width = 64
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATA'
        SummaryFooterType = cstCount
        SummaryFooterFormat = '###0'
      end
      object dbgConsultaMesAno: TdxDBGridColumn
        Caption = 'MesAno'
        Width = 82
        BandIndex = 0
        RowIndex = 0
        FieldName = 'MESANO'
      end
      object dbgConsultaNUMERO: TdxDBGridMaskColumn
        Caption = 'N'#250'mero'
        HeaderAlignment = taCenter
        Width = 68
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NUMERO'
      end
      object dbgConsultaNOMECLIENTE: TdxDBGridMaskColumn
        Caption = 'Cliente'
        HeaderAlignment = taCenter
        Width = 152
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMECLIENTE'
      end
      object dbgConsultaFAVORECIDO: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 42
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FAVORECIDO'
      end
      object dbgConsultaTOTALITEM: TdxDBGridColumn
        Caption = 'Vlr. Venda'
        Width = 72
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TOTALITENS'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaCUSTO: TdxDBGridMaskColumn
        Caption = 'Custo'
        HeaderAlignment = taCenter
        Sorted = csUp
        Visible = False
        Width = 122
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CUSTO'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaDESCONTO: TdxDBGridMaskColumn
        Caption = 'Desconto'
        HeaderAlignment = taCenter
        Width = 63
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCONTO'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaTOTAL: TdxDBGridMaskColumn
        Caption = 'Vlr. Final'
        HeaderAlignment = taCenter
        Width = 75
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TOTAL'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaNOMEVENDEDOR: TdxDBGridMaskColumn
        Caption = 'Vendedor'
        HeaderAlignment = taCenter
        Width = 82
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMEVENDEDOR'
      end
      object dbgConsultaDATAENTREGA: TdxDBGridDateColumn
        Caption = 'Data Entrega'
        HeaderAlignment = taCenter
        Visible = False
        Width = 42
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATAENTREGA'
      end
      object dbgConsultaDESCCAIXA: TdxDBGridMaskColumn
        Caption = 'Caixa'
        HeaderAlignment = taCenter
        Visible = False
        Width = 244
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCCAIXA'
      end
      object dbgConsultaDESCSTATUS: TdxDBGridMaskColumn
        Caption = 'Status'
        HeaderAlignment = taCenter
        Visible = False
        Width = 93
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCSTATUS'
      end
      object dbgConsultaOBS: TdxDBGridMaskColumn
        Caption = 'Observa'#231#245'es'
        HeaderAlignment = taCenter
        Visible = False
        Width = 810
        BandIndex = 0
        RowIndex = 0
        FieldName = 'OBS'
      end
      object dbgConsultaJUROS: TdxDBGridMaskColumn
        Caption = 'Juros'
        HeaderAlignment = taCenter
        Visible = False
        Width = 60
        BandIndex = 0
        RowIndex = 0
        FieldName = 'JUROS'
      end
      object dbgConsultaFRETE: TdxDBGridCurrencyColumn
        Caption = 'Frete'
        HeaderAlignment = taCenter
        Visible = False
        Width = 60
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FRETE'
        Nullable = False
      end
      object dbgConsultaColumn20: TdxDBGridColumn
        Caption = 'Tabela de Pre'#231'o'
        HeaderAlignment = taCenter
        Visible = False
        Width = 112
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCTABELAPRECO'
      end
      object dbgConsultaNOMEMEDICO: TdxDBGridMaskColumn
        Caption = 'M'#233'dico'
        Visible = False
        Width = 285
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMEMEDICO'
      end
      object dbgConsultaMEDICO: TdxDBGridMaskColumn
        DisableCustomizing = True
        Visible = False
        Width = 59
        BandIndex = 0
        RowIndex = 0
        FieldName = 'MEDICO'
      end
      object dbgConsultaDESCTIPOMOVIMENTO: TdxDBGridMaskColumn
        Caption = 'Tipo Opera'#231#227'o'
        HeaderAlignment = taCenter
        Width = 95
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCTIPOMOVIMENTO'
      end
      object dbgConsultaBASECALCICMS: TdxDBGridMaskColumn
        Caption = 'Base Calc'
        Width = 80
        BandIndex = 0
        RowIndex = 0
        FieldName = 'BASECALCICMS'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaVALORICMS: TdxDBGridMaskColumn
        Caption = 'Valor ICMS'
        Width = 83
        BandIndex = 0
        RowIndex = 0
        FieldName = 'VALORICMS'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
    end
    inherited PainelFullSelect: TTS_Panel
      Top = 779
      Width = 1547
      inherited cbFullSelect: TTS_CheckBox
        Height = 19
      end
    end
    object clbTipoOperacao: TTS_CheckListBox
      Left = 439
      Top = 82
      Width = 174
      Height = 147
      BorderStyle = bsNone
      Ctl3D = False
      ItemHeight = 13
      ParentCtl3D = False
      TabOrder = 4
      Visible = False
      KeyField = 'TIPOMOVIMENTO'
      ListField = 'DESCRICAO'
      Transparent = False
    end
  end
  inherited TS_Panel1: TTS_Panel
    Height = 798
    inherited btSair: TTS_SpeedButton
      Top = 753
    end
    inherited btAtualizar: TTS_SpeedButton
      OnClick = btAtualizarClick
    end
  end
  inherited ppmPadrao: TTS_PopupMenu
    Left = 162
  end
  inherited FormStorage: TFormStorage
    Left = 120
  end
  inherited Q_Consulta: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    SQL.Strings = (
      
        'select s.saida, s.data, f.nome as nomecliente, f.favorecido, v.n' +
        'ome as nomevendedor, m.nome as nomemedico, gc.descricao as grupo' +
        'cliente, s.totalitens, s.desconto,s.total, s.imposto, s.numero, ' +
        '  p.descricao as descplanopagamento, s.dataentrega, s.valorcomis' +
        'sao, c.descricao as desccaixa, st.descricao as descstatus, s.obs' +
        ',  s.juros, s.frete, f.tipofavorecido, s.vendedor, s.medico, tp.' +
        'descricao as desctabelapreco, f.bairro, f.cpf_cnpj, f.cidade, f.' +
        'bairro, f.cep, f.endereco, f.uf, f.fone1, '
      
        'tm.descricao as desctipomovimento, (select cast(sum(si.customedi' +
        'o * si.quantidade) as numeric(15,2)) from saidasitens si where s' +
        'i.saida = s.saida) as custo, s.basecalcicms , s.valoricms'
      'from saidas s'
      '     left join favorecidos f on f.favorecido = s.favorecido'
      '     left join favorecidos v on v.favorecido = s.vendedor'
      '     left join favorecidos m on m.favorecido = s.medico'
      
        '     left join gruposcliente gc on gc.grupocliente = f.grupoclie' +
        'nte'
      
        '     left join planospagamento p on p.planopagamento = s.planopa' +
        'gamento'
      '     left join statussaidas st on st.status = s.status'
      '     left join contas c on c.conta = s.caixa'
      
        '     left join tabelaspreco tp on tp.tabelapreco = s.tabelapadra' +
        'o'
      
        '     left join tiposmovimento tm on tm.tipomovimento = s.tipomov' +
        'imento')
    Left = 340
  end
  inherited C_ConsultaDS: TDataSource
    Left = 339
  end
  inherited P_Consulta: TDataSetProvider
    Left = 341
  end
  inherited C_Consulta: TClientDataSet
    Left = 340
    Top = 217
    object C_ConsultaSAIDA: TIntegerField
      FieldName = 'SAIDA'
      Origin = 'SAIDAS.SAIDA'
      Required = True
    end
    object C_ConsultaDATA: TDateField
      DisplayLabel = 'Data'
      FieldName = 'DATA'
      Origin = 'SAIDAS.DATA'
    end
    object C_ConsultaNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Origin = 'FAVORECIDOS.NOMECLIENTE'
      Size = 50
    end
    object C_ConsultaFAVORECIDO: TIntegerField
      FieldName = 'FAVORECIDO'
      Origin = 'FAVORECIDOS.FAVORECIDO'
    end
    object C_ConsultaNOMEVENDEDOR: TStringField
      FieldName = 'NOMEVENDEDOR'
      Origin = 'FAVORECIDOS.NOMEVENDEDOR'
      Size = 50
    end
    object C_ConsultaNUMERO: TStringField
      FieldName = 'NUMERO'
      Origin = 'SAIDAS.NUMERO'
      Size = 12
    end
    object C_ConsultaDESCPLANOPAGAMENTO: TStringField
      FieldName = 'DESCPLANOPAGAMENTO'
      Origin = 'PLANOSPAGAMENTO.DESCPLANOPAGAMENTO'
      Size = 30
    end
    object C_ConsultaDATAENTREGA: TDateField
      FieldName = 'DATAENTREGA'
      Origin = 'SAIDAS.DATAENTREGA'
    end
    object C_ConsultaDESCCAIXA: TStringField
      FieldName = 'DESCCAIXA'
      Origin = 'CONTAS.DESCCAIXA'
      Size = 50
    end
    object C_ConsultaDESCSTATUS: TStringField
      FieldName = 'DESCSTATUS'
      Origin = 'STATUSSAIDAS.DESCSTATUS'
      Size = 30
    end
    object C_ConsultaOBS: TStringField
      FieldName = 'OBS'
      Origin = 'SAIDAS.OBS'
      Size = 255
    end
    object C_ConsultaTIPOFAVORECIDO: TIntegerField
      FieldName = 'TIPOFAVORECIDO'
      Origin = 'FAVORECIDOS.TIPOFAVORECIDO'
    end
    object C_ConsultaVENDEDOR: TIntegerField
      FieldName = 'VENDEDOR'
      Origin = 'SAIDAS.VENDEDOR'
    end
    object C_ConsultaNOMEMEDICO: TStringField
      FieldName = 'NOMEMEDICO'
      Size = 50
    end
    object C_ConsultaMEDICO: TIntegerField
      FieldName = 'MEDICO'
    end
    object C_ConsultaMESANO: TStringField
      FieldName = 'MESANO'
      Size = 13
    end
    object C_ConsultaBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Size = 30
    end
    object C_ConsultaCPF_CNPJ: TStringField
      FieldName = 'CPF_CNPJ'
      Size = 14
    end
    object C_ConsultaCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 30
    end
    object C_ConsultaBAIRRO1: TStringField
      FieldName = 'BAIRRO1'
      Size = 30
    end
    object C_ConsultaCEP: TStringField
      FieldName = 'CEP'
      Size = 10
    end
    object C_ConsultaENDERECO: TStringField
      FieldName = 'ENDERECO'
      Size = 50
    end
    object C_ConsultaUF: TStringField
      FieldName = 'UF'
      Size = 2
    end
    object C_ConsultaFONE1: TStringField
      FieldName = 'FONE1'
    end
    object C_ConsultaDESCTIPOMOVIMENTO: TStringField
      FieldName = 'DESCTIPOMOVIMENTO'
      Size = 30
    end
    object C_ConsultaGRUPOCLIENTE: TStringField
      FieldName = 'GRUPOCLIENTE'
      Size = 30
    end
    object C_ConsultaTOTALITENS: TFloatField
      FieldName = 'TOTALITENS'
    end
    object C_ConsultaDESCONTO: TFloatField
      FieldName = 'DESCONTO'
    end
    object C_ConsultaTOTAL: TFloatField
      FieldName = 'TOTAL'
    end
    object C_ConsultaIMPOSTO: TFloatField
      FieldName = 'IMPOSTO'
    end
    object C_ConsultaVALORCOMISSAO: TFloatField
      FieldName = 'VALORCOMISSAO'
    end
    object C_ConsultaJUROS: TFloatField
      FieldName = 'JUROS'
    end
    object C_ConsultaFRETE: TFloatField
      FieldName = 'FRETE'
    end
    object C_ConsultaDESCTABELAPRECO: TStringField
      FieldName = 'DESCTABELAPRECO'
      Size = 25
    end
    object C_ConsultaCUSTO: TBCDField
      FieldName = 'CUSTO'
      Precision = 18
      Size = 2
    end
    object C_ConsultaBASECALCICMS: TFloatField
      FieldName = 'BASECALCICMS'
    end
    object C_ConsultaVALORICMS: TFloatField
      FieldName = 'VALORICMS'
    end
  end
  inherited ppmDados: TTS_PopupMenu
    Left = 219
    Top = 145
  end
  inherited ppmAgrupamento: TTS_PopupMenu
    Left = 225
    Top = 198
  end
  inherited ppmLayout: TTS_PopupMenu
    Left = 232
    Top = 97
  end
  inherited ppmImprimir: TTS_PopupMenu
    Left = 226
    Top = 306
  end
  inherited ppmSumarizacao: TTS_PopupMenu
    Left = 227
    Top = 249
  end
  inherited ppmExportacao: TTS_PopupMenu
    Left = 227
    Top = 357
  end
  inherited ppmHelp: TTS_PopupMenu
    Left = 665
  end
  inherited GridPrinter: TdxComponentPrinter
    PreviewOptions.PreviewBoundsRect = {00000000000000000000000000000000}
    Left = 111
    inherited GridPrinterLink: TdxDBGridReportLink
      DateTime = 37455.7444424769
      PrinterPage._dxMeasurementUnits_ = 2
      PrinterPage._dxLastMU_ = 2
      BuiltInReportLink = True
    end
  end
  inherited dxPSEngineController1: TdxPSEngineController
    Left = 112
  end
  inherited ppmTemplates: TTS_PopupMenu
    Left = 128
  end
  inherited Q_SQLReport: TIBQuery
    Left = 664
  end
  inherited ppmSelTemplate: TTS_PopupMenu
    Left = 664
    Top = 88
  end
  inherited FormsComponent: TFormsComponent
    BeforeClearParams = FormsComponentBeforeClearParams
    BarEndColor = 14859922
    CaptionShadow = 14859922
    FormColor = 16116702
    Modulo = 'Clientes / Outras Saidas'
    Caption = 'Outras Saidas.'
    Left = 663
  end
  object Q_TiposMovimentos: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'Select TIPOMOVIMENTO, DESCRICAO from TIPOSMOVIMENTO where   TIPO' +
        'PADRAO in (5,8,40) order by DESCRICAO')
    Left = 456
    Top = 112
    object Q_TiposMovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'TIPOSMOVIMENTO.TIPOMOVIMENTO'
      Required = True
    end
    object Q_TiposMovimentosDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = 'TIPOSMOVIMENTO.DESCRICAO'
      Size = 50
    end
  end
  object P_TiposMovimentos: TDataSetProvider
    DataSet = Q_TiposMovimentos
    Constraints = True
    Left = 456
    Top = 168
  end
  object C_TiposMovimentos: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'TIPOMOVIMENTO'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'DESCRICAO'
        DataType = ftString
        Size = 30
      end>
    IndexDefs = <
      item
        Name = 'DEFAULT_ORDER'
      end
      item
        Name = 'CHANGEINDEX'
      end>
    Params = <>
    ProviderName = 'P_TiposMovimentos'
    StoreDefs = True
    Left = 456
    Top = 224
    object C_TiposMovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'TIPOSMOVIMENTO.TIPOMOVIMENTO'
      Required = True
    end
    object C_TiposMovimentosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'TIPOSMOVIMENTO.DESCRICAO'
      Size = 50
    end
  end
  object C_TipoMovimentoDS: TDataSource
    DataSet = C_TiposMovimentos
    Left = 456
    Top = 280
  end
end
