inherited RptSalesOrdersItens: TRptSalesOrdersItens
  Left = 338
  Top = 157
  Width = 884
  Height = 483
  Caption = 'Clientes / Vendas'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnTitulo: TTS_MaxPanel
    Width = 868
    Gradient.ColorStart = 14859922
    inherited lbCaption: TdxfLabel
      Width = 404
      Caption = 'Relat'#243'rio de Pedidos de Vendas com Itens'
      Effect3D.ShadowedColor = 14859922
    end
    inherited btHelp: TTS_SpeedButton
      Left = 887
    end
    inherited btTemplates: TTS_SpeedButton
      Left = 747
    end
    inherited lbTemplate: TTS_Label
      Left = 799
    end
  end
  inherited pnGrid: TPanel
    Width = 815
    Height = 407
    inherited RichEdit: TRichEdit
      Left = -2
      Top = 224
    end
    inherited pnDados: TTS_Panel
      Width = 815
      Height = 41
      Color = 16116702
      object TS_Label1: TTS_Label
        Left = 8
        Top = 5
        Width = 44
        Caption = 'Per'#237'odo:'
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label2: TTS_Label
        Left = 132
        Top = 4
        Width = 15
        Alignment = taCenter
        Caption = 'a'
        FormatoTabela = False
        LinkToResult = 0
      end
      object btClientes: TTS_SpeedButton
        Left = 593
        Top = 20
        Width = 117
        Height = 20
        Caption = 'Selecionar &Clientes'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Glyph.Data = {
          
          E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
        ParentFont = False
        OnClick = btClientesClick
        RepeatedClick = False
        Border = True
      end
      object TS_Label3: TTS_Label
        Left = 270
        Top = 5
        Width = 46
        Caption = 'Status:'
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label4: TTS_Label
        Left = 240
        Top = 22
        Width = 76
        Alignment = taLeftJustify
        Caption = 'Tipo opera'#231#227'o :'
        FormatoTabela = False
        LinkToResult = 0
      end
      object edDataI: TTS_DateTimePicker
        Left = 53
        Top = 2
        Width = 78
        TabOrder = 0
        AutoSize = False
        StyleController = DMProjeto.esClientes
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        DateOnError = deToday
        UseEditMask = True
        OnDateChange = edDataIDateChange
        Height = 19
        StoredValues = 4
      end
      object edDataF: TTS_DateTimePicker
        Left = 149
        Top = 2
        Width = 81
        TabOrder = 1
        AutoSize = False
        StyleController = DMProjeto.esClientes
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        DateOnError = deToday
        UseEditMask = True
        Height = 19
        StoredValues = 4
      end
      object cbFaturadas: TTS_CheckBox
        Left = 510
        Top = 2
        Width = 121
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 2
        Caption = 'Somente Faturadas'
        OnChange = cbFaturadasChange
        DisableEdit = False
        Height = 19
      end
      object TS_PopupEdit1: TTS_PopupEdit
        Left = 319
        Top = 2
        Width = 154
        TabOrder = 3
        AutoSelect = False
        StyleController = DMProjeto.esClientes
        HideEditCursor = True
        PopupControl = clbStatus
        PopupFormBorderStyle = pbsSysPanel
        OnCloseUp = TS_PopupEdit1CloseUp
        LookupKeyValue = 0
        Height = 19
      end
      object cbCanceladas: TTS_CheckBox
        Left = 511
        Top = 20
        Width = 82
        TabOrder = 4
        Caption = 'Canceladas'
        StyleController = DMProjeto.esClientes
        OnChange = cbCanceladasChange
        DisableEdit = False
        Height = 19
      end
      object TS_PopupEdit2: TTS_PopupEdit
        Left = 319
        Top = 21
        Width = 154
        TabOrder = 5
        AutoSelect = False
        StyleController = DMProjeto.esClientes
        HideEditCursor = True
        PopupControl = clbTipoOperacao
        PopupFormBorderStyle = pbsSysPanel
        OnCloseUp = TS_PopupEdit2CloseUp
        LookupKeyValue = 0
        Height = 19
      end
      object FiltroItens: TTS_PopupFiltrarItens
        Left = 10
        Top = 21
        Width = 219
        Hint = 'Click na seta de escolha para restrin'#231#227'o de registros'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 6
        AutoSelect = False
        StyleController = DMProjeto.esClientes
        Text = 'Filtrar itens...'
        HideEditCursor = True
        PopupFormBorderStyle = pbsSysPanel
        PopupClientEdge = True
        LookupKeyValue = 0
        TS_DB_Projeto = DMProjeto.DB_Projeto
        TS_TipoEstoque = 0
        TS_FornecedorSelecionado = 0
        TS_FornPrefSelecionado = 0
        TS_NaoPossuiEstoque = False
        TS_EstoqueMinimo = False
        TS_EstoqueMaximo = False
        TS_EstoquePontoPedido = False
        TS_EstoqueNegativo = False
        TS_ItensDesativado = False
        TS_C_LocalizarItem = DMProjeto.C_LocalizarItens
        TS_C_LocalizarFav = DMProjeto.C_LocalizarFav
        TS_PossuiEstoque = False
        TS_TipoItem = 0
        TS_AliasItens = 'i'
        TS_ItemAlterado = False
        Height = 19
      end
    end
    inherited dbgConsulta: TTS_QDBGrid
      Top = 41
      Width = 815
      Height = 347
      KeyField = 'SAIDA'
      SummaryGroups = <
        item
          DefaultGroup = True
          SummaryItems = <
            item
              SummaryField = 'SUBTOTALITEM'
              SummaryFormat = '>SUBTOTALITEM=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'NUMERO'
              SummaryFormat = '>NUMERO=###0'
              SummaryType = cstCount
            end
            item
              SummaryField = 'COMISSAOPECAVENDA'
              SummaryFormat = '>COMISSAOPECAVENDA=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'ComissaoPecaTabela'
              SummaryFormat = '>ComissaoPecaTabela=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'QUANTIDADEVOLUME'
              SummaryFormat = '>QUANTIDADEVOLUME=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'QUANTIDADE'
              SummaryFormat = '>QUANTIDADE=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'ACRESCIMOITEM'
              SummaryFormat = '>ACRESCIMOITEM=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'DESCONTOITEM'
              SummaryFormat = '>DESCONTOITEM=#,###,##0.00'
              SummaryType = cstSum
            end>
          Name = 'Default'
        end>
      Filter.Criteria = {00000000}
      OnGetSummaryTopText = dbgConsultaGetSummaryTopText
      TS_SelectedColumn = 'DESCITEM'
      TS_SummaryFields.Strings = (
        'SUBTOTALITEM;sum'
        'NUMERO;COUNT;'
        'COMISSAOPECAVENDA;sum'
        'ComissaoPecaTabela;sum'
        'QUANTIDADEVOLUME;sum'
        'QUANTIDADE;SUM')
      object dbgConsultaSAIDA: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SAIDA'
      end
      object dbgConsultaDATA: TdxDBGridDateColumn
        Caption = 'Data'
        HeaderAlignment = taCenter
        Visible = False
        Width = 44
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATA'
      end
      object dbgConsultaNUMERO: TdxDBGridMaskColumn
        Caption = 'Pedido.'
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Sorted = csUp
        Visible = False
        Width = 45
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NUMERO'
        SummaryFooterType = cstCount
        SummaryFooterFormat = '###0'
        GroupIndex = 0
      end
      object dbgConsultaCLIENTE: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CLIENTE'
      end
      object dbgConsultaDESCITEM: TdxDBGridMaskColumn
        Caption = 'Item'
        HeaderAlignment = taCenter
        Width = 202
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCITEM'
        SummaryFooterFormat = '###0'
      end
      object dbgConsultaPRECO: TdxDBGridMaskColumn
        Caption = 'Pre'#231'o'
        HeaderAlignment = taCenter
        Width = 65
        BandIndex = 0
        RowIndex = 0
        FieldName = 'PRECO'
        SummaryFooterType = cstSum
      end
      object dbgConsultaQUANTIDADE: TdxDBGridCurrencyColumn
        Caption = 'Qtd'
        HeaderAlignment = taCenter
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'QUANTIDADE'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
        DisplayFormat = '0.00;-0.00'
        Nullable = False
      end
      object dbgConsultaQUANTIDADEVOLUME: TdxDBGridMaskColumn
        Caption = 'Qtd. Volume'
        HeaderAlignment = taCenter
        Width = 40
        BandIndex = 0
        RowIndex = 0
        FieldName = 'QUANTIDADEVOLUME'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaSUBTOTALITEM: TdxDBGridMaskColumn
        Caption = 'Total'
        HeaderAlignment = taCenter
        Width = 69
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SUBTOTALITEM'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaQTDFATURADA: TdxDBGridCurrencyColumn
        Caption = 'Qtd. Faturada'
        HeaderAlignment = taCenter
        Width = 83
        BandIndex = 0
        RowIndex = 0
        FieldName = 'QTDFATURADA'
        SummaryFooterType = cstSum
        DisplayFormat = '0.00;-0.00'
        Nullable = False
      end
      object dbgConsultaNOME: TdxDBGridMaskColumn
        Caption = 'Cliente'
        HeaderAlignment = taCenter
        Visible = False
        Width = 65
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOME'
      end
      object dbgConsultaVENDEDOR: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'VENDEDOR'
      end
      object dbgConsultaNOMEVENDEDOR: TdxDBGridMaskColumn
        Caption = 'Vendedor'
        HeaderAlignment = taCenter
        Width = 71
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMEVENDEDOR'
      end
      object dbgConsultaSTATUSDESC: TdxDBGridMaskColumn
        Caption = 'Status'
        HeaderAlignment = taCenter
        Width = 93
        BandIndex = 0
        RowIndex = 0
        FieldName = 'STATUSDESC'
      end
      object dbgConsultaITEM: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ITEM'
        SummaryFooterFormat = '###0'
      end
      object dbgConsultaTOTALITENS: TdxDBGridCurrencyColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TOTALITENS'
        Nullable = False
      end
      object dbgConsultaDESCONTO: TdxDBGridCurrencyColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCONTO'
        Nullable = False
      end
      object dbgConsultaFRETE: TdxDBGridCurrencyColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FRETE'
        Nullable = False
      end
      object dbgConsultaOUTRASDESPESAS: TdxDBGridCurrencyColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'OUTRASDESPESAS'
        Nullable = False
      end
      object dbgConsultaJUROS: TdxDBGridCurrencyColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'JUROS'
        Nullable = False
      end
      object dbgConsultaIMPOSTO: TdxDBGridCurrencyColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'IMPOSTO'
        Nullable = False
      end
      object dbgConsultaTOTAL: TdxDBGridCurrencyColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TOTAL'
        Nullable = False
      end
      object dbgConsultaBASEIMPOSTO: TdxDBGridCurrencyColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'BASEIMPOSTO'
        Nullable = False
      end
      object dbgConsultaDESCRICAO: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCRICAO'
        SummaryFooterFormat = '###0'
      end
      object dbgConsultaSTATUS: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'STATUS'
      end
      object dbgConsultaSAIDAITEM: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SAIDAITEM'
      end
      object dbgConsultaINVOICE: TdxDBGridMaskColumn
        Caption = 'Nota'
        HeaderAlignment = taCenter
        Width = 61
        BandIndex = 0
        RowIndex = 0
        FieldName = 'INVOICE'
      end
      object dbgConsultaSAIDAINVOICE: TdxDBGridMaskColumn
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 48
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SAIDAINVOICE'
      end
      object dbgConsultaFABRICANTE: TdxDBGridMaskColumn
        Caption = 'Fabricante'
        HeaderAlignment = taCenter
        Width = 121
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FABRICANTE'
      end
      object dbgConsultaZona: TdxDBGridColumn
        Caption = 'Zona'
        HeaderAlignment = taCenter
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ZONA'
      end
      object dbgConsultaFUNCVENDA: TdxDBGridColumn
        Caption = 'Vendedor Pe'#231'a'
        HeaderAlignment = taCenter
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FUNCVENDA'
      end
      object dbgConsultaCOMISSAOPECATABELA: TdxDBGridColumn
        Caption = 'Comiss'#227'o Prc. Tabela'
        HeaderAlignment = taCenter
        BandIndex = 0
        RowIndex = 0
        FieldName = 'COMISSAOPECATABELA'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaCOMISSAOPECAVENDA: TdxDBGridColumn
        Caption = 'Comiss'#227'o Pre'#231'o Venda'
        HeaderAlignment = taCenter
        BandIndex = 0
        RowIndex = 0
        FieldName = 'COMISSAOPECAVENDA'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaMesANO: TdxDBGridDateColumn
        Alignment = taCenter
        Caption = 'M'#234's/Ano'
        HeaderAlignment = taCenter
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'MESANO'
        DateButtons = []
      end
      object dbgConsultaDESCONTOITEM: TdxDBGridMaskColumn
        Alignment = taRightJustify
        Caption = 'Desconto Item'
        HeaderAlignment = taCenter
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCONTOITEM'
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaACRESCIMO: TdxDBGridMaskColumn
        Alignment = taRightJustify
        Caption = 'Acr'#233'scimo Item'
        HeaderAlignment = taCenter
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ACRESCIMOITEM'
        SummaryFooterFormat = '#,###,##0.00'
      end
    end
    inherited PainelFullSelect: TTS_Panel
      Top = 388
      Width = 815
      inherited cbFullSelect: TTS_CheckBox
        Height = 19
      end
    end
  end
  inherited TS_Panel1: TTS_Panel
    Height = 407
    inherited btSair: TTS_SpeedButton
      Top = 362
    end
    inherited btAtualizar: TTS_SpeedButton
      OnClick = btAtualizarClick
    end
  end
  object clbStatus: TTS_CheckListBox [3]
    Left = 87
    Top = 194
    Width = 174
    Height = 131
    BorderStyle = bsNone
    Ctl3D = False
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 3
    Visible = False
    KeyField = 'STATUS'
    ListField = 'DESCRICAO'
    Transparent = False
  end
  object clbTipoOperacao: TTS_CheckListBox [4]
    Left = 431
    Top = 99
    Width = 174
    Height = 147
    BorderStyle = bsNone
    Ctl3D = False
    ItemHeight = 13
    ParentCtl3D = False
    TabOrder = 4
    Visible = False
    KeyField = 'TIPOMOVIMENTO'
    ListField = 'DESCRICAO'
    Transparent = False
  end
  inherited ppmPadrao: TTS_PopupMenu
    Left = 98
    Top = 174
    object AbrirCadastrodeClientes1: TMenuItem [0]
      Caption = 'Abrir Cadastro de Clientes'
      OnClick = AbrirCadastrodeClientes1Click
    end
    object AbrirSalesOrder1: TMenuItem [1]
      Caption = 'Abrir Sales Order'
      OnClick = AbrirSalesOrder1Click
    end
    object AbrirRelatriodeFaturamentodeSO1: TMenuItem [2]
      Caption = 'Abrir Relat'#243'rio de Faturamento de S.O.'
      OnClick = AbrirRelatriodeFaturamentodeSO1Click
    end
    object N2: TMenuItem [3]
      Caption = '-'
    end
  end
  inherited FormStorage: TFormStorage
    Left = 72
  end
  inherited Q_Consulta: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    SQL.Strings = (
      
        'Select s.saida, s.numero, s.data, f.favorecido as cliente, f.nom' +
        'e,'
      
        'fu.favorecido as vendedor,  fu.nome as NomeVendedor, s.totaliten' +
        's,'
      's.desconto, s.frete, s.outrasdespesas, s.juros,'
      
        's.imposto, s.total, s.baseimposto, ss.descricao as statusdesc, s' +
        'i.preco,'
      
        'si.descricao, si.status, si.subtotalitem, si.saidaitem, si.item,' +
        ' it.descricao as DescItem,'
      
        'si.quantidade, si.qtdfaturada, (select max(numero) from saidas s' +
        '2, saidasitens si2, saidasitensmescla sim'
      'where s2.saida = si2.saida and si2.saidaitem = sim.saidaitem'
      'and sim.saidaitem_original = si.saidaitem) as Invoice,'
      '(select max(s3.saida) from saidas s3, saidasitens si3, '
      'saidasitensmescla sim3'
      'where s3.saida = si3.saida and si3.saidaitem = sim3.saidaitem'
      'and sim3.saidaitem_original = si.saidaitem) as SaidaInvoice,'
      'fab.descricao as Fabricante, z.descricao as zona,'
      
        'V.NOME AS FuncVenda, ((si.precotabela * si.quantidade)* it.comis' +
        'sao/100) as ComissaoPecaTabela,'
      
        '((si.preco * si.quantidade)* it.comissao/100) as ComissaoPecaVen' +
        'da, si.quantidadevolume,'
      
        'cast(si.desconto * -1 as numeric(15, 2)) as descontoitem, case w' +
        'hen si.preco > si.precotabela then cast(si.preco - si.precotabel' +
        'a as numeric(15, 2)) else 0 end as acrescimoitem,'
      
        'case when strlen(extract(month from s.data)) = 2 then extract(mo' +
        'nth from s.data) || '#39'/'#39' || extract(year from s.data) else '#39'0'#39' ||' +
        ' extract(month from s.data) || '#39'/'#39' || extract(year from s.data) ' +
        'end as MesAno'
      ''
      'from saidas s '
      'join favorecidos f on s.favorecido = f.favorecido'
      'inner join SaidasItens si on s.saida = si.saida'
      'left join favorecidos fu on s.vendedor = fu.favorecido'
      'inner join statussaidasitens ss on si.status = ss.status'
      'inner join Itens it on it.item = si.item'
      'left join fabricantes fab on it.fabricante = fab.fabricante'
      'left join zonas z on z.zona = f.zona'
      'LEFT JOIN FAVORECIDOS V ON V.FAVORECIDO = SI.funcproducao'
      'where s.tipopadrao = 2 and s.situacao = '#39'N'#39' and'
      's.data between :idata and :fdata')
    Left = 292
    ParamData = <
      item
        DataType = ftDateTime
        Name = 'idata'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'fdata'
        ParamType = ptUnknown
      end>
  end
  inherited C_ConsultaDS: TDataSource
    Left = 291
  end
  inherited P_Consulta: TDataSetProvider
    Left = 293
  end
  inherited C_Consulta: TClientDataSet
    Params = <
      item
        DataType = ftDateTime
        Name = 'idata'
        ParamType = ptUnknown
      end
      item
        DataType = ftDateTime
        Name = 'fdata'
        ParamType = ptUnknown
      end>
    Left = 292
    object C_ConsultaSAIDA: TIntegerField
      FieldName = 'SAIDA'
      Required = True
    end
    object C_ConsultaNUMERO: TStringField
      FieldName = 'NUMERO'
      Size = 12
    end
    object C_ConsultaDATA: TDateField
      FieldName = 'DATA'
    end
    object C_ConsultaCLIENTE: TIntegerField
      FieldName = 'CLIENTE'
      Required = True
    end
    object C_ConsultaNOME: TStringField
      FieldName = 'NOME'
      Size = 50
    end
    object C_ConsultaVENDEDOR: TIntegerField
      FieldName = 'VENDEDOR'
    end
    object C_ConsultaNOMEVENDEDOR: TStringField
      FieldName = 'NOMEVENDEDOR'
      Size = 50
    end
    object C_ConsultaTOTALITENS: TFloatField
      FieldName = 'TOTALITENS'
    end
    object C_ConsultaDESCONTO: TFloatField
      FieldName = 'DESCONTO'
    end
    object C_ConsultaFRETE: TFloatField
      FieldName = 'FRETE'
    end
    object C_ConsultaOUTRASDESPESAS: TFloatField
      FieldName = 'OUTRASDESPESAS'
    end
    object C_ConsultaJUROS: TFloatField
      FieldName = 'JUROS'
    end
    object C_ConsultaIMPOSTO: TFloatField
      FieldName = 'IMPOSTO'
    end
    object C_ConsultaTOTAL: TFloatField
      FieldName = 'TOTAL'
    end
    object C_ConsultaBASEIMPOSTO: TFloatField
      FieldName = 'BASEIMPOSTO'
    end
    object C_ConsultaSTATUSDESC: TStringField
      FieldName = 'STATUSDESC'
      Size = 30
    end
    object C_ConsultaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 255
    end
    object C_ConsultaSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 1
    end
    object C_ConsultaSUBTOTALITEM: TFloatField
      FieldName = 'SUBTOTALITEM'
      DisplayFormat = '#,###,##0.00'
    end
    object C_ConsultaSAIDAITEM: TIntegerField
      FieldName = 'SAIDAITEM'
      Required = True
    end
    object C_ConsultaITEM: TIntegerField
      FieldName = 'ITEM'
    end
    object C_ConsultaQTDFATURADA: TFloatField
      FieldName = 'QTDFATURADA'
    end
    object C_ConsultaINVOICE: TStringField
      FieldName = 'INVOICE'
      Size = 12
    end
    object C_ConsultaSAIDAINVOICE: TIntegerField
      FieldName = 'SAIDAINVOICE'
    end
    object C_ConsultaDESCITEM: TStringField
      FieldName = 'DESCITEM'
      Size = 50
    end
    object C_ConsultaPRECO: TFloatField
      FieldName = 'PRECO'
    end
    object C_ConsultaFABRICANTE: TStringField
      FieldName = 'FABRICANTE'
      Size = 30
    end
    object C_ConsultaZONA: TStringField
      FieldName = 'ZONA'
      Size = 30
    end
    object C_ConsultaFUNCVENDA: TStringField
      FieldName = 'FUNCVENDA'
      Size = 50
    end
    object C_ConsultaCOMISSAOPECATABELA: TFloatField
      FieldName = 'COMISSAOPECATABELA'
    end
    object C_ConsultaCOMISSAOPECAVENDA: TFloatField
      FieldName = 'COMISSAOPECAVENDA'
    end
    object C_ConsultaQUANTIDADEVOLUME: TFloatField
      FieldName = 'QUANTIDADEVOLUME'
    end
    object C_ConsultaMESANO: TStringField
      FieldName = 'MESANO'
      Size = 14
    end
    object C_ConsultaQUANTIDADE: TFloatField
      FieldName = 'QUANTIDADE'
    end
    object C_ConsultaACRESCIMOITEM: TBCDField
      FieldName = 'ACRESCIMOITEM'
      Precision = 18
      Size = 2
    end
    object C_ConsultaDESCONTOITEM: TBCDField
      FieldName = 'DESCONTOITEM'
      Precision = 18
      Size = 2
    end
  end
  inherited ppmDados: TTS_PopupMenu
    Left = 179
  end
  inherited ppmAgrupamento: TTS_PopupMenu
    Left = 185
  end
  inherited ppmLayout: TTS_PopupMenu
    Left = 304
    Top = 358
  end
  inherited ppmImprimir: TTS_PopupMenu
    Left = 186
  end
  inherited ppmSumarizacao: TTS_PopupMenu
    Left = 187
  end
  inherited ppmExportacao: TTS_PopupMenu
    Left = 187
  end
  inherited ppmHelp: TTS_PopupMenu
    Left = 617
  end
  inherited GridPrinter: TdxComponentPrinter
    PreviewOptions.PreviewBoundsRect = {00000000000000000000000000000000}
    Left = 60
    Top = 193
    inherited GridPrinterLink: TdxDBGridReportLink
      DateTime = 37233.6101571181
      PrinterPage._dxMeasurementUnits_ = 2
      PrinterPage._dxLastMU_ = 2
      BuiltInReportLink = True
    end
  end
  inherited dxPSEngineController1: TdxPSEngineController
    Left = 64
  end
  inherited Q_SQLReport: TIBQuery
    Left = 416
  end
  inherited ppmSelTemplate: TTS_PopupMenu
    Left = 712
    Top = 112
  end
  inherited FormsComponent: TFormsComponent
    BeforeClearParams = FormsComponentBeforeClearParams
    BarEndColor = 14859922
    CaptionShadow = 14859922
    FormColor = 16116702
    Modulo = 'Clientes / Vendas'
    Caption = 'Relat'#243'rio de Pedidos de Vendas com Itens'
    Left = 613
    Top = 211
  end
  object Q_Status: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select * from statussaidasitens'
      'order by descricao')
    Left = 356
    Top = 112
  end
  object P_Status: TDataSetProvider
    DataSet = Q_Status
    Constraints = True
    Left = 358
    Top = 164
  end
  object C_Status: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'P_Status'
    Left = 363
    Top = 213
    object C_StatusSTATUS: TStringField
      FieldName = 'STATUS'
      Origin = 'STATUSSAIDASITENS.STATUS'
      Required = True
      Size = 1
    end
    object C_StatusDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'STATUSSAIDASITENS.DESCRICAO'
      Size = 30
    end
    object C_StatusORDEM: TIntegerField
      FieldName = 'ORDEM'
      Origin = 'STATUSSAIDASITENS.ORDEM'
    end
    object C_StatusDESCRICAO1: TStringField
      FieldName = 'DESCRICAO1'
      Origin = 'STATUSSAIDASITENS.DESCRICAO1'
      Size = 30
    end
    object C_StatusDESCRICAO2: TStringField
      FieldName = 'DESCRICAO2'
      Origin = 'STATUSSAIDASITENS.DESCRICAO2'
      Size = 30
    end
    object C_StatusDESCRICAO3: TStringField
      FieldName = 'DESCRICAO3'
      Origin = 'STATUSSAIDASITENS.DESCRICAO3'
      Size = 30
    end
  end
  object C_StatusDS: TDataSource
    DataSet = C_Status
    Left = 366
    Top = 276
  end
  object C_TipoMovimentoDS: TDataSource
    DataSet = C_TiposMovimentos
    Left = 528
    Top = 280
  end
  object C_TiposMovimentos: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'TIPOMOVIMENTO'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'DESCRICAO'
        DataType = ftString
        Size = 30
      end>
    IndexDefs = <
      item
        Name = 'DEFAULT_ORDER'
      end
      item
        Name = 'CHANGEINDEX'
      end>
    Params = <>
    ProviderName = 'P_TiposMovimentos'
    StoreDefs = True
    Left = 528
    Top = 224
    object C_TiposMovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'TIPOSMOVIMENTO.TIPOMOVIMENTO'
      Required = True
    end
    object C_TiposMovimentosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'TIPOSMOVIMENTO.DESCRICAO'
      Size = 50
    end
  end
  object P_TiposMovimentos: TDataSetProvider
    DataSet = Q_TiposMovimentos
    Constraints = True
    Left = 528
    Top = 168
  end
  object Q_TiposMovimentos: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'Select TIPOMOVIMENTO, DESCRICAO from TIPOSMOVIMENTO where   TIPO' +
        'PADRAO = 2 order by DESCRICAO')
    Left = 528
    Top = 112
    object Q_TiposMovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'TIPOSMOVIMENTO.TIPOMOVIMENTO'
      Required = True
    end
    object Q_TiposMovimentosDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = 'TIPOSMOVIMENTO.DESCRICAO'
      Size = 50
    end
  end
end
