unit rpt_ProdutosComprasGrupoPai;
...
type
  TrptProdutosComprasGrupoPai = class(TFrmModeloCadastros)
    TS_Label1: TTS_Label;
    TS_Label2: TTS_Label;
    Q_GrupoPAI: TIBQuery;
    ...
    Q_GrupoPAIQTDCOMPRADA: TFloatField;
    Q_SomatorioGrupoPaiQTDCOMPRADA: TFloatField;
...
var
  rptProdutosComprasGrupoPai: TrptProdutosComprasGrupoPai;
...
procedure TrptProdutosComprasGrupoPai.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TrptProdutosComprasGrupoPai.ppDBText7GetText(Sender: TObject;
  var Text: String);
begin
  inherited;
...
procedure TrptProdutosComprasGrupoPai.btGravarClick(Sender: TObject);
Var sTotal: String;
begin
  inherited;
  ...
  with Q_SomatorioGrupoPai do Begin
    Close;
    Sql.Text := 'select g.grupopai, sum(quantidade) as QtdComprada, avg(preco) as PrcMedio, '+
                'sum(si.subtotalitem) as SubTotal, (sum(si.subtotalitem) / '+sTotal+') * 100 as percente '+
                'from ENTRADAS s  LEFT join ENTRADASITENS si on si.ENTRADA = s.ENTRADA '+
                'LEFT join itens i on si.item = i.item LEFT join grupos g on i.grupo = g.grupo '+
                'where (g.grupopai is not null) and (s.situacao = ''N'') and (si.situacao = ''N'') and (s.data >= :dtIni) and (s.data <= :dtFim) and ((s.tipopadrao = 101) or ((s.tipoPadrao = 102) and (s.status = ''H''))) '+
                'group by g.grupopai ';
    Params[0].AsDateTime := dataini.Date;
    Params[1].AsDateTime := datafim.Date;
    try
  ...
  End;
    lbTituloRelatorio.Caption := 'Compras no Período: '+DateToStr(dataini.date)+' e '+DateToStr(datafim.date);
    DMProjeto.ImprimirCabecalho( regCab );
    ppVendasGrupoPai.Print;
end;

procedure TrptProdutosComprasGrupoPai.ppSomatorioGeralGetText(Sender: TObject;
  var Text: String);
begin
  inherited;
  sTotalGeral := Text;
