inherited RptComprasProdutos: TRptComprasProdutos
  Left = 221
  Top = 109
  Width = 1085
  Height = 592
  Caption = 'Clientes / Vendas'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnTitulo: TTS_MaxPanel
    Width = 1069
    Gradient.ColorStart = 11775972
    inherited lbCaption: TdxfLabel
      Width = 205
      Caption = 'Compras de Produtos'
      Effect3D.ShadowedColor = 11775972
    end
    inherited btHelp: TTS_SpeedButton
      Left = 1043
    end
    inherited btTemplates: TTS_SpeedButton
      Left = 1016
    end
  end
  inherited pnGrid: TPanel
    Width = 1016
    Height = 516
    inherited pnDados: TTS_Panel
      Width = 1016
      Height = 31
      Color = 15196656
      object TS_Label1: TTS_Label
        Left = 3
        Top = 4
        Width = 78
        Caption = 'No per'#237'odo de:'
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label2: TTS_Label
        Left = 161
        Top = 5
        Width = 18
        Caption = ' a '
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label3: TTS_Label
        Left = 269
        Top = 4
        Width = 81
        Height = 15
        Caption = 'Tipo Opera'#231#227'o :'
        FormatoTabela = False
        LinkToResult = 0
      end
      object btFornecedores: TTS_SpeedButton
        Left = 641
        Top = 3
        Width = 116
        Height = 20
        Caption = '&Fornecedores'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Glyph.Data = {
          
          E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
        ParentFont = False
        OnClick = btFornecedoresClick
        RepeatedClick = False
        Border = True
      end
      object DataI: TTS_DateTimePicker
        Left = 83
        Top = 2
        Width = 81
        TabOrder = 0
        AutoSize = False
        StyleController = DMProjeto.esFornecedores
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = DataIDateChange
        Height = 19
        StoredValues = 4
      end
      object DataF: TTS_DateTimePicker
        Left = 184
        Top = 2
        Width = 81
        TabOrder = 1
        AutoSize = False
        StyleController = DMProjeto.esFornecedores
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = DataIDateChange
        Height = 19
        StoredValues = 4
      end
      object PopupFiltrarItens: TTS_PopupFiltrarItens
        Left = 509
        Top = 2
        Width = 125
        Hint = 'Click na seta de escolha para restrin'#231#227'o de registros'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        AutoSelect = False
        StyleController = DMProjeto.esFornecedores
        Text = 'Filtrar itens...'
        HideEditCursor = True
        PopupFormBorderStyle = pbsSysPanel
        PopupClientEdge = True
        LookupKeyValue = 0
        TS_DB_Projeto = DMProjeto.DB_Projeto
        TS_TipoEstoque = 0
        TS_FornecedorSelecionado = 0
        TS_FornPrefSelecionado = 0
        TS_NaoPossuiEstoque = False
        TS_EstoqueMinimo = False
        TS_EstoqueMaximo = False
        TS_EstoquePontoPedido = False
        TS_EstoqueNegativo = False
        TS_ItensDesativado = False
        TS_C_LocalizarItem = DMProjeto.C_LocalizarItens
        TS_C_LocalizarFav = DMProjeto.C_LocalizarFav
        TS_PossuiEstoque = False
        TS_TipoItem = 0
        TS_AliasItens = 'i'
        TS_ItemAlterado = False
        OnSelecionou = PopupFiltrarItensSelecionou
        Height = 19
      end
      object ppeTipoOperacao: TTS_PopupEdit
        Left = 354
        Top = 2
        Width = 149
        TabOrder = 3
        AutoSelect = False
        StyleController = DMProjeto.esFornecedores
        HideEditCursor = True
        PopupControl = clbTipoOperacao
        PopupFormBorderStyle = pbsSysPanel
        OnCloseUp = ppeTipoOperacaoCloseUp
        LookupKeyValue = 0
        Height = 19
      end
    end
    inherited dbgConsulta: TTS_QDBGrid
      Top = 31
      Width = 1016
      Height = 466
      KeyField = 'ENTRADAITEM'
      SummaryGroups = <
        item
          DefaultGroup = True
          SummaryItems = <
            item
              SummaryField = 'SubTotalItem'
              SummaryFormat = '>SubTotalItem=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'Quantidade'
              SummaryFormat = '>Quantidade=#,###,##0.00'
              SummaryType = cstSum
            end
            item
              SummaryField = 'Quantidadevolume'
              SummaryFormat = '>Quantidadevolume=#,###,##0.00'
              SummaryType = cstSum
            end>
          Name = 'Default'
        end>
      Filter.Criteria = {00000000}
      TS_SelectedColumn = 'DATA'
      TS_SummaryFields.Strings = (
        'SubTotalItem;Sum'
        'Quantidade;Sum'
        'Quantidadevolume;sum')
      object dbgConsultaENTRADAITEM: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Visible = False
        Width = 33
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ENTRADAITEM'
      end
      object dbgConsultaENTRADA: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Visible = False
        Width = 24
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ENTRADA'
      end
      object dbgConsultaDATA: TdxDBGridDateColumn
        Alignment = taLeftJustify
        Caption = 'Data'
        HeaderAlignment = taCenter
        Width = 71
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATA'
      end
      object dbgConsultaMesAno: TdxDBGridColumn
        Caption = 'M'#234's/Ano'
        HeaderAlignment = taCenter
        Width = 82
        BandIndex = 0
        RowIndex = 0
        FieldName = 'MESANO'
      end
      object dbgConsultaNUMERO: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Caption = 'N'#250'mero'
        HeaderAlignment = taCenter
        Width = 57
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NUMERO'
      end
      object dbgConsultaCODIGO: TdxDBGridColumn
        Caption = 'C'#243'digo'
        HeaderAlignment = taCenter
        Width = 75
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CODIGO'
      end
      object dbgConsultaDESCRICAO: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Caption = 'Descri'#231#227'o'
        HeaderAlignment = taCenter
        Width = 236
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCRICAO'
      end
      object dbgConsultaDESCRICAOGRUPO: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Caption = 'Grupo'
        HeaderAlignment = taCenter
        Width = 148
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCRICAOGRUPO'
      end
      object dbgConsultaFABRICANTE: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Caption = 'Fabricante'
        HeaderAlignment = taCenter
        Width = 128
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FABRICANTE'
      end
      object dbgConsultaUNIDADE: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Caption = 'Unid'
        HeaderAlignment = taCenter
        Width = 40
        BandIndex = 0
        RowIndex = 0
        FieldName = 'UNIDADE'
      end
      object dbgConsultaQUANTIDADE: TdxDBGridCurrencyColumn
        Alignment = taRightJustify
        Caption = 'Quantidade'
        HeaderAlignment = taCenter
        Width = 86
        BandIndex = 0
        RowIndex = 0
        FieldName = 'QUANTIDADE'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
        DecimalPlaces = 0
        DisplayFormat = '###,#00'
        Nullable = False
      end
      object dbgConsultaQUANTIDADEVOLUME: TdxDBGridMaskColumn
        Caption = 'Qtd. Volume'
        HeaderAlignment = taCenter
        Width = 83
        BandIndex = 0
        RowIndex = 0
        FieldName = 'QUANTIDADEVOLUME'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
      end
      object dbgConsultaPRECO: TdxDBGridMaskColumn
        Alignment = taRightJustify
        Caption = 'Pre'#231'o'
        HeaderAlignment = taCenter
        Width = 64
        BandIndex = 0
        RowIndex = 0
        FieldName = 'PRECO'
      end
      object dbgConsultaSUBTOTALITEM: TdxDBGridCurrencyColumn
        Alignment = taRightJustify
        Caption = 'Total'
        HeaderAlignment = taCenter
        Width = 67
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SUBTOTALITEM'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '#,###,##0.00'
        Nullable = False
      end
      object dbgConsultaNOMECLIENTE: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Caption = 'Fornecedor'
        HeaderAlignment = taCenter
        Width = 121
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMECLIENTE'
      end
      object dbgConsultaFAVORECIDO: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 29
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FAVORECIDO'
      end
      object dbgConsultaTIPOFAVORECIDO: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 40
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TIPOFAVORECIDO'
      end
      object dbgConsultaNOMEFORNECEDOR: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        Caption = #218'ltimo Fornecedor'
        HeaderAlignment = taCenter
        Sorted = csUp
        Visible = False
        Width = 186
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMEFORNECEDOR'
      end
      object dbgConsultaCUSTOMEDIO: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 30
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CUSTOMEDIO'
      end
      object dbgConsultaCUSTOCONTABIL: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 39
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CUSTOCONTABIL'
      end
      object dbgConsultaRATEIODESCONTO: TdxDBGridCurrencyColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 47
        BandIndex = 0
        RowIndex = 0
        FieldName = 'RATEIODESCONTO'
        Nullable = False
      end
      object dbgConsultaCOLUNA1: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'COLUNA1'
      end
      object dbgConsultaCOLUNA2: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'COLUNA2'
      end
      object dbgConsultaCOLUNA3: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'COLUNA3'
      end
      object dbgConsultaCOLUNA4: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'COLUNA4'
      end
      object dbgConsultaBAIRRO: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 75
        BandIndex = 0
        RowIndex = 0
        FieldName = 'BAIRRO'
      end
      object dbgConsultaCPF_CNPJ: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 34
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CPF_CNPJ'
      end
      object dbgConsultaENDERECO: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 124
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ENDERECO'
      end
      object dbgConsultaCEP: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 24
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CEP'
      end
      object dbgConsultaCIDADE: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 75
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CIDADE'
      end
      object dbgConsultaUF: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 22
        BandIndex = 0
        RowIndex = 0
        FieldName = 'UF'
      end
      object dbgConsultaFONE1: TdxDBGridMaskColumn
        Alignment = taLeftJustify
        HeaderAlignment = taCenter
        Visible = False
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FONE1'
      end
      object dbgConsultaPISCOFINS: TdxDBGridColumn
        HeaderAlignment = taCenter
        Visible = False
        Width = 68
        BandIndex = 0
        RowIndex = 0
        FieldName = 'PISCOFINS'
      end
      object dbgConsultaTribFederal: TdxDBGridColumn
        Caption = 'Trib. Federal'
        HeaderAlignment = taCenter
        Width = 184
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TIPOTRIBFEDERAL'
      end
      object dbgConsultaTPMOVIMENTO: TdxDBGridColumn
        Caption = 'Opera'#231#227'o'
        HeaderAlignment = taCenter
        Width = 184
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TPMOVIMENTO'
      end
      object dbgConsultaColumn37: TdxDBGridColumn
        Caption = 'Total Nota'
        HeaderAlignment = taCenter
        Width = 64
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TOTAL'
      end
    end
    inherited PainelFullSelect: TTS_Panel
      Top = 497
      Width = 1016
      inherited cbFullSelect: TTS_CheckBox
        Height = 19
      end
    end
    object clbTipoOperacao: TTS_CheckListBox
      Left = 358
      Top = 52
      Width = 150
      Height = 123
      BorderStyle = bsNone
      Ctl3D = False
      ItemHeight = 13
      ParentCtl3D = False
      TabOrder = 4
      Visible = False
      KeyField = 'TIPOMOVIMENTO'
      ListField = 'DESCRICAO'
      Transparent = False
    end
  end
  inherited TS_Panel1: TTS_Panel
    Height = 516
    inherited btSair: TTS_SpeedButton
      Top = 471
    end
    inherited btAtualizar: TTS_SpeedButton
      OnClick = btAtualizarClick
    end
  end
  inherited ppmPadrao: TTS_PopupMenu
    Left = 138
    Top = 142
  end
  inherited FormStorage: TFormStorage
    Left = 96
    Top = 144
  end
  inherited Q_Consulta: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    SQL.Strings = (
      
        'select ei.entradaitem, ei.entrada, ei.descricao, ei.unidade, ei.' +
        'quantidade, ei.preco, ei.subtotalitem, f.nome as nomecliente, e.' +
        'data, e.numero, e.favorecido, f.tipofavorecido,gi.descricaogrupo' +
        ', fb.descricao as fabricante, fn.nome as nomefornecedor, ei.cust' +
        'omedio*ei.fator as customedio, ei.custocontabil*ei.fator as cust' +
        'ocontabil, ei.rateiodesconto, ei.coluna1,ei.coluna2, ei.coluna3,' +
        ' ei.coluna4, f.bairro, f.cpf_cnpj, f.endereco, f.cep, f.cidade, ' +
        'f.uf,f.fone1, i.codigo, Extract(year from e.data)||'#39'/'#39'||Extract(' +
        'month from e.data) as MesAno, i.PISCOFINS ,'
      't.descricao as TipoTribFederal,'
      'tm.descricao as TpMovimento, ei.quantidadevolume, E.TOTAL'
      'From entradas e'
      '    inner join EntradasItens ei on ei.entrada = e.entrada'
      '    inner join Itens i on i.item = ei.item'
      
        '    left Join  TIPOTRIBUTACAOFEDERAL T on t.IDTRIBFEDERAL = i.ID' +
        'TRIBFEDERAL'
      '    left join  Favorecidos f on f.favorecido = e.favorecido'
      
        '    left join  Favorecidos fn on fn.favorecido = i.ultimofornece' +
        'dor'
      '    left join  Fabricantes fb on fb.fabricante = i.fabricante'
      '    left join  Grupos gi on gi.grupo = i.grupo'
      
        '    left join tiposmovimento tm on tm.tipomovimento = e.tipomovi' +
        'mento')
    Left = 314
    Top = 144
  end
  inherited C_ConsultaDS: TDataSource
    Left = 315
    Top = 307
  end
  inherited P_Consulta: TDataSetProvider
    Left = 317
    Top = 195
  end
  inherited C_Consulta: TClientDataSet
    Left = 318
    Top = 248
    object C_ConsultaENTRADAITEM: TIntegerField
      FieldName = 'ENTRADAITEM'
      Origin = 'ENTRADASITENS.ENTRADAITEM'
      Required = True
    end
    object C_ConsultaENTRADA: TIntegerField
      FieldName = 'ENTRADA'
      Origin = 'ENTRADASITENS.ENTRADA'
    end
    object C_ConsultaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'ENTRADASITENS.DESCRICAO'
      Size = 255
    end
    object C_ConsultaUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Origin = 'ENTRADASITENS.UNIDADE'
      Size = 6
    end
    object C_ConsultaNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Origin = 'FAVORECIDOS.NOMECLIENTE'
      Size = 50
    end
    object C_ConsultaDATA: TDateField
      FieldName = 'DATA'
      Origin = 'ENTRADAS.DATA'
      Required = True
    end
    object C_ConsultaNUMERO: TStringField
      FieldName = 'NUMERO'
      Origin = 'ENTRADAS.NUMERO'
      Size = 12
    end
    object C_ConsultaFAVORECIDO: TIntegerField
      FieldName = 'FAVORECIDO'
      Origin = 'ENTRADAS.FAVORECIDO'
    end
    object C_ConsultaTIPOFAVORECIDO: TIntegerField
      FieldName = 'TIPOFAVORECIDO'
      Origin = 'FAVORECIDOS.TIPOFAVORECIDO'
    end
    object C_ConsultaDESCRICAOGRUPO: TStringField
      FieldName = 'DESCRICAOGRUPO'
      Origin = 'GRUPOS.DESCRICAOGRUPO'
      Size = 30
    end
    object C_ConsultaFABRICANTE: TStringField
      FieldName = 'FABRICANTE'
      Origin = 'FABRICANTES.FABRICANTE'
      Size = 30
    end
    object C_ConsultaNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Origin = 'FAVORECIDOS.NOMEFORNECEDOR'
      Size = 50
    end
    object C_ConsultaCOLUNA1: TStringField
      FieldName = 'COLUNA1'
      Origin = 'ENTRADASITENS.COLUNA1'
    end
    object C_ConsultaCOLUNA2: TStringField
      FieldName = 'COLUNA2'
      Origin = 'ENTRADASITENS.COLUNA2'
    end
    object C_ConsultaCOLUNA3: TStringField
      FieldName = 'COLUNA3'
      Origin = 'ENTRADASITENS.COLUNA3'
    end
    object C_ConsultaCOLUNA4: TStringField
      FieldName = 'COLUNA4'
      Origin = 'ENTRADASITENS.COLUNA4'
    end
    object C_ConsultaBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Origin = 'FAVORECIDOS.BAIRRO'
      Size = 30
    end
    object C_ConsultaCPF_CNPJ: TStringField
      FieldName = 'CPF_CNPJ'
      Origin = 'FAVORECIDOS.CPF_CNPJ'
      Size = 14
    end
    object C_ConsultaENDERECO: TStringField
      FieldName = 'ENDERECO'
      Origin = 'FAVORECIDOS.ENDERECO'
      Size = 50
    end
    object C_ConsultaCEP: TStringField
      FieldName = 'CEP'
      Origin = 'FAVORECIDOS.CEP'
      Size = 10
    end
    object C_ConsultaCIDADE: TStringField
      FieldName = 'CIDADE'
      Origin = 'FAVORECIDOS.CIDADE'
      Size = 30
    end
    object C_ConsultaUF: TStringField
      FieldName = 'UF'
      Origin = 'FAVORECIDOS.UF'
      Size = 2
    end
    object C_ConsultaFONE1: TStringField
      FieldName = 'FONE1'
      Origin = 'FAVORECIDOS.FONE1'
    end
    object C_ConsultaCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'ITENS.CODIGO'
      Size = 13
    end
    object C_ConsultaMESANO: TStringField
      FieldName = 'MESANO'
      Required = True
      Size = 13
    end
    object C_ConsultaTIPOTRIBFEDERAL: TStringField
      FieldName = 'TIPOTRIBFEDERAL'
      Origin = 'TIPOTRIBUTACAOFEDERAL.TIPOTRIBFEDERAL'
      Size = 30
    end
    object C_ConsultaTPMOVIMENTO: TStringField
      FieldName = 'TPMOVIMENTO'
      Size = 30
    end
    object C_ConsultaQUANTIDADE: TFloatField
      FieldName = 'QUANTIDADE'
    end
    object C_ConsultaPRECO: TFloatField
      FieldName = 'PRECO'
    end
    object C_ConsultaSUBTOTALITEM: TFloatField
      FieldName = 'SUBTOTALITEM'
    end
    object C_ConsultaCUSTOMEDIO: TFloatField
      FieldName = 'CUSTOMEDIO'
    end
    object C_ConsultaCUSTOCONTABIL: TFloatField
      FieldName = 'CUSTOCONTABIL'
    end
    object C_ConsultaRATEIODESCONTO: TFloatField
      FieldName = 'RATEIODESCONTO'
    end
    object C_ConsultaPISCOFINS: TStringField
      FieldName = 'PISCOFINS'
      FixedChar = True
      Size = 1
    end
    object C_ConsultaQUANTIDADEVOLUME: TFloatField
      FieldName = 'QUANTIDADEVOLUME'
    end
    object C_ConsultaTOTAL: TFloatField
      FieldName = 'TOTAL'
    end
  end
  inherited ppmDados: TTS_PopupMenu
    Left = 203
    Top = 153
  end
  inherited ppmAgrupamento: TTS_PopupMenu
    Left = 209
    Top = 206
  end
  inherited ppmLayout: TTS_PopupMenu
    Left = 216
    Top = 105
  end
  inherited ppmImprimir: TTS_PopupMenu
    Left = 210
    Top = 314
  end
  inherited ppmSumarizacao: TTS_PopupMenu
    Left = 211
    Top = 257
  end
  inherited ppmExportacao: TTS_PopupMenu
    Left = 211
    Top = 365
  end
  inherited GridPrinter: TdxComponentPrinter
    PreviewOptions.PreviewBoundsRect = {00000000000000000000000000000000}
    Left = 87
    Top = 226
    inherited GridPrinterLink: TdxDBGridReportLink
      DateTime = 37456.4298704282
      PrinterPage._dxMeasurementUnits_ = 2
      PrinterPage._dxLastMU_ = 2
      BuiltInReportLink = True
    end
  end
  inherited dxPSEngineController1: TdxPSEngineController
    Left = 112
    Top = 288
  end
  inherited ppmTemplates: TTS_PopupMenu
    Left = 104
    Top = 360
  end
  inherited ppmSelTemplate: TTS_PopupMenu
    Top = 136
  end
  inherited FormsComponent: TFormsComponent
    BeforeClearParams = FormsComponentBeforeClearParams
    BarEndColor = 11775972
    CaptionShadow = 11775972
    FormColor = 15196656
    Modulo = 'Clientes / Vendas'
    Caption = 'Compras de Produtos'
  end
  object Q_TiposMovimentos: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'Select TIPOMOVIMENTO, DESCRICAO from TIPOSMOVIMENTO where   TIPO' +
        ' = '#39'E'#39' order by DESCRICAO')
    Left = 424
    Top = 152
    object Q_TiposMovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'TIPOSMOVIMENTO.TIPOMOVIMENTO'
      Required = True
    end
    object Q_TiposMovimentosDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = 'TIPOSMOVIMENTO.DESCRICAO'
      Size = 50
    end
  end
  object P_TiposMovimentos: TDataSetProvider
    DataSet = Q_TiposMovimentos
    Constraints = True
    Left = 424
    Top = 208
  end
  object C_TiposMovimentos: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'TIPOMOVIMENTO'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'DESCRICAO'
        DataType = ftString
        Size = 30
      end>
    IndexDefs = <
      item
        Name = 'DEFAULT_ORDER'
      end
      item
        Name = 'CHANGEINDEX'
      end>
    Params = <>
    ProviderName = 'P_TiposMovimentos'
    StoreDefs = True
    Left = 424
    Top = 264
    object C_TiposMovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'TIPOSMOVIMENTO.TIPOMOVIMENTO'
      Required = True
    end
    object C_TiposMovimentosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'TIPOSMOVIMENTO.DESCRICAO'
      Size = 50
    end
  end
  object C_TipoMovimentoDS: TDataSource
    DataSet = C_TiposMovimentos
    Left = 424
    Top = 320
  end
end
