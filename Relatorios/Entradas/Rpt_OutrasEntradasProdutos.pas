unit Rpt_OutrasEntradasProdutos;
...
type
  TRptOutrasEntradasProdutos = class(TRptPadrao)
    TS_Label1: TTS_Label;
    TS_Label2: TTS_Label;
...
var
  RptOutrasEntradasProdutos: TRptOutrasEntradasProdutos;
...
procedure TRptOutrasEntradasProdutos.FormsComponentBeforeClearParams(
  Sender: TObject);
var i : integer;
begin
  inherited;
...
procedure TRptOutrasEntradasProdutos.Atualizar;
var
 nTP_MV: string;
begin
 nTP_MV := '';
 if TS_LCBTipoMovimento.LookupKeyValue <> null then begin
     ...
     with C_Consulta do begin
  	Close;
        CommandText := 'Select ' + getCampos + ' ' +
                   'From '   + getTabelas + ' ' +
                   'Where  e.data >= :datai and e.data <= :dataf and e.tipopadrao in (105,150) and e.situacao = ''N'' '+
  ...
  FormsComponent.Caption := 'Outras Entradas com Produtos '+DateTostr(DataI.Date) +' a '+
                                                DateTostr(DataF.Date);


end;

procedure TRptOutrasEntradasProdutos.btAtualizarClick(Sender: TObject);
begin
  inherited;
	Atualizar;
end;

procedure TRptOutrasEntradasProdutos.DataIDateChange(Sender: TObject);
begin
  inherited;
	Atualizar;
end;

procedure TRptOutrasEntradasProdutos.TS_LCBTipoMovimentoChange(Sender: TObject);
begin
  inherited;
	Atualizar;
end;

procedure TRptOutrasEntradasProdutos.PopupFiltrarItensSelecionou(
  Sender: TObject);
begin
  inherited;
	Atualizar;
...
procedure TRptOutrasEntradasProdutos.btFornecedoresClick(Sender: TObject);
begin
  inherited;
...
procedure TRptOutrasEntradasProdutos.dbgConsultaDblClick(Sender: TObject);
var
   nId : Integer;
begin
  inherited;
