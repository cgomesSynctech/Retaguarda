inherited rptFichacardex: TrptFichacardex
  Left = 354
  Top = 175
  Width = 468
  Height = 314
  Caption = 'M'#243'dulo de Itens'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnNavigator: TTS_Panel
    Left = 342
    Height = 243
    inherited btFecharCadastro: TTS_SpeedButton
      Top = 209
    end
    inherited btLimpar: TTS_SpeedButton
      Visible = False
    end
    inherited btGravar: TTS_SpeedButton
      Caption = 'Visualizar'
      Glyph.Data = {
        
        DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
    end
  end
  inherited pnTitulo: TTS_MaxPanel
    Width = 460
    Gradient.ColorStart = 9027548
    inherited lbEstadoForm: TTS_Label
      Width = 109
      Caption = 'Relat'#243'rio Ficha Cardex'
    end
    inherited btHelp: TTS_SpeedButton
      Left = 424
    end
    inherited lbCaption: TdxfLabel
      Width = 128
      AutoSize = True
      Caption = 'Ficha Cardex.'
      Effect3D.ShadowedColor = 9027548
    end
  end
  inherited pnDados: TTS_Panel
    Width = 342
    Height = 243
    Color = 14019327
    object rgOrdem: TTS_RadioGroup
      Left = 1
      Top = 106
      Width = 340
      Height = 42
      Align = alTop
      Caption = 'Ordena'#231#227'o:'
      Columns = 2
      ItemIndex = 0
      Items.Strings = (
        'C'#243'digo'
        'Descri'#231#227'o')
      TabOrder = 0
      Transparent = True
    end
    object TS_PanelCorner1: TTS_PanelCorner
      Left = 1
      Top = 36
      Width = 340
      Height = 70
      Align = alTop
      Color = 14019327
      TabOrder = 1
      BoxWithShadow = False
      object TS_Label1: TTS_Label
        Left = 59
        Top = 29
        Width = 65
        Caption = 'Inicio:'
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label2: TTS_Label
        Left = 56
        Top = 49
        Width = 65
        Caption = 'Fim:'
        FormatoTabela = False
        LinkToResult = 0
      end
      object dtInicio: TTS_DateTimePicker
        Left = 123
        Top = 26
        Width = 121
        TabOrder = 0
        AutoSize = False
        StyleController = DMProjeto.esItens
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        Height = 19
        StoredValues = 4
      end
      object dtFim: TTS_DateTimePicker
        Left = 123
        Top = 47
        Width = 121
        TabOrder = 1
        AutoSize = False
        StyleController = DMProjeto.esItens
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        Height = 19
        StoredValues = 4
      end
      object cbPeriodo: TTS_CheckBox
        Left = 15
        Top = 6
        Width = 241
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 2
        OnClick = cbPeriodoClick
        Caption = 'Desconsiderar Per'#237'odo.'
        StyleController = DMProjeto.esItens
        DisableEdit = False
        Height = 19
      end
    end
    object TS_PanelCorner2: TTS_PanelCorner
      Left = 1
      Top = 1
      Width = 340
      Height = 35
      Align = alTop
      Color = 14019327
      TabOrder = 2
      BoxWithShadow = False
      object FiltroItens: TTS_PopupFiltrarItens
        Left = 14
        Top = 7
        Width = 279
        Hint = 'Click na seta de escolha para restrin'#231#227'o de registros'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        AutoSelect = False
        StyleController = DMProjeto.esItens
        Text = 'Filtrar itens...'
        HideEditCursor = True
        PopupFormBorderStyle = pbsSysPanel
        PopupClientEdge = True
        LookupKeyValue = 0
        TS_DB_Projeto = DMProjeto.DB_Projeto
        TS_TipoEstoque = 0
        TS_FornecedorSelecionado = 0
        TS_FornPrefSelecionado = 0
        TS_NaoPossuiEstoque = False
        TS_EstoqueMinimo = False
        TS_EstoqueMaximo = False
        TS_EstoquePontoPedido = False
        TS_EstoqueNegativo = False
        TS_ItensDesativado = False
        TS_C_LocalizarItem = DMProjeto.C_LocalizarItens
        TS_C_LocalizarFav = DMProjeto.C_LocalizarFav
        TS_PossuiEstoque = False
        TS_TipoItem = 0
        TS_AliasItens = 'i'
        TS_ItemAlterado = False
        OnSelecionou = FiltroItensSelecionou
        Height = 19
      end
    end
  end
  inherited ppmPadrao: TTS_PopupMenu
    Left = 185
    Top = 242
  end
  inherited Beep: TBTBeeper
    Left = 39
    Top = 240
  end
  inherited FormStorage: TFormStorage
    Left = 227
    Top = 246
  end
  inherited FormsComponent: TFormsComponent
    BeforeClearParams = FormsComponentBeforeClearParams
    ModuloStyle = DMProjeto.msItens
    BarEndColor = 9027548
    CaptionShadow = 9027548
    FormColor = 14019327
    Modulo = 'M'#243'dulo de Itens'
    Caption = 'Ficha Cardex.'
    Left = 116
    Top = 238
  end
  inherited ppmHelp: TTS_PopupMenu
    Left = 155
    Top = 238
  end
  object Q_Itens: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select i.item,  i.codigo, i.referencia, i.descricao, g.descricao' +
        'grupo, i.unidade ,'
      
        '(select cast(coalesce( Sum((y.fator* y.quantidade)/i.fatorundven' +
        'da) ,0) as Numeric(8,3)) from transacoesestoque y'
      '  where y.item = i.item and y.data < :Data) as SaldoAnterior'
      'From Itens I'
      '  Inner join grupos g on g.grupo = i.grupo'
      'order by i.item')
    Left = 189
    Top = 11
  end
  object Q_Transacoes: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BeforeOpen = Q_TransacoesBeforeOpen
    BufferChunks = 1000
    CachedUpdates = False
    DataSource = Q_Master
    SQL.Strings = (
      'Select t.data,  '
      '           t.unidade, '
      '           case when t.numero is not null then t.numero'
      
        '                   when (t.numero is null) and (s.numero is null' +
        ') then e.numero'
      
        '                   when (t.numero is null) and (e.numero is null' +
        ') then s.numero'
      '           end as Numero ,'
      
        '           cast(coalesce((t.fator*t.quantidade)/i.fatorundvenda,' +
        '0) as Numeric(8,3))  As LANCAMENTO,'
      
        '           case when t.quantidade > 0 then cast(coalesce((t.fato' +
        'r*t.quantidade)/i.fatorundvenda,0) as Numeric(8,3)) end  Entrada' +
        ' ,'
      
        '           case when t.quantidade < 0 then cast(coalesce((t.fato' +
        'r*t.quantidade)/i.fatorundvenda,0) as Numeric(8,3)) end  Saida, '
      
        '           (select cast(sum(coalesce((x.fator*x.quantidade)/i.fa' +
        'torundvenda,0)) as Numeric(8,3))  From transacoesestoque x where' +
        ' x.item = t.item and x.data <= t.data) as Saldo'
      'from transacoesestoque t'
      '  inner join itens i on i.item = t.item'
      
        '  left join entradas e on e.entrada = t.idgerador and (t.entrada' +
        'item is not null)'
      
        '  left join saidas s on s.saida = t.idgerador and (t.saidaitem i' +
        's not null)'
      
        'where t.item = :ITEM AND (T.DATA >= :DTINICIO AND T.DATA <= :DTF' +
        'IM)'
      'order by  1'
      '')
    UniDirectional = True
    Left = 311
    Top = 8
  end
  object Q_ItensDs: TDataSource
    DataSet = C_Itens
    Left = 190
    Top = 120
  end
  object Q_TransacoesDs: TDataSource
    DataSet = C_Transacoes
    Left = 315
    Top = 110
  end
  object ppDBItens: TppDBPipeline
    DataSource = Q_ItensDs
    OpenDataSource = False
    UserName = 'DBItens'
    Left = 192
    Top = 191
    object ppDBItensppField1: TppField
      FieldAlias = 'ITEM'
      FieldName = 'ITEM'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 0
      Searchable = False
      Sortable = False
    end
    object ppDBItensppField2: TppField
      FieldAlias = 'CODIGO'
      FieldName = 'CODIGO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 1
      Searchable = False
      Sortable = False
    end
    object ppDBItensppField3: TppField
      FieldAlias = 'REFERENCIA'
      FieldName = 'REFERENCIA'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 2
      Searchable = False
      Sortable = False
    end
    object ppDBItensppField4: TppField
      FieldAlias = 'DESCRICAO'
      FieldName = 'DESCRICAO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 3
      Searchable = False
      Sortable = False
    end
    object ppDBItensppField5: TppField
      FieldAlias = 'DESCRICAOGRUPO'
      FieldName = 'DESCRICAOGRUPO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 4
      Searchable = False
      Sortable = False
    end
    object ppDBItensppField6: TppField
      FieldAlias = 'UNIDADE'
      FieldName = 'UNIDADE'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 5
      Searchable = False
      Sortable = False
    end
    object ppDBItensppField7: TppField
      FieldAlias = 'SALDOANTERIOR'
      FieldName = 'SALDOANTERIOR'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 6
      Searchable = False
      Sortable = False
    end
    object ppDBItensppField8: TppField
      FieldAlias = 'Q_Transacoes'
      FieldName = 'Q_Transacoes'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 7
      Searchable = False
      Sortable = False
    end
  end
  object ppDBTransacoes: TppDBPipeline
    DataSource = Q_TransacoesDs
    UserName = 'DBTransacoes'
    Left = 310
    Top = 165
    MasterDataPipelineName = 'ppDBItens'
    object ppDBTransacoesppField1: TppField
      FieldAlias = 'DATA'
      FieldName = 'DATA'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 0
      Searchable = False
      Sortable = False
    end
    object ppDBTransacoesppField2: TppField
      FieldAlias = 'UNIDADE'
      FieldName = 'UNIDADE'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 1
      Searchable = False
      Sortable = False
    end
    object ppDBTransacoesppField3: TppField
      FieldAlias = 'NUMERO'
      FieldName = 'NUMERO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 2
      Searchable = False
      Sortable = False
    end
    object ppDBTransacoesppField4: TppField
      FieldAlias = 'LANCAMENTO'
      FieldName = 'LANCAMENTO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 3
      Searchable = False
      Sortable = False
    end
    object ppDBTransacoesppField5: TppField
      FieldAlias = 'ENTRADA'
      FieldName = 'ENTRADA'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 4
      Searchable = False
      Sortable = False
    end
    object ppDBTransacoesppField6: TppField
      FieldAlias = 'SAIDA'
      FieldName = 'SAIDA'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 5
      Searchable = False
      Sortable = False
    end
    object ppDBTransacoesppField7: TppField
      FieldAlias = 'SALDO'
      FieldName = 'SALDO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 6
      Searchable = False
      Sortable = False
    end
    object ppDBTransacoesppField8: TppField
      FieldAlias = 'icSALDO'
      FieldName = 'icSALDO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 7
      Searchable = False
      Sortable = False
    end
  end
  object ppFichaCardex: TppReport
    AutoStop = False
    DataPipeline = ppDBItens
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    DeviceType = 'Screen'
    Left = 255
    Top = 194
    Version = '6.02'
    mmColumnWidth = 0
    DataPipelineName = 'ppDBItens'
    object ppHeaderBand1: TppHeaderBand
      mmBottomOffset = 0
      mmHeight = 32279
      mmPrintPosition = 0
      object regCab: TppRegion
        UserName = 'regCab'
        Caption = 'regCab'
        Pen.Style = psClear
        mmHeight = 25400
        mmLeft = 1323
        mmTop = 529
        mmWidth = 194734
        BandType = 0
        mmBottomOffset = 0
        mmOverFlowOffset = 0
        mmStopPosition = 0
      end
      object lbPeriodo: TppLabel
        UserName = 'lbPeriodo'
        Caption = 'lbPeriodo'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Tahoma'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 4233
        mmLeft = 91811
        mmTop = 27781
        mmWidth = 14288
        BandType = 0
      end
    end
    object ppDetailBand1: TppDetailBand
      PrintHeight = phDynamic
      mmBottomOffset = 0
      mmHeight = 24077
      mmPrintPosition = 0
      object ppDBText1: TppDBText
        UserName = 'DBText1'
        AutoSize = True
        DataField = 'DESCRICAO'
        DataPipeline = ppDBItens
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 12
        Font.Style = [fsBold, fsItalic]
        Transparent = True
        DataPipelineName = 'ppDBItens'
        mmHeight = 4995
        mmLeft = 46831
        mmTop = 5821
        mmWidth = 25231
        BandType = 4
      end
      object ppDBText2: TppDBText
        UserName = 'DBText2'
        DataField = 'CODIGO'
        DataPipeline = ppDBItens
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 12
        Font.Style = [fsBold, fsItalic]
        Transparent = True
        DataPipelineName = 'ppDBItens'
        mmHeight = 4498
        mmLeft = 16404
        mmTop = 5821
        mmWidth = 28840
        BandType = 4
      end
      object ppLabel1: TppLabel
        UserName = 'Label1'
        Caption = 'Item:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 12
        Font.Style = [fsBold, fsItalic]
        Transparent = True
        mmHeight = 4498
        mmLeft = 5027
        mmTop = 5821
        mmWidth = 10319
        BandType = 4
      end
      object ppLabel2: TppLabel
        UserName = 'Label2'
        Caption = 'Ultimos Lan'#231'amentos...'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        Transparent = True
        mmHeight = 3175
        mmLeft = 36777
        mmTop = 12700
        mmWidth = 28575
        BandType = 4
      end
      object ppDBText8: TppDBText
        UserName = 'DBText8'
        DataField = 'SALDOANTERIOR'
        DataPipeline = ppDBItens
        DisplayFormat = '###,###,##0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBItens'
        mmHeight = 3979
        mmLeft = 180446
        mmTop = 11642
        mmWidth = 17198
        BandType = 4
      end
      object ppLabel9: TppLabel
        UserName = 'Label9'
        Caption = 'Saldo Anterior:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3969
        mmLeft = 155575
        mmTop = 11642
        mmWidth = 23548
        BandType = 4
      end
      object ppSubReport1: TppSubReport
        UserName = 'SubReport1'
        ExpandAll = False
        NewPrintJob = False
        TraverseAllData = False
        DataPipelineName = 'ppDBTransacoes'
        mmHeight = 5027
        mmLeft = 0
        mmTop = 16933
        mmWidth = 197300
        BandType = 4
        mmBottomOffset = 0
        mmOverFlowOffset = 0
        mmStopPosition = 0
        object ppChildReport1: TppChildReport
          AutoStop = False
          DataPipeline = ppDBTransacoes
          PrinterSetup.BinName = 'Default'
          PrinterSetup.DocumentName = 'Report'
          PrinterSetup.PaperName = 'A4'
          PrinterSetup.PrinterName = 'Default'
          PrinterSetup.mmMarginBottom = 6350
          PrinterSetup.mmMarginLeft = 6350
          PrinterSetup.mmMarginRight = 6350
          PrinterSetup.mmMarginTop = 6350
          PrinterSetup.mmPaperHeight = 297000
          PrinterSetup.mmPaperWidth = 210000
          PrinterSetup.PaperSize = 9
          Version = '6.02'
          mmColumnWidth = 0
          DataPipelineName = 'ppDBTransacoes'
          object ppTitleBand1: TppTitleBand
            mmBottomOffset = 0
            mmHeight = 8731
            mmPrintPosition = 0
            object ppLine1: TppLine
              UserName = 'Line1'
              Pen.Style = psDot
              Weight = 0.75
              mmHeight = 1323
              mmLeft = 1058
              mmTop = 7408
              mmWidth = 196586
              BandType = 1
            end
            object ppSALDOANTERIOR: TppLabel
              UserName = 'SALDOANTERIOR'
              Caption = 'Saldo'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              TextAlignment = taRightJustified
              Transparent = True
              mmHeight = 3969
              mmLeft = 188119
              mmTop = 2910
              mmWidth = 8996
              BandType = 1
            end
            object ppLabel6: TppLabel
              UserName = 'Label6'
              Caption = 'Saidas'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              TextAlignment = taRightJustified
              Transparent = True
              mmHeight = 3969
              mmLeft = 165365
              mmTop = 2910
              mmWidth = 10848
              BandType = 1
            end
            object ppLabel5: TppLabel
              UserName = 'Label5'
              Caption = 'Entradas'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              TextAlignment = taRightJustified
              Transparent = True
              mmHeight = 3969
              mmLeft = 136261
              mmTop = 2910
              mmWidth = 14023
              BandType = 1
            end
            object ppLabel4: TppLabel
              UserName = 'Label4'
              Caption = 'Und.'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              TextAlignment = taCentered
              Transparent = True
              mmHeight = 3969
              mmLeft = 114829
              mmTop = 2910
              mmWidth = 7408
              BandType = 1
            end
            object ppLabel3: TppLabel
              UserName = 'Label3'
              Caption = 'Data'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              Transparent = True
              mmHeight = 3969
              mmLeft = 7408
              mmTop = 2910
              mmWidth = 7408
              BandType = 1
            end
            object ppLine4: TppLine
              UserName = 'Line4'
              Pen.Style = psDot
              Weight = 0.75
              mmHeight = 1323
              mmLeft = 1058
              mmTop = 7408
              mmWidth = 196586
              BandType = 1
            end
            object ppLabel16: TppLabel
              UserName = 'Label16'
              Caption = 'N'#186' Lan'#231'amento'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              TextAlignment = taRightJustified
              Transparent = True
              mmHeight = 3969
              mmLeft = 57415
              mmTop = 3175
              mmWidth = 24342
              BandType = 1
            end
          end
          object ppDetailBand2: TppDetailBand
            mmBottomOffset = 0
            mmHeight = 4763
            mmPrintPosition = 0
            object ppDBText3: TppDBText
              UserName = 'DBText3'
              DataField = 'DATA'
              DataPipeline = ppDBTransacoes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              ParentDataPipeline = False
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 3969
              mmLeft = 7408
              mmTop = 265
              mmWidth = 24077
              BandType = 4
            end
            object ppDBText4: TppDBText
              UserName = 'DBText4'
              DataField = 'UNIDADE'
              DataPipeline = ppDBTransacoes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taCentered
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 3969
              mmLeft = 110067
              mmTop = 265
              mmWidth = 17198
              BandType = 4
            end
            object ppDBText5: TppDBText
              UserName = 'DBText5'
              DataField = 'ENTRADA'
              DataPipeline = ppDBTransacoes
              DisplayFormat = '###,###,##0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 3969
              mmLeft = 133086
              mmTop = 265
              mmWidth = 17198
              BandType = 4
            end
            object ppDBText6: TppDBText
              UserName = 'DBText6'
              DataField = 'SAIDA'
              DataPipeline = ppDBTransacoes
              DisplayFormat = '###,###,##0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              ParentDataPipeline = False
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 3969
              mmLeft = 159015
              mmTop = 265
              mmWidth = 17198
              BandType = 4
            end
            object ppDBText10: TppDBText
              UserName = 'DBText10'
              DataField = 'numero'
              DataPipeline = ppDBTransacoes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 3969
              mmLeft = 57415
              mmTop = 265
              mmWidth = 37306
              BandType = 4
            end
            object ppDBText7: TppDBText
              UserName = 'DBText7'
              DataField = 'SALDO'
              DataPipeline = ppDBTransacoes
              DisplayFormat = '###,###,##0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = []
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 3969
              mmLeft = 179917
              mmTop = 529
              mmWidth = 17198
              BandType = 4
            end
          end
          object ppSummaryBand1: TppSummaryBand
            mmBottomOffset = 0
            mmHeight = 12435
            mmPrintPosition = 0
            object ppLabel8: TppLabel
              UserName = 'Label8'
              Caption = 'Total Movimenta'#231#227'o ==>>'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = [fsBold]
              Transparent = True
              mmHeight = 4233
              mmLeft = 65881
              mmTop = 3440
              mmWidth = 43773
              BandType = 7
            end
            object ppDBCalc1: TppDBCalc
              UserName = 'DBCalc1'
              DataField = 'ENTRADA'
              DataPipeline = ppDBTransacoes
              DisplayFormat = '###,###,###,##0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 9
              Font.Style = [fsBold]
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 3969
              mmLeft = 121973
              mmTop = 3704
              mmWidth = 28310
              BandType = 7
            end
            object ppDBCalc2: TppDBCalc
              UserName = 'DBCalc2'
              DataField = 'SAIDA'
              DataPipeline = ppDBTransacoes
              DisplayFormat = '###,###,###,##0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 9
              Font.Style = [fsBold]
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 3969
              mmLeft = 151342
              mmTop = 3704
              mmWidth = 25135
              BandType = 7
            end
            object ppLine2: TppLine
              UserName = 'Line2'
              Weight = 0.75
              mmHeight = 1323
              mmLeft = 794
              mmTop = 11113
              mmWidth = 196321
              BandType = 7
            end
            object ppLine3: TppLine
              UserName = 'Line3'
              Pen.Style = psDot
              Weight = 0.75
              mmHeight = 1323
              mmLeft = 69056
              mmTop = 265
              mmWidth = 128323
              BandType = 7
            end
            object ppDBText9: TppDBText
              UserName = 'DBText9'
              DataField = 'SALDO'
              DataPipeline = ppDBTransacoes
              DisplayFormat = '###,###,##0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Name = 'Arial'
              Font.Size = 10
              Font.Style = [fsBold]
              TextAlignment = taRightJustified
              Transparent = True
              DataPipelineName = 'ppDBTransacoes'
              mmHeight = 4233
              mmLeft = 179917
              mmTop = 3704
              mmWidth = 17198
              BandType = 7
            end
          end
          object raCodeModule1: TraCodeModule
            ProgramStream = {00}
          end
        end
      end
    end
  end
  object P_Itens: TDataSetProvider
    DataSet = Q_Itens
    Constraints = True
    Left = 188
    Top = 62
  end
  object C_Itens: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'P_Itens'
    Left = 143
    Top = 107
    object C_ItensITEM: TIntegerField
      FieldName = 'ITEM'
      Origin = 'ITENS.ITEM'
      Required = True
    end
    object C_ItensCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'ITENS.CODIGO'
      Size = 13
    end
    object C_ItensREFERENCIA: TStringField
      FieldName = 'REFERENCIA'
      Origin = 'ITENS.REFERENCIA'
      Size = 25
    end
    object C_ItensDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = 'ITENS.DESCRICAO'
      Size = 50
    end
    object C_ItensDESCRICAOGRUPO: TStringField
      FieldName = 'DESCRICAOGRUPO'
      Origin = 'GRUPOS.DESCRICAOGRUPO'
      Required = True
      Size = 30
    end
    object C_ItensUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Origin = 'ITENS.UNIDADE'
      Size = 6
    end
    object C_ItensSALDOANTERIOR: TBCDField
      FieldName = 'SALDOANTERIOR'
      Precision = 9
      Size = 3
    end
    object C_ItensQ_Transacoes: TDataSetField
      FieldName = 'Q_Transacoes'
    end
  end
  object C_Transacoes: TClientDataSet
    Aggregates = <>
    DataSetField = C_ItensQ_Transacoes
    Params = <>
    Left = 312
    Top = 65
    object C_TransacoesDATA: TDateField
      FieldName = 'DATA'
      Origin = 'TRANSACOESESTOQUE.DATA'
    end
    object C_TransacoesUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Origin = 'TRANSACOESESTOQUE.UNIDADE'
      Size = 6
    end
    object C_TransacoesNUMERO: TStringField
      FieldName = 'NUMERO'
      Size = 15
    end
    object C_TransacoesLANCAMENTO: TBCDField
      FieldName = 'LANCAMENTO'
      Precision = 9
      Size = 3
    end
    object C_TransacoesENTRADA: TBCDField
      FieldName = 'ENTRADA'
      Precision = 9
      Size = 3
    end
    object C_TransacoesSAIDA: TBCDField
      FieldName = 'SAIDA'
      Precision = 9
      Size = 3
    end
    object C_TransacoesSALDO: TBCDField
      FieldName = 'SALDO'
      Precision = 9
      Size = 3
    end
    object C_TransacoesicSALDO: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'icSALDO'
    end
  end
  object Q_Master: TDataSource
    DataSet = Q_Itens
    Left = 117
    Top = 12
  end
end
