...
begin
    ...
    Q_Consulta.SQL.Text := 'select distinct s.saida, s.numero, s.data, s.favorecido, coalesce(fa.nome, ''ND'') as nomefavorecido, fa.tipofavorecido, ' +
        ...
        ' s.total, tm.tipomovimento, tm.descricao as operacao, s.tipopadrao, coalesce(p.descricao, ''Não Definido'') as planopagamento ' +
    ...
    FormsComponent.Caption := 'Saída (Produtos Identificados) no Período de: ' + DateToStr(DataI.Date) + ' a ' + DateToStr(DataF.Date);
