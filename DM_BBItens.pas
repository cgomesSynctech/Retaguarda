...
type
  TDMBBItens = class(TDataModule)
    ...
    C_InventarioCUSTO: TFloatField;
    C_InventarioCUSTOMEDIO: TFloatField;
    C_InventarioCUSTOCONTABIL: TFloatField;
    C_InventarioPRECOCOMPRA: TFloatField;
    C_ServicosCUSTOMEDIO: TFloatField;
    C_ServicosCUSTOCONTABIL: TFloatField;
    C_ServicosPRECOCOMPRA: TFloatField;
    C_InventarioCUSTOOLD: TFloatField;
    C_SemInventCUSTOMEDIO: TFloatField;
    C_DescontosCUSTOMEDIO: TFloatField;
    C_EncargosCUSTOMEDIO: TFloatField;
    C_EncargosPRECOCOMPRA: TFloatField;
    C_EncargosCUSTOCONTABIL: TFloatField;
    ...
    C_InventarioDATACADASTRO: TDateField;
    C_InventarioULTIMACOMPRA: TDateField;
    C_InventarioUNIDADEENTRADA: TStringField;
    C_InventarioUNIDADEVAREJO: TStringField;
    C_InventarioULTIMAVENDA: TDateField;
    C_InventarioREDUCAOCST: TBCDField;
    C_InventarioIPICOMPRA: TBCDField;
    C_InventarioPESOBRUTO: TBCDField;
    C_InventarioPESOLIQUIDO: TBCDField;
    C_InventarioALIQICMS: TBCDField;
    C_InventarioSITUACAOECF: TStringField;
    C_ProdutosPrecoUNIDADE: TStringField;
    C_GruposItensFOTO: TStringField;
    C_GruposItensFATORLUCRO: TBCDField;
    C_GruposItensCST: TStringField;
    C_GruposItensREDUCAOCST: TBCDField;
    C_GruposItensALIQICMS: TBCDField;
    C_GruposItensSITUACAOECF: TStringField;
    C_InventarioQ_ItensUnidades: TDataSetField;
    C_InventarioCODIGOBARRAS: TStringField;
    C_InventarioLOCALIZACAO: TIntegerField;
    C_InventarioFORNECPREFERENCIA: TIntegerField;
    C_InventarioORIGEMMERCADORIA: TIntegerField;
    C_InventarioMODALIDADEBCICMS: TIntegerField;
    C_InventarioMODALIDADEBCICMSST: TIntegerField;
    C_InventarioCSTPISCOFINS: TStringField;
    C_InventarioALIQPIS: TBCDField;
    C_InventarioALIQCOFINS: TBCDField;
    C_InventarioCSTIPI: TStringField;
    C_InventarioGENERO: TStringField;
    C_InventarioALIQIR: TBCDField;
    C_InventarioALIQSS: TBCDField;
    C_InventarioIPPT: TStringField;
    C_InventarioIAT: TStringField;
    C_InventarioALIQIPI: TFloatField;
    C_InventarioCSTPISCOFINSSAIDA: TStringField;
    C_InventarioALIQPISSAIDA: TFloatField;
    C_InventarioALIQCOFINSSAIDA: TFloatField;
    C_InventarioCODANP: TStringField;
    C_InventarioCODIF: TStringField;
    C_InventarioCODIGOVENDA: TStringField;
    C_InventarioCEST_OPC: TStringField;
    C_InventarioCSTIPIENTRADA: TStringField;
    C_InventarioCENQIPICOMPRA: TStringField;
    C_InventarioCENQIPIVENDA: TStringField;
    C_InventarioCLASFISCAL: TStringField;
    C_InventarioALIQICMSCOMPRA: TBCDField;
    C_ItensCodigos: TClientDataSet;
    C_ItensCodigosDs: TDataSource;
    Q_ItensCodigos: TIBQuery;
    U_ItensCodigos: TIBUpdateSQL;
    C_InventarioQ_ItensCodigos: TDataSetField;
    C_ItensCodigosITEM: TIntegerField;
    C_ItensCodigosCODIGO: TStringField;
    C_ItensCodigosUNIDADE: TStringField;
    C_ItensCodigosORIGEMREPLIC: TStringField;
    C_InventarioCST: TStringField;
  ...
  public
    ...
    bImportando: boolean;
    nTipoItem: integer;
    function AdicionaTabelasPreco(xPreco:currency;sUnid:string = ''):boolean;
    ...
    function LocalizaFornecedor(sCodigo:string):integer;
  end;
...
begin
  ...
  // Inventário
  try
    if not C_Inventario.Active then
      C_Inventario.Open;
{    C_Inventario.first;
    ...
    while not C_Inventario.eof do begin
      if (C_InventarioIDNovo.value < 0) then begin
        nAjuste := 0;
        if (C_InventarioQuantidade.value <> 0) then
          nAjuste := AjustarItem;
      end;
      C_Inventario.next;
    end;
...
begin
  ...
  C_InventarioQuantidade.value,C_InventarioCusto.value,'Saldo Inicial: '+C_InventarioDescricao.value,'',DMProjeto.dDataSistema);
end;
...
begin
  ...
  C_InventarioQuantidade.value,C_InventarioCusto.value,'Saldo Inicial: '+C_InventarioDescricao.value, C_InventarioUnidade.value);
end;
...
begin
  ...
  C_InventarioMotivo.value := 'Saldo Inicial';
  C_InventarioConta.value := nContaBB;
  C_InventarioCusto.value := 0;
  C_InventarioULTIMOFORNECEDOR.Value := -9;
  C_InventarioFORNECPREFERENCIA.Value := -9;
...
begin
  ...
  Dataset.FieldByName('ALIQPIS').value := 0;                   
  Dataset.FieldByName('ALIQCOFINS').value := 0;                
  Dataset.FieldByName('ALIQIR').value := 0;                    
  Dataset.FieldByName('ALIQSS').value := 0;               
  Dataset.FieldByName('IPPT').value := 'T';                      
  Dataset.FieldByName('IAT').value := 'T';                       
  Dataset.FieldByName('ALIQIPI').value := 0;                   
  Dataset.FieldByName('ALIQPISSAIDA').value := 0;
  Dataset.FieldByName('ALIQCOFINSSAIDA').value := 0;
  Dataset.FieldByName('ALIQICMSCOMPRA').value := 0;
end;
  ...
  if (C_ProdutosPrecoItem.IsNull) then with C_TabelasPreco do begin
    ...
    while not eof do begin
      if not C_ProdutosPreco.Locate('tabelapreco',FieldByName('TabelaPreco').AsInteger,[]) then begin
        ...
        if sUnid <> '' then
          C_ProdutosPrecoUnidade.value := sUnid;
...
function TDMBBItens.LocalizaFornecedor;
begin
  result := 0;
  Q_Aux.close;
  Q_Aux.SQL.text := 'select favorecido from favorecidos where codigo = :codigo and tipofavorecido = 2 and desativado = ''N''';
  Q_Aux.params[0].asstring := sCodigo;
  Q_Aux.open;
...
procedure TDMBBItens.C_ItensUnidadesNewRecord(DataSet: TDataSet);
begin
  C_ItensUnidadesItemUnidade.value := -1;
end;

procedure TDMBBItens.C_ItensCodigosNewRecord(DataSet: TDataSet);
begin
  C_ItensCodigosITEM.Value := -1;
  C_ItensCodigosCodigo.Value := '-1';
end;
