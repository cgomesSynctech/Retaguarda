inherited DlgHistoricoRetiradas: TDlgHistoricoRetiradas
  Left = 140
  Top = 140
  Width = 554
  Height = 324
  Caption = 'Fornecedores / Compras'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnDados: TTS_Panel
    Top = 48
    Width = 428
    Height = 242
    Color = 15196656
    object TS_Panel1: TTS_Panel
      Left = 1
      Top = 1
      Width = 426
      Height = 58
      Align = alTop
      BevelOuter = bvNone
      Color = 15196913
      TabOrder = 0
      object TS_Label2: TTS_Label
        Left = 61
        Top = 23
        Width = 68
        Caption = '&No Per'#237'odo:'
        FocusControl = dtDataI
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label1: TTS_Label
        Left = 225
        Top = 23
        Width = 15
        Alignment = taCenter
        Caption = ' '#224' '
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_DBLookupTipoFav1: TTS_DBLookupTipoFav
        Left = 20
        Top = 1
        Width = 109
        Hint = 
          'Selecione o tipo de favorecido para filtrar a pesquisar no campo' +
          ' de edi'#231#227'o ao lado.'
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        StyleController = DMProjeto.esFornecedores
        DropDownRows = 15
        ListFieldName = 'DESCRICAO'
        KeyFieldName = 'TIPOFAVORECIDO'
        ListSource = DMProjeto.C_TipoFavDS
        LookupKeyValue = Null
        TipoFavorecido = 2
      end
      object edFavorecido: TTS_EditFavorecido
        Left = 130
        Top = 2
        Width = 207
        Hint = 
          '#C'#243'digo => Pesquisa pelo c'#243'digo. Ex: #100, #DAP'#13'SSN:N'#250'mero SSN =' +
          '> Pesquisa pelo SSN. Ex: SSN:123366642'#13'EIN:N'#250'mero EIN => Pesquis' +
          'a pelo EIN. Ex: EIN:6719034663'
        Style.ButtonStyle = btsFlat
        TabOrder = 1
        OnKeyDown = edFavorecidoKeyDown
        StyleController = DMProjeto.esFornecedores
        OnChange = edFavorecidoChange
        Buttons = <
          item
            Default = True
            Glyph.Data = {
              
              07070707070707070700}
            Kind = bkGlyph
            Width = 18
          end>
        LookupTipoFav = TS_DBLookupTipoFav1
        SelecionarVarios = False
        TipoFavorecido = 2
        C_Localizar = DMProjeto.C_LocalizarFav
        ID = 0
        OnSelecionou = edFavorecidoSelecionou
        Height = 19
        ExistButtons = True
      end
      object dtDataI: TTS_DateTimePicker
        Left = 130
        Top = 21
        Width = 87
        TabOrder = 2
        AutoSize = False
        StyleController = DMProjeto.esFornecedores
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = dtDataIDateChange
        Height = 19
        StoredValues = 4
      end
      object dtDataF: TTS_DateTimePicker
        Left = 250
        Top = 21
        Width = 87
        TabOrder = 3
        AutoSize = False
        StyleController = DMProjeto.esFornecedores
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = dtDataIDateChange
        Height = 19
        StoredValues = 4
      end
      object ckbCancelados: TTS_CheckBox
        Left = 127
        Top = 38
        Width = 206
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 4
        Caption = '&Visualizar tamb'#233'm os cancelados'
        StyleController = DMProjeto.esFornecedores
        OnChange = ckbCanceladosChange
        DisableEdit = False
        Height = 21
      end
    end
    object DBRetiradas: TTS_QDBGrid
      Left = 1
      Top = 59
      Width = 426
      Height = 182
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'IDDOC'
      SummaryGroups = <>
      SummarySeparator = ', '
      Align = alClient
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = ppmPadrao
      TabOrder = 1
      OnDblClick = btGravarClick
      AutoSearchColor = 6611199
      AutoSearchTextColor = clBlue
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = []
      DataSource = C_HistoricoDS
      Filter.Criteria = {00000000}
      GroupPanelColor = 15461355
      HeaderColor = 15461355
      HeaderFont.Charset = DEFAULT_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -11
      HeaderFont.Name = 'Tahoma'
      HeaderFont.Style = []
      HideFocusRect = True
      HideSelection = True
      HideSelectionColor = 15394553
      HighlightColor = 15394553
      OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoEnterThrough, edgoImmediateEditor, edgoMultiSort, edgoTabs, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      OnCustomDrawCell = DBRetiradasCustomDrawCell
      TS_PermitirQuantidade = False
      TS_DescriptionCanChange = False
      TS_AppendOnEnter = False
      RowFooterNodeFont.Charset = DEFAULT_CHARSET
      RowFooterNodeFont.Color = clWindowText
      RowFooterNodeFont.Height = -11
      RowFooterNodeFont.Name = 'Tahoma'
      RowFooterNodeFont.Style = [fsBold]
      TS_AccountFooterStyle = False
      TS_HideFocusedRect = hfShowAlways
      TS_HideGroupHeader = False
      TS_AnotherColor = clWindow
      TS_ReportHeaderStyle = False
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clWindowText
      GroupNodeFont.Height = -11
      GroupNodeFont.Name = 'Tahoma'
      GroupNodeFont.Style = []
      TS_SummaryFooterFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterFont.Color = clWindowText
      TS_SummaryFooterFont.Height = -11
      TS_SummaryFooterFont.Name = 'Tahoma'
      TS_SummaryFooterFont.Style = []
      TS_SummaryFooterSelFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterSelFont.Color = clWindowText
      TS_SummaryFooterSelFont.Height = -11
      TS_SummaryFooterSelFont.Name = 'Tahoma'
      TS_SummaryFooterSelFont.Style = []
      TS_ShowSelectionColumn = True
      TS_LikeString = False
      TS_SelectionColor = 12054783
      TS_SelectionFont.Charset = DEFAULT_CHARSET
      TS_SelectionFont.Color = clWindowText
      TS_SelectionFont.Height = -11
      TS_SelectionFont.Name = 'Tahoma'
      TS_SelectionFont.Style = []
      TS_SelectedColumn = '_icSelecionado'
      TS_ID = 0
      TS_TipoDescricao = tdVenda
      TS_SummaryFooterQtdText = 'Qtd. Geral:'
      TS_SummaryFooterQtdSelectedText = 'Qtd. Selecionada:'
      object DBRetiradasRETIRADA: TdxDBGridMaskColumn
        HeaderAlignment = taCenter
        Visible = False
        Width = 42
        BandIndex = 0
        RowIndex = 0
        FieldName = 'RETIRADA'
      end
      object DBRetiradasTITULO: TdxDBGridMaskColumn
        HeaderAlignment = taCenter
        Visible = False
        Width = 42
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TITULO'
      end
      object DBRetiradasDATA: TdxDBGridDateColumn
        DisableEditor = True
        HeaderAlignment = taCenter
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATA'
      end
      object DBRetiradasNOME: TdxDBGridMaskColumn
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 193
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOME'
      end
      object DBRetiradasFORMAPAGAMENTO: TdxDBGridColumn
        Caption = 'Forma Pag.'
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 82
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FORMAPAGAMENTO'
      end
      object DBRetiradasVALOR: TdxDBGridMaskColumn
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 70
        BandIndex = 0
        RowIndex = 0
        FieldName = 'VALOR'
      end
      object DBRetiradasFAVORECIDODOC: TdxDBGridMaskColumn
        HeaderAlignment = taCenter
        Visible = False
        Width = 69
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FAVORECIDODOC'
      end
      object DBRetiradasSituacao: TdxDBGridColumn
        DisableCustomizing = True
        Visible = False
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Situacao'
      end
    end
  end
  inherited pnNavigator: TTS_Panel
    Left = 428
    Top = 48
    Height = 242
    inherited btFecharCadastro: TTS_SpeedButton
      Top = 208
    end
    inherited btComando2: TTS_SpeedButton
      Top = 73
      Caption = '&Cancelar'
      Glyph.Data = {
        
        B9B9B9B9B9B9B9B9B9B9B9B9B9B9B9B9B9B9}
      NumGlyphs = 2
      Visible = True
      OnClick = btComando2Click
    end
    inherited btComando1: TTS_SpeedButton
      Top = 106
    end
    inherited btLimpar: TTS_SpeedButton
      Caption = '&Pesquisar'
      Glyph.Data = {
        
        DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
    end
    inherited btGravar: TTS_SpeedButton
      Caption = '&Alterar'
    end
  end
  inherited pnTitulo: TTS_MaxPanel
    Width = 546
    Height = 48
    Gradient.ColorStart = 11775972
    inherited lbCaption: TdxfLabel
      Width = 239
      Caption = 'Hist'#243'rico de Pagamentos'
      Effect3D.ShadowedColor = 11775972
    end
    inherited lbEstadoForm: TTS_Label
      Top = 22
      Width = 224
      Height = 26
      Caption = 
        'Opera'#231#245'es realizadas anteriormente.'#13#10'Utilize para alterar ou can' +
        'celar um pagamento.'
    end
    inherited lbUpperHint: TTS_Label
      Left = 538
    end
  end
  inherited ppmPadrao: TTS_PopupMenu
    Left = 272
    Top = 130
    object Alterar1: TMenuItem [0]
      Caption = 'Alterar'
      Default = True
      ShortCut = 116
      OnClick = btGravarClick
    end
    object Pesquisar1: TMenuItem [1]
      Caption = 'Pesquisar'
      ShortCut = 120
      OnClick = btLimparClick
    end
    object Cancelar1: TMenuItem [2]
      Caption = 'Cancelar'
      OnClick = btComando2Click
    end
    object N1: TMenuItem [3]
      Caption = '-'
    end
  end
  inherited FormsComponent: TFormsComponent
    BeforeClearParams = FormsComponentBeforeClearParams
    ModuloStyle = DMProjeto.msFornecedores
    BarEndColor = 11775972
    CaptionShadow = 11775972
    FormColor = 15196656
    Modulo = 'Fornecedores / Compras'
    Caption = 'Hist'#243'rico de Pagamentos'
    OnRefresh = FormsComponentRefresh
    FirstEditField = edFavorecido
  end
  inherited FormStorage: TFormStorage
    Top = 128
  end
  object C_Historico: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'P_Historico'
    OnCalcFields = C_HistoricoCalcFields
    Left = 68
    Top = 128
    object C_HistoricoRETIRADA: TIntegerField
      FieldName = 'RETIRADA'
      Required = True
    end
    object C_HistoricoDATA: TDateField
      DisplayLabel = 'Data'
      FieldName = 'DATA'
    end
    object C_HistoricoNOME: TStringField
      DisplayLabel = 'Favorecido'
      FieldName = 'NOME'
      Size = 50
    end
    object C_HistoricoVALOR: TBCDField
      DisplayLabel = 'Valor'
      FieldName = 'VALOR'
      DisplayFormat = '###,###,##0.00'
      Precision = 18
      Size = 2
    end
    object C_HistoricoFAVORECIDODOC: TIntegerField
      FieldName = 'FAVORECIDODOC'
    end
    object C_Historico_icSelecionado: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = '_icSelecionado'
    end
    object C_HistoricoID: TStringField
      FieldKind = fkInternalCalc
      FieldName = 'ID'
      Size = 10
    end
    object C_HistoricoSITUACAO: TStringField
      FieldName = 'SITUACAO'
      Size = 1
    end
    object C_HistoricoIDDOC: TIntegerField
      FieldName = 'IDDOC'
      Required = True
    end
    object C_HistoricoFORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Size = 50
    end
  end
  object P_Historico: TDataSetProvider
    DataSet = Q_Historico
    Constraints = True
    Options = [poAllowCommandText]
    Left = 128
    Top = 168
  end
  object Q_Historico: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'Select r.Retirada, rd.IDDoc, r.Data, r.Situacao, f.Nome, c.Descr' +
        'icao as NomeConta, r.Valor, rd.FavorecidoDoc, fp.FormaPagamento'
      
        'from Retiradas r inner join RetiradasDoc rd on r.retirada = rd.r' +
        'etirada'
      
        '        inner join Favorecidos f on rd.FavorecidoDoc = f.Favorec' +
        'ido'
      '        inner join Contas c on rd.Conta = c.Conta'
      
        '        inner join FormasPagamento fp on fp.formapagamento = rd.' +
        'formapagamento')
    Left = 128
    Top = 128
  end
  object C_HistoricoDS: TDataSource
    DataSet = C_Historico
    Left = 128
    Top = 208
  end
end
