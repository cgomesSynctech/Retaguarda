inherited DlgHistoricoDepositos: TDlgHistoricoDepositos
  Left = 396
  Top = 169
  Width = 554
  Height = 328
  HelpContext = 0
  Caption = 'Clientes / Vendas'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnNavigator: TTS_Panel
    Left = 420
    Top = 47
    Height = 243
    inherited btFecharCadastro: TTS_SpeedButton
      Top = 209
    end
    inherited btComando2: TTS_SpeedButton
      Top = 73
      Hint = 'Cancela os recebimentos selecionados'
      Caption = '&Cancelar'
      Glyph.Data = {
        
        B9B9B9B9B9B9B9B9B9B9B9B9B9B9B9B9B9B9}
      NumGlyphs = 2
      Visible = True
      OnClick = btComando2Click
    end
    inherited btComando1: TTS_SpeedButton
      Top = 106
    end
    inherited btLimpar: TTS_SpeedButton
      Hint = 'Pesquisar de acordo com o cliente e/ou per'#237'odo informado.'
      Caption = '&Pesquisar'
      Glyph.Data = {
        
        DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
    end
    inherited btGravar: TTS_SpeedButton
      Hint = 'Visualizar/Alterar o recebimento selecionado'
      Caption = '&Alterar'
    end
  end
  inherited pnTitulo: TTS_MaxPanel
    Width = 538
    Height = 47
    Gradient.ColorStart = 14859922
    inherited lbEstadoForm: TTS_Label
      Top = 21
      Width = 229
      Height = 26
      Caption = 
        'Opera'#231#245'es realizadas anteriormente. '#13#10'Utilize para alterar ou ca' +
        'ncelar um recebimento.'
    end
    inherited lbUpperHint: TTS_Label
      Left = 538
    end
    inherited lbCaption: TdxfLabel
      Width = 257
      AutoSize = True
      Caption = 'Hist'#243'rico de Recebimentos'
      Effect3D.ShadowedColor = 14859922
    end
  end
  inherited pnDados: TTS_Panel
    Top = 47
    Width = 420
    Height = 243
    Color = 16116702
    object TS_Panel1: TTS_Panel
      Left = 1
      Top = 1
      Width = 418
      Height = 57
      Align = alTop
      BevelOuter = bvNone
      Color = 16247774
      TabOrder = 0
      object TS_Label2: TTS_Label
        Left = 62
        Top = 23
        Width = 68
        Caption = '&No Per'#237'odo:'
        FocusControl = dtDataI
        FormatoTabela = False
        LinkToResult = 0
      end
      object TS_Label1: TTS_Label
        Left = 235
        Top = 22
        Width = 15
        Alignment = taCenter
        Caption = ' '#224' '
        FormatoTabela = False
        LinkToResult = 0
      end
      object edFavorecido: TTS_EditFavorecido
        Left = 133
        Top = 1
        Width = 244
        Hint = 
          '#C'#243'digo => Pesquisa pelo c'#243'digo. Ex: #100, #DAP'#13'SSN:N'#250'mero SSN =' +
          '> Pesquisa pelo SSN. Ex: SSN:123366642'#13'EIN:N'#250'mero EIN => Pesquis' +
          'a pelo EIN. Ex: EIN:6719034663'
        Style.ButtonStyle = btsFlat
        TabOrder = 1
        OnKeyDown = edFavorecidoKeyDown
        StyleController = DMProjeto.esClientes
        OnChange = edFavorecidoChange
        Buttons = <
          item
            Default = True
            Glyph.Data = {
              
              07070707070707070700}
            Kind = bkGlyph
            Width = 18
          end
          item
            Default = False
            Glyph.Data = {
              
              07070707070707000000}
            Kind = bkGlyph
            Width = 17
          end>
        LookupTipoFav = TS_DBLookupTipoFav1
        SelecionarVarios = False
        C_Localizar = DMProjeto.C_LocalizarFav
        ID = 0
        OnSelecionou = edFavorecidoSelecionou
        Height = 19
        ExistButtons = True
      end
      object dtDataI: TTS_DateTimePicker
        Left = 132
        Top = 20
        Width = 89
        TabOrder = 2
        OnKeyDown = edFavorecidoKeyDown
        AutoSize = False
        StyleController = DMProjeto.esClientes
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = dtDataIDateChange
        Height = 19
        StoredValues = 4
      end
      object dtDataF: TTS_DateTimePicker
        Left = 264
        Top = 20
        Width = 89
        TabOrder = 3
        OnKeyDown = edFavorecidoKeyDown
        AutoSize = False
        StyleController = DMProjeto.esClientes
        ButtonGlyph.Data = {
          
          FFFFFFA40000FDA4A4A4A4A4A4A4A4FD0000}
        Date = -700000
        UseEditMask = True
        OnDateChange = dtDataIDateChange
        Height = 19
        StoredValues = 4
      end
      object TS_DBLookupTipoFav1: TTS_DBLookupTipoFav
        Left = 25
        Top = 0
        Width = 105
        Hint = 
          'Selecione o tipo de favorecido para filtrar a pesquisar no campo' +
          ' de edi'#231#227'o ao lado.'
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        StyleController = DMProjeto.esClientes
        ListFieldName = 'DESCRICAO'
        KeyFieldName = 'TIPOFAVORECIDO'
        ListSource = DMProjeto.C_TipoFavDS
        LookupKeyValue = Null
        TipoFavorecido = 1
      end
      object ckbCancelados: TTS_CheckBox
        Left = 129
        Top = 37
        Width = 249
        Style.BorderStyle = xbsNone
        Style.ButtonStyle = btsSimple
        Style.HotTrack = False
        TabOrder = 4
        Caption = '&Visualizar tamb'#233'm os cancelados'
        OnChange = ckbCanceladosChange
        DisableEdit = False
        Height = 21
      end
    end
    object DBDepositos: TTS_QDBGrid
      Left = 1
      Top = 58
      Width = 418
      Height = 184
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'DEPOSITO'
      SummaryGroups = <>
      SummarySeparator = ', '
      Align = alClient
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = ppmPadrao
      TabOrder = 1
      OnDblClick = btGravarClick
      AutoSearchColor = 6611199
      AutoSearchTextColor = clBlue
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = []
      DataSource = C_HistoricoDS
      Filter.Criteria = {00000000}
      GroupPanelColor = 15461355
      HeaderColor = 15461355
      HeaderFont.Charset = DEFAULT_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -11
      HeaderFont.Name = 'Tahoma'
      HeaderFont.Style = []
      HideFocusRect = True
      HideSelection = True
      OptionsBehavior = [edgoAutoSearch, edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoEnterThrough, edgoImmediateEditor, edgoMultiSort, edgoTabs, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      OnCustomDrawCell = DBDepositosCustomDrawCell
      TS_PermitirQuantidade = False
      TS_DescriptionCanChange = False
      TS_AppendOnEnter = False
      RowFooterNodeFont.Charset = DEFAULT_CHARSET
      RowFooterNodeFont.Color = clWindowText
      RowFooterNodeFont.Height = -11
      RowFooterNodeFont.Name = 'Tahoma'
      RowFooterNodeFont.Style = [fsBold]
      TS_AccountFooterStyle = False
      TS_HideFocusedRect = hfShowAlways
      TS_HideGroupHeader = False
      TS_AnotherColor = clWindow
      TS_ReportHeaderStyle = False
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clWindowText
      GroupNodeFont.Height = -11
      GroupNodeFont.Name = 'Tahoma'
      GroupNodeFont.Style = []
      TS_SummaryFooterFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterFont.Color = clWindowText
      TS_SummaryFooterFont.Height = -11
      TS_SummaryFooterFont.Name = 'Tahoma'
      TS_SummaryFooterFont.Style = []
      TS_SummaryFooterSelFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterSelFont.Color = clWindowText
      TS_SummaryFooterSelFont.Height = -11
      TS_SummaryFooterSelFont.Name = 'Tahoma'
      TS_SummaryFooterSelFont.Style = []
      TS_ShowSelectionColumn = True
      TS_LikeString = False
      TS_SelectionColor = 12054783
      TS_SelectionFont.Charset = DEFAULT_CHARSET
      TS_SelectionFont.Color = clWindowText
      TS_SelectionFont.Height = -11
      TS_SelectionFont.Name = 'Tahoma'
      TS_SelectionFont.Style = []
      TS_SelectedColumn = '_icSelecionado'
      TS_ID = 0
      TS_TipoDescricao = tdVenda
      TS_SummaryFooterQtdText = 'Qtd. Geral:'
      TS_SummaryFooterQtdSelectedText = 'Qtd. Selecionada:'
      object Data: TdxDBGridColumn
        Caption = 'Data'
        DisableEditor = True
        HeaderAlignment = taCenter
        Sorted = csUp
        Width = 66
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATA'
      end
      object Nome: TdxDBGridColumn
        Caption = 'Nome'
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 157
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOME'
      end
      object Conta: TdxDBGridColumn
        Caption = 'Conta'
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 121
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOMECONTA'
      end
      object Valor: TdxDBGridColumn
        Caption = 'Valor'
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 65
        BandIndex = 0
        RowIndex = 0
        FieldName = 'VALOR'
      end
      object Favorecido: TdxDBGridColumn
        Caption = 'Favorecido'
        Visible = False
        Width = 36
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FAVORECIDO'
      end
      object DBDepositosSituacao: TdxDBGridColumn
        DisableCustomizing = True
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'SITUACAO'
      end
      object DBDepositosColumn9: TdxDBGridMaskColumn
        Caption = 'Cr'#233'dito Gerado'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CREDITOGERADO'
      end
      object DBDepositosColumn10: TdxDBGridMaskColumn
        Caption = 'Cr'#233'dito Utilizado'
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CREDITOUTILIZADO'
      end
    end
  end
  inherited ppmPadrao: TTS_PopupMenu
    Left = 208
    Top = 114
    object Alterar1: TMenuItem [0]
      Caption = 'Alterar'
      Default = True
      ShortCut = 116
      OnClick = btGravarClick
    end
    object Pesquisar1: TMenuItem [1]
      Caption = 'Pesquisar'
      OnClick = btLimparClick
    end
    object Cancelar1: TMenuItem [2]
      Caption = 'Cancelar'
      OnClick = btComando2Click
    end
    object N1: TMenuItem [3]
      Caption = '-'
    end
    inherited Parametros1: TMenuItem
      Visible = False
    end
  end
  inherited FormsComponent: TFormsComponent
    BeforeClearParams = FormsComponentBeforeClearParams
    ModuloStyle = DMProjeto.msClientes
    BarEndColor = 14859922
    CaptionShadow = 14859922
    FormColor = 16116702
    Modulo = 'Clientes / Vendas'
    Caption = 'Hist'#243'rico de Recebimentos'
    OnRefresh = FormsComponentRefresh
    FirstEditField = edFavorecido
    Left = 210
    Top = 170
  end
  inherited ppmHelp: TTS_PopupMenu
    inherited Ajuda1: TMenuItem
      HelpContext = 659
    end
  end
  object C_Historico: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'P_Historico'
    Left = 76
    Top = 104
    object C_HistoricoDATA: TDateField
      FieldName = 'DATA'
    end
    object C_HistoricoNOME: TStringField
      FieldName = 'NOME'
      Size = 50
    end
    object C_HistoricoDEPOSITO: TIntegerField
      FieldName = 'DEPOSITO'
      Required = True
    end
    object C_HistoricoNOMECONTA: TStringField
      FieldName = 'NOMECONTA'
      Size = 50
    end
    object C_HistoricoVALOR: TBCDField
      FieldName = 'VALOR'
      DisplayFormat = '###,###,##0.00'
      Precision = 18
      Size = 2
    end
    object C_HistoricoFAVORECIDO: TIntegerField
      FieldName = 'FAVORECIDO'
    end
    object C_Historico_icSelecionado: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = '_icSelecionado'
    end
    object C_HistoricoSITUACAO: TStringField
      FieldName = 'SITUACAO'
      Size = 1
    end
    object C_HistoricoCREDITOGERADO: TBCDField
      FieldName = 'CREDITOGERADO'
      Precision = 18
      Size = 2
    end
    object C_HistoricoCREDITOUTILIZADO: TBCDField
      FieldName = 'CREDITOUTILIZADO'
      Precision = 18
      Size = 2
    end
  end
  object P_Historico: TDataSetProvider
    DataSet = Q_Historico
    Constraints = True
    Options = [poAllowCommandText]
    Left = 136
    Top = 108
  end
  object Q_Historico: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'Select distinct dt.Deposito, t.Titulo, d.Data, f.Nome, c.Descric' +
        'ao as NomeConta, d.Valor, d.Favorecido, d.Situacao, d.creditoger' +
        'ado, d.creditoutilizado'
      'from Depositos d '
      'inner join DepositosTitulos dt on d.deposito = dt.deposito'
      'inner join TitulosAReceber t on dt.titulo = t.ID'
      'inner join Contas c on d.Conta = c.Conta'
      'left join Favorecidos f on d.Favorecido = f.Favorecido')
    Left = 72
    Top = 156
  end
  object C_HistoricoDS: TDataSource
    DataSet = C_Historico
    Left = 132
    Top = 156
  end
end
