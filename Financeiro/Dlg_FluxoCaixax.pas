...
type
  TDlgFluxoCaixa = class(TFrmModeloCadastros)
    ...
    N4: TMenuItem;
    Imprimir1: TMenuItem;
    Q_AReceberVENCIMENTO: TDateField;
    Q_APagarVENCIMENTO: TDateField;
    C_FluxoCaixaVENCIMENTO: TDateField;
    ...
    procedure btComando1MouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure C_FluxoCaixaVENCIMENTOGetText(Sender: TField;
      var Text: String; DisplayText: Boolean);
    procedure dbTreeContasClick(Sender: TObject);
  private
...
uses DM_Projeto, funcoes, DM_Financeiro, DateUtils;
  ...
  with C_FluxoCaixa do begin
    Close;
       ...
       with Q_APagar do begin
       	close;
        SQL.Text := 'Select ''Conta  '' as Tipo, d.ID, d.Vencimento, f.Nome, (d.Valor-d.ValorPago-d.CreditoUtilizado-d.Descontos) as Valor, d.Vencimento as Data '+
                    ...
                    'where d.vencimento <= :dataate and ((d.status > 0 and d.status < 50) or d.status= -3) '+
										'UNION '+
                    'Select ''Cheque '' as Tipo,rd.IDDoc, rd.Vencimento, f.Nome, rd.Valor as Valor, rd.Vencimento as Data '+
       ...
       with Q_AReceber do begin
       	close;
				sql.text := 'Select ''Invoice'' as Tipo,t.ID, t.Vencimento, f.Nome, (t.Valor-t.ValorPago-t.CreditoUtilizado-t.Descontos) as Valor, t.Vencimento as Data '+
                    ...
                    'UNION '+
                    'Select ''Cheque '' as Tipo, dd.IDDoc, dd.Vencimento, f.Nome, dd.Valor as Valor, dd.Vencimento as Data '+
       ...
       CommandText := 'select Vencimento, '+
                   ...
                   '       Sum(MediaMeses) as MediaMeses '+
       				 		 'from FluxoCaixa '+
                   'group by Ano, Semana, Vencimento ';
       with Q_APagar do begin
       	close;
        SQL.Text := 'Select ''Conta  '' as Tipo,d.ID, d.vencimento, f.Nome, (d.Valor-d.ValorPago-d.CreditoUtilizado-d.Descontos) as Valor, d.Vencimento as Data '+
										'from duplicatasapagar d '+
...
procedure TDlgFluxoCaixa.C_FluxoCaixaVENCIMENTOGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
  inherited;
	if PorDia.Checked then
  	Text := formatdatetime(iif(Copy(UpperCase(ShortDateFormat),1,1)='D','dd/mm/yyyy','mm/dd/yyyy'),Sender.Value)
  else if PorSemana.Checked then
  	Text := formatfloat('00',WeekOfTheYear(Sender.Value))+'/'+formatdatetime('yyyy',Sender.Value)
  else
		Text := formatdatetime('mm/yyyy',Sender.Value);
end;

procedure TDlgFluxoCaixa.dbTreeContasClick(Sender: TObject);
begin
  inherited;
  nSaldo := 0;
  with C_Contas do begin
