...
type
    ...
    public
        ...
        function PagarRapido(Retirada, Duplicata, Natureza: integer; Data, Competencia: TDateTime;
            ...
            TipoPag: integer = 4; Troco: Currency = 0; EmpresaOrigem: string = 'ST'): integer;

        function PagarDuplicatas(Retirada: integer; sItens: string;
            Data: TDateTime; CreditoUtilizado, CreditoGerado: Currency;
            Historico: string; CreditoAbatido: Currency; Troco: Currency = 0; EmpresaOrigem: string = 'ST' ): integer;
        ...
        {Funcoes de estorno completo - usa as funcoes acima para as transacoes necessárias}
...
begin
    ...
    with Q_SQL do
        begin
            ...
            ParamByName('Data').asDate := DMProjeto.dDataSistema; // Data da Transação
            ...
            if EmpresaDevendo then
                begin
                    ...
                    sDescricao := 'Gerado um crédito para ' + getFavorecido(Favorecido) + ' no valor de ' + formatfloat('###,##0.00', nCredito);
                end
            else
                begin
                    ParamByName('EmpDev').asString := 'N'; //A Empresa possui um Crédito para com o Favorecido
                    sDescricao := 'Gerado um crédito para a Empresa, devedor: ' + getFavorecido(Favorecido) + ' no valor de ' + formatfloat('###,##0.00', nCredito);
                end;
            ...
            while not eof do
                begin
                    ...
                    if pos(sChave, ',' + sComp + ',') > 0 then
                        begin
                            result := true; // o funcionario ou a máquina estão vinculados a um caixa;
...
begin
    ...
    with Q_Transacoes do
        begin
            ...
            {Lançando em Operações Financeiras Detalhe (Cheques)}
            if sIDDoc <> '' then
                begin
                    close;
...
begin
    result := false;
    sMotivo := '';
    sMotivo := inputbox('Exclusão de Documento(s)', 'Motivo:', '');
        ...
        with Q_SQL do
            begin
                ...
                SQL.Text := 'Insert into Retiradas ' +
                    '     (Retirada, Natureza, Data, Competencia, Historico, Valor, Usuario, TipoRetirada, Situacao, CreditoUtilizado, CreditoGerado, CreditoAbatido, Troco, pdv, empresa, empresaorigem) ' +
                    'Values ' +
                    '     (:Ret, :Natureza, :Data, :Competencia, :Hist, :Valor, :Func, :TipoRetirada, ''N'', :CreditoUtilizado, :CreditoGerado, :CreditoAbatido, :Troco, :pdv, :empresa, :empresaorigem ) ';
                ...
                ParamByName('PDV').AsInteger := DMProjeto.nPDV;
                ParamByName('EMPRESA').asInteger := DMPRojeto.nEmpresaLogada;
                ParamByName('EMPRESAORIGEM').asstring := EmpresaOrigem;
                ExecSQL;
    ...
    try
        ...
        with Q_SQL do
            begin
                Close;
                SQL.Text := 'Insert into Retiradas ' +
                    ' (Retirada, Data, Historico, Valor, Usuario, TipoRetirada, Situacao, CreditoUtilizado, CreditoGerado, CreditoAbatido, Troco, pdv, empresa, EmpresaOrigem) ' +
                    'Values ' +
                    ' (:Ret, :Data, :Hist, :Valor, :Func, :TipoRetirada, ''N'', :CreditoUtilizado, :CreditoGerado, :CreditoAbatido, :Troco, :PDV , :empresa, :EmpresaOrigem ) ';
                ParamByName('Ret').asInteger := Retirada;
                ParamByName('Data').asDateTime := Data;
                ParamByName('Hist').asString := Historico;
                    ...
                    begin
                        ...
                        if Especie = 5 then
                            ...
                            begin
                                ...
                                ParamByName('Venc').asDateTime := Data;
                                ParamByName('Status').asInteger := 3; //  Aguardando Compensação
                                ParamByName('FormaPagamento').asInteger := FormaPagamento;
                                if FormaPagamento = 2 then
                                    ParamByName('AVista').asString := 'S'
                                else
                            ...
                            begin
                                ...
                                SQL.Text := replace(SQL.Text, ':NCh', 'null');
                                ParamByName('Venc').asDateTime := Data;
                                ParamByName('Status').asInteger := 1; // Aguandando Crédito
                                ParamByName('AVista').asString := 'N';
                                ParamByName('Taxa').AsCurrency := getEncargos(FormaPagamento, Valor);
                                ParamByName('TitularCheque').asString := TitularCheque;
                                Tipo := 6;
                        ...
                        //A critica do erro ta na função.
...
begin
    ...
    try
        ...
        on E: Exception do
            DlgMsg.ShowMsg(1, 'Transferência ' + e.message, '');
...
var sIDDoc, sDoc, sDocs, sMotivo: string;
    ...
    Vencimento: TDateTime;
begin
    {Pegando os documentos da apresentação}
    ...
    with Q_ComandoSQL do
        begin
            if TipoOperacao = 7 then
                begin {Quando a Operacao for "Depósito em Banco" - Atualizando em DepositosDoc para o Caixa Principal}
                    for i := 1 to ContaStrings(sOperacoes, ',') do
                        begin
                            try
                                ...
                                { Colocado o Try/Except para evitar que o erro seja exibido ao cancelar Depósito em Dinheiro. Obs: depósito em dinheiro não
                                    registra na tabela OperacoesFinanceirasDet. }
                            end;
                        end;
                end
            else if TipoOperacao = 9 then
                begin {Quando a Operação for Cheque Devolvido}
                    for i := 1 to ContaStrings(sOperacoes, ',') do
                        begin
                            close;
                ...
                begin {Quando a Operação for Acerto de Cheques}
                    for i := 1 to ContaStrings(sOperacoes, ',') do
                        begin
                            close;
                            SQL.Text := 'update DepositosDoc set ' +
                ...
                begin {Apresentação de Convênios}
                    for i := 1 to ContaStrings(sOperacoes, ',') do
                        begin
                            Close;
                            SQL.Text := 'Update DepositosDoc set Status = 1, AVista = ''N'', ForadaEmpresa=''N'', LocalTitulo= :L, ContaAtual = 1 ' +
                ...
                begin {Transferência entre Empresas}
                    close;
                ...
                begin {Reparcelamento}
                    ...
                    sDoc := Copy(sDoc, 1, length(sDoc) - 1);
                    {Verificando se houve pagamento para alguma parcela, caso haja poderá
                     ser cancelado}
                    if (Trim(sDoc) <> '') then
                        begin
                            with Q_ComandoSQL do
            ...
            {Operações comuns de cancelamento}

            {Excluindo de Operações Financeiras Detalhe}
            if IDDoc <> 0 then
                begin
                    close;
            ...
            {Cancelando de Operações Financeiras}
            if IDDoc = 0 then
                begin
                    close;
            ...
            begin
                ...
                for i := 2 to slItems.Count - 1 do
                    begin
                        ...
                        Close;
                        {Verificação se ja existe um cheque igual nesta operação.}
                {
                       SQL.Text := ' Delete from OperacoesFinanceirasDet '+
                        ...
                        {Lançando em Operações Financeiras Detalhes}
                        SQL.Text := 'Insert into OperacoesFinanceirasDet ' +
                ...
                AuditoriaFinanceira('select current_date,current_time, ' + IntToStr(DMProjeto.nFuncionario) + ', 11, ' +
                    '''Depositado cheque nº'' || dd.numcheque || '' de '' || dd.TitularCheque || '' para a conta ' +
...
begin
    ...
    try
        ...
        with Q_ComandoSQL do
            begin
                {Depositando os Cheques no Banco escolhido - AVista = S porque qq. cheque depositado em banco está disponível para aplicação}
                Close;
                SQL.Text :=
                    'Insert into transacoes ' +
                    '    (Conta, Data, Valor, TipoTransacao, NumCheque, FormaPagamento, IDGerador, IDDoc, ' +
                ...
                AuditoriaFinanceira('select current_date,current_time, ' + IntToStr(DMProjeto.nFuncionario) + ', ''Custodia'',' +
                    '''Depósito de Cheque em Custódia nº'' || numcheque || '' depositado na conta '' || localtitulo || '' de '' || TitularCheque,''DepositosDoc'',IDDoc,Valor,null ' +
                    'from depositosdoc ' +
                    'Where Status = 6 and Vencimento <= CURRENT_DATE');
                {Retirar taxas por cheque em custódia}
                with Q_SQL do
                    begin
                        Close;
                        ...
                        while not EOF do
                            begin
                                ...
                                Retirar(0, 0, fieldbyname('contaatual').AsInteger, DMProjeto.dDataSistema, nValorFolha * fieldbyname('qtde').AsInteger,
                                    0, 2, 'Taxa Cheques Custódia', DMProjeto.dDataSistema, '', 'S', nIDBanco);
                                Next;
                            end;
                        enablecontrols;
                        Close;
    ...
    try
        ...
        with Q_ComandoSQL do
            begin
                ...
                {Retirando o Cheque na Empresa - não disponível (Avista = N)}
    ...
    with Q_ComandoSQL do
        begin
            ...
            try
                ...
                {Retirando os Cartões da Empresa}
                Close;
                SQL.Text := 'Insert into transacoes ' +
                    '(Conta,Data,Valor,TipoTransacao,NumCheque,FormaPagamento, IDGerador, IDDoc, ' +
                    'Historico, FromBanco, Favorecido, Hora, Usuario, TipoOperacao, pdv, empresa) ' +
                ...
                SQL.Text := replace(SQL.Text, ':Historico', QuotedStr('Cartão Liquidado'));
                ExecSQL;
...
begin
    ...
    with Q_SQL2 do
        begin
            ...
            enablecontrols;
            {Mostrar os documentos que não se encontram na Empresa}
            if bMostrarDoc then
                begin
                    if sIDDocs <> '' then
                        begin
...
begin
    ...
    with Q_SQL2 do
        begin
            {Vericando se o Título já foi Cancelado}
            Close;
    ...
    with Q_SQL2 do
        begin
            ...
            {Registrando o Crédito}
            if ((nCreditoG + nValorPago) > 0) then
                begin
                    DlgMsg.ShowMsg(50, [pChar('Sera Gerado um Crédito para o Cliente no Valor de: R$ ' + FormatFloat('0.00', nValorPago - nCreditoG))]);
...
begin
    ...
    for i := 1 to ContaStrings(sDepositos, ',') do
        begin
            ...
            {Verificando se o depósito ja foi cancelado}
            with Q_ComandoSQL do
                begin
                    Close;
            ...
            if ddata < dDataFinanceiro then
                begin
                    DlgMsg.ShowMsg(50, [pChar('Operação não permitida, Transação já finalizada !')]);
                ...
                begin
                    ...
                    if bCancelouTodosDocs then
                        begin
                            ...
                            {Corrigindo ValorPago dos Títulos afetados pelo deposito}
                            Close;
                            ...
                            {Caso for proveniente de Recebimento Rápido, apaga o titulo a receber}
...
begin
    ...
    {Validação / Crítica}
    {Verificando se o Documento já está cancelado}
    with Q_ComandoSQL do
        begin
            Close;
            ...
            {Apagando Operações com este documento}
            ApagarDoc(IDDoc);
...
begin
    ...
    with Q_SQL2 do
        begin
            {Vericando se a Duplicata já foi Cancelada}
            Close;
...
begin
    ...
    for i := 1 to ContaStrings(sRetiradas, ',') do
        begin
            ...
            {Verificando se a retirada já foi cancelada}
            with Q_ComandoSQL do
                begin
                    Close;
                ...
                begin
                    ...
                    if bCancelouTodosDocs then
                        begin
                            ...
                            {Caso for proveniente de Pagamento Rápido, apaga o titulo a pagar}
    ...
    try
            ...
            begin
                ...
                if (TotalFinal > Valor) then
                    begin
                        ...
                        {Corrigindo o Valor do Depósito}
                        Close;
                ...
                {Inserindo outro documento}
                   {Agora a tela de recebimento rápido que será feito o acerto de cheque}
                   {
        ...
        on e: exception do
            begin
                ...
                // ShowMessage('Não foi possível cancelar Documento!'+#13+e.Message);
        ...
        begin
            ...
            if sDepositos <> '' then
                begin
                    ...
                    parambyname('Tipo').AsString := 'D'; {Depósitos/Recebimentos}
                    ParamByName('PDV').AsInteger := DMProjeto.nPDV;
                    ParamByName('EMPRESA').asInteger := DMPRojeto.nEmpresaLogada;
                    ExecSQL;
                    close;
            ...
            if sCheques <> '' then
                begin
                    ...
                    parambyname('Tipo').AsString := 'C'; {Compensação}
                    ParamByName('PDV').AsInteger := DMProjeto.nPDV;
                    ParamByName('EMPRESA').asInteger := DMPRojeto.nEmpresaLogada;
                    ExecSQL;
            ...
            if sCartoes <> '' then begin
             ...
             sql.text := 'insert into OperacoesFinanceirasDet ( OpDoc, Operacao, Obs, TipoConciliacao ) '+
              ...
              parambyname('Tipo').AsString 			:= 'E'; // Cartão
              ExecSQL;
...
begin
    ...
    with Q_ComandoSQL do
        begin
            ...
            {Verificando se este depósito foi utilizado para pagar apenas este título}
            if sDep <> '' then
                begin
                    close;
                    sql.text := 'Select Titulo From DepositosTitulos ' +
            ...
            if sDep <> '' then
                begin
                    ...
                    SQL.Text := 'Delete from DepositosDoc ' +
                        'Where Deposito IN (' + sDep + ')';
                    ExecSQL;
                    {Delete from Transações}
                    Close;
                    SQL.Text := 'Delete from Transacoes ' +
                        'Where IDGerador IN (' + sDep + ')';
                    ExecSQL;
                    {Delete from TransaçõesCréditos}
                    Close;
