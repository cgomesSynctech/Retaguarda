inherited RptChequesPorOperacao: TRptChequesPorOperacao
  Left = 195
  Top = 105
  Width = 737
  PixelsPerInch = 96
  ...
  inherited pnTitulo: TTS_MaxPanel
    ...
    inherited lbCaption: TdxfLabel
      Width = 215
      Caption = 'Cheques Por Opera'#231#227'o'
    end
  ...
  inherited Q_Consulta: TIBQuery
    ...
    SQL.Strings = (
      ...
      's.numero, sd.descricao as descstatus'
      'From DepositosDoc dd'
      'inner join Depositos d on dd.deposito = d.deposito'
      'left join depositostitulos dt on d.deposito = dt.deposito'
      'left join titulosareceber tr on dt.titulo = tr.id'
      'left join saidas s on tr.venda = s.saida'
      ...
      'left join statusdoc sd on dd.status = sd.status'
      'Where dd.vencimento >= :dataI and dd.vencimento <= :dataF and  '
      'dd.formapagamento in (2,3) ')
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'dataI'
