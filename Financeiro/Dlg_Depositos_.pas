...
uses
  ...
  teForm, TS_DBImageEdit, TS_DBMaskEdit, BTOdeum, Placemnt, TS_Edit;
...
type
  ...
  private
    ...
    procedure AbrirTitulos( sItensHist : string = ''; sFavorecidos : string = ''; bSomenteSelecionadas : boolean = false; sSaidas : string = '' );
...
begin
  ...
  with DMProjeto do begin
      ...
      if QtdParametrosForm > 3 then
      	sSaidas := getParametrosForm(3)
      else
      	sSaidas := '';
  end;
  {Abrindo a Consulta}
  AbrirTitulos( sTitulosSelecionados, '', (sTitulosSelecionados <> '') or (sSaidas <> ''), sSaidas );
  {Selecionando as Parcelas}
  if (C_Titulos.Active) and (C_Titulos.RecordCount > 0) and ((sTitulosSelecionados<>'') or (sSaidas<>'')) then begin
  ...
  if (Especie <> 0) and (C_DepositosCreditoUtilizado.Value < DBTitulos.TotalSelecionado('Pagamento')) then begin
    ...
    if C_Titulos.Active and (DBTitulos.TotalSelecionado('Pagamento') < C_DepositosCreditoCliente.Value) then
  ...
  with C_Titulos do begin
    ...
    CommandText := 'Select t.ID, t.Titulo, t.NotaFiscal, t.Vencimento, '+
                   ...
                   'left join Saidas s on t.venda = s.saida '+
                	 'Where ';
    if not bSomenteSelecionadas then begin
    ...
    if (dfNotaPedido.Text <> '') then begin
...
begin
  ...
  if (bAlteraTroco) and (bOperacaoPadraoTroco) then begin
  ...
  C_DepositosValor.Value := Recebido -
...
begin
  ...
  if (FormaPagamento <> ValorParcelas) then begin
    DlgMsg.ShowMsg( 8047 );
    Exit;
  end
  else if (FormaPagamento > ValorParcelas) and (ValorParcelas>ValorFaltaReceber) then begin
...
begin
  ...
  if C_Cheques.State in [dsEdit,dsInsert] then begin
  ...
  {Atualizando dados dos cheques}
  if C_ChequesEletronico.State in [dsEdit,dsInsert] then begin
  ...
  if C_Cartoes.State in [dsEdit,dsInsert] then begin
  ...
  if (C_DepositosCliente.Isnull) or (dfFavorecido.ID = 0) then begin
	...
	if C_DepositosData.Value > DMProjeto.dDataSistema then begin
    DlgMsg.ShowMsg( 901 );
    exit;
  end;
  ...
  {A data de pagamento não pode ser muito antiga}
	...
	if (C_DepositosConta.IsNull) or (C_DepositosConta.AsInteger = 0) then begin
...
{  if (not C_Titulos.Active) or (DBTitulos.Selecionados = '') then begin
  ...
  for i:=1 to ContaStrings(sSelecionados,',') do begin
  	if (C_TitulosPagamento.IsNull) or ((C_TitulosPagamento.AsCurrency = 0) and (C_TitulosDescontos.AsCurrency=0)) then begin
    	DlgMsg.ShowMsg(929, ['o valor do Recebimento'] );
      exit;
    end;
  	if (C_TitulosPagamento.AsCurrency < 0) then begin
    	DlgMsg.ShowMsg(929, ['o valor do Recebimento. (Não pode ser Negativo)'] );
      exit;
    end;
   	if (C_TitulosDescontos.IsNull) then begin
    	DlgMsg.ShowMsg(929, ['o valor do Desconto não pode está vazio.'] );
      exit;
    end;
   	if (C_TitulosJuros.IsNull) then begin
    	DlgMsg.ShowMsg(929, ['o valor dos Juros não pode está vazio.'] );
      exit;
    end;
  	if (C_TitulosDescontos.AsCurrency < 0) then begin
    	DlgMsg.ShowMsg(929, ['o valor do Desconto. (Não pode ser Negativo)'] );
      exit;
    end;
  	if (C_TitulosJuros.AsCurrency < 0) then begin
  ...
  if (DBCartoes.TotalGeral('Valor') > 0) and (C_DepositosValorParcela.Value < DMFinanceiro.getParcelaMinima(C_CartoesCartao.Value)) then begin
	...
	if (C_DepositosCreditoUtilizado.Value > 0) and (C_DepositosCreditoUtilizado.Value > C_DepositosCreditoCliente.Value) then begin
  	DlgMsg.ShowMsg( 519 );
	...
	if (C_DepositosCreditoUtilizado.Value > 0) and
  	 (DBTitulos.TotalSelecionado('Pagamento')+DBTitulos.TotalSelecionado('Juros')=0) then begin
  	DlgMsg.ShowMsg( 507 );
    if dfUtilizarCredito.CanFocus then
    	dfUtilizarCredito.SetFocus;
    Exit;
	...
	if (C_DepositosOutroValor.Value > 0) and (DMFinanceiro.getEspecieID(Integer(cmbOutraForma.LookUpKeyValue))=40) and (cmbContaOutro.Text = '') then begin
  	DlgMsg.ShowMsg( 921 );
    if cmbOutraForma.CanFocus then
    	cmbOutraForma.SetFocus;
    Exit;
  ...
  if (DBTitulos.TotalSelecionado('Pagamento')+DBTitulos.TotalSelecionado('Juros')>0) and
     ...
     (C_DepositosCreditoUtilizado.Value=0) then begin
  ...
  if (getTotalEmFormaPagamento(false) = 0) and (DBTitulos.ValorSelecionado('Descontos')<=0) then begin
    DlgMsg.ShowMsg(524);
    Exit;
  end;

	if (C_DepositosTroco.Value > 0) and ((C_DepositosTroco.Value) >= (C_DepositosValor.Value+C_DepositosTroco.Value)) then begin
  	DlgMsg.ShowMsg( 955 );
    Exit;
  end;
  ...
  if (C_Titulos.Active) and (DBTitulos.Selecionados <> '') and (TotalFormaPagamento <> getTotalEmTitulos) then begin
    C_Titulos.Edit;
    ...
    if not Corrigir(TotalFormaPagamento,getTotalEmTitulos, DBTitulos.TotalSelecionado('ValorAReceber')) then
     	exit;
    if TotalFormaPagamento > getTotalEmTitulos then begin
...
//        if (C_TitulosPagamento.Value) = C_TitulosValorAReceber.Value then
      ...
      {Pagando a mais e mais de uma parcela selecionada}
        ...
        for i:=1 to ContaStrings(sSelecionados,',') do begin
            ...
            else if i = ContaStrings(sSelecionados,',') then begin
               C_Titulos.Edit;
    ...
    {Pagando a menos}
      ...
      if DBTitulos.QuantidadeSelecionada = 1 then begin
        ...
        for i:=1 to ContaStrings(sSelecionados,',') do begin
            if nTotalSel <= 0 then
               DBTitulos.RemoverLinha
            else if nTotalSel < C_TitulosPagamento.Value then begin
  ...
  if (C_DepositosCreditoGerado.Value > 0) and (C_DepositosCliente.Value = 1) then begin
  	DlgMsg.ShowMsg(804);
    Exit;
  end;
  ...
  if DMProjeto.Parametro('Confirmacao') <> 'N' then begin
    bMensa := false;
    ...
    if bMensa then begin
      if (C_Titulos.Active) and (DBTitulos.QuantidadeSelecionada = 1) and (ValorAnterior<>0) then begin
        C_Titulos.Edit;
  ...
  try
    ...
    if DepositoHist <> 0 then begin
      ...
      {Verificando se o crédito já foi utilizado}
  		if nCreditoGerado > 0 then begin
  			if (nCreditoGerado-C_DepositosCreditoGerado.Value) > DMFinanceiro.CreditoFavorecido(C_DepositosCliente.Value, true) then begin
      ...
      i := 10;
    	while not C_Cheques.EOF do begin
       	...
       	DMFinanceiro.ReceberDoc(aIDDoc[i], 1, Deposito, 2, C_ChequesValor.Value,
                             		...
                             		C_ChequesBanco.Value, C_DepositosNome.Value, 0, 0, C_ChequesContaReceber.Value );
  ...
  if clbForma.Checked[nIndex] then begin
      ...
      {Cartão de Crédito}
        ...
        C_CartoesValorParcela.Value	:= nValor;
       	C_Cartoes.Post;
       	getTotalEmFormaPagamento;
      end else if SeparaStrings(clbForma.TagStr,',',nIndex+1) = '6' then
      	C_Depositos.FieldByName(sCampo).Value := C_DepositosCreditoCliente.Value
			else if not (StrToInt(SeparaStrings(clbForma.TagStr,',',nIndex+1)) in [1,2]) then begin
    		 C_Depositos.FieldByName(sCampo).Value := nValor;
         if nIndex = 3 then begin //Crédito Utilizado
...
//  end else
  if nPagamento > C_TitulosValorAReceber.AsCurrency then begin
    C_TitulosJuros.OnChange := nil;
...
begin
  with DMFinanceiro do begin
    ...
    for i:=1 to ContaStrings(sTitulosHist,',') do begin
    ...
    clbForma.Enabled := (C_Titulos.Active) and (C_Titulos.RecordCount > 0) and (DBTitulos.Selecionados <> '');
   	if sTitulosHist <> '' then begin
       for j:=1 to ContaStrings(sTitulosHist,',') do begin
       		with Q_SQL2 do begin
            	...
            	if C_Titulos.Locate('ID',StrToInt(SeparaStrings(sTitulosHist,',',j)),[]) then begin
                DBTitulos.SelecionarLinha;
            		C_Titulos.Edit;
                C_TitulosDescontos.OnChange := nil;
                C_TitulosPagamento.OnChange := nil;
       ...
       with Q_SQL2 do begin
          	...
          	SQL.Text := 'select dd.IDDoc, dd.Deposito, dd.FormaPagamento, fp.Especie, '+
                        ...
                        'dd.ContaReceber, dd.Tipo, dd.NumeroCartao, dd.Validade, dd.obs '+
            ...
            j := 70;
          	while not EOF do begin
                ...
                inc(i);
            	end else if fieldbyname('Especie').AsInteger = 2 then begin // Cheques Eletronico
                ...
                inc(k);
            	end else if fieldbyname('Especie').AsInteger = 3 then begin // Cartao de Crédito
            	...
            	end else if fieldbyname('Especie').AsInteger = 20 then begin {Deposito Bancario}
            	...
            	end else begin {Outras Formas de Pagamento}
...
begin
  ...
  if C_TitulosDescontos.Value > 0 then begin
    ...
    if nDesconto > DMProjeto.nMaxDescontoFunc then
      if not DMProjeto.DlgAutorizacao.ExecuteX( self.name, 'Perdescmax' ) then
...
begin
  ...
  if DMFinanceiro.C_ContasCxBcTipoConta.Value = 2 then begin
...
begin
  ...
  if bSelected then begin
    {Setando o Valor a Pagar = FaltaReceber}
    C_TitulosJuros.Value 					:= 0;
    C_TitulosDescontos.Value      := 0;
    if C_TitulosPagamento.AsCurrency = 0 then
    	C_TitulosPagamento.Value := C_TitulosValorAReceber.Value;
    //if C_DepositosData.Value <> C_TitulosCompetencia.Value then
    	if C_DepositosData.Value <= C_TitulosDataAntecipacao.Value then begin
        ...
        //DBTitulosDescontos.Font.Color := clRed;
        //DBTitulosDescontos.Font.Style := [fsBold];
    	end;
    if (C_TitulosDataAtrasado.Value < C_DepositosData.Value) and (JurosParametrizado > 0) then
...
//	DBTitulos.TS_SelectedColumn := 'Pagamento';
  ...
  if DBTitulos.Selecionados = '' then begin
    for i:=0 to clbForma.Items.Count-1 do
      clbForma.Checked[i] := False;
    clbForma.Enabled := False;
    pgForma.Visible := False;
...
begin
  ...
  if not bOperacaoPadraoTroco then begin
...
//    shpTroco.Visible := false;
  ...
  if DMProjeto.Parametro('LayoutRecebimento') <> 'F' then begin
  	pnTitulos.Top   	:= 50;
...
begin
  inherited;
   if UpperCase(Column.FieldName) = 'DESCBANCO' then begin
  	Allow := false;
...
begin
  inherited;
    ...
    if (C_TitulosVenda.Value <> 0) and (not C_TitulosVenda.IsNull) then begin
...
begin
  inherited;
   if DMFinanceiro.ExisteChequeDevolvido(dfFavorecido.ID) then begin
   		if DlgMsg.ShowMsg(961) = 100 then begin
   ...
   if not bParametro then
     ...
     if not C_Titulos.IsEmpty then begin
    	...
    	if C_DepositosCreditoCliente.Value > 0 then begin
      ...
      if C_Titulos.IsEmpty then begin
        dfFavorecido.SetFocus;
...
begin
  ...
  with DMProjeto do begin
    ...
    if nBotao = 1 then begin
  		if ExisteParametrosForm then
      	getHistorico( DMProjeto.getParametrosForm(0) )
    end else if (DMProjeto.QtdParametrosForm >= 2) and (pos(IntToStr(DepositoHist),DMProjeto.getParametrosForm(1))>0) then
    	Novo;
    LimparParametrosForm;
  end;
end;
...
begin
  inherited;
	if key = '%' then begin
  	if DBTitulos.TS_SelectedColumn = 'Descontos' then begin
      ...
      if C_TitulosDescontos.Value < 0 then
      	C_TitulosDescontos.Value := 0;
      Key := #0;
    end else if DBTitulos.TS_SelectedColumn = 'Juros' then begin
   		if C_Titulos.State in [dsEdit,dsInsert] then
   			C_Titulos.Post;
    	C_Titulos.Edit;
    	C_TitulosJuros.Value := DMFinanceiro.CalculaJuros(C_TitulosPagamento.Value, JurosParametrizado,
