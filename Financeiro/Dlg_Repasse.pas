...
uses
  ...
  TS_CheckBox, ppDB, ppDBPipe, Placemnt, BTOdeum;

type
  TDlgRepasse = class(TFrmModeloCadastros)
    Q_Cheques: TIBQuery;
...
function TDlgRepasse.Validacoes;
...
procedure TDlgRepasse.RepassarTodos1Click(Sender: TObject);
begin
  inherited;
...
procedure TDlgRepasse.NoRepassar1Click(Sender: TObject);
begin
  inherited;
  with Q_Cheques do begin
    disablecontrols;
...
procedure TDlgRepasse.edFicareiChange(Sender: TObject);
begin
  inherited;
	edRepassar.Value := nSaldoDin - edFicarei.Value;
end;

procedure TDlgRepasse.edRepassarChange(Sender: TObject);
begin
  inherited;
  edFicarei.Value := nSaldoDin - edRepassar.Value;
end;

procedure TDlgRepasse.btGravarClick(Sender: TObject);
var sIDDoc : string;
begin
  inherited;
  ...
  if not Validacoes(true) then
   ...
   try
    ...
    Repasse( edRepassar.Value, DBChequesParaRepassar.TotalGeral('Valor'), sIDDoc);
...
procedure TDlgRepasse.DBChequesDragDrop(Sender, Source: TObject; X,
  Y: Integer);
var
...
procedure TDlgRepasse.DBChequesDragOver(Sender, Source: TObject; X,
...
procedure TDlgRepasse.Carregar;
begin
  lbCaixa.Caption := DMFinanceiro.Conta_Nome( DMFinanceiro.nContaPadrao );
  lbCaixa2.Caption:= lbCaixa.Caption;
	with C_Cheques do begin
  	close;
    fetchParams;
//    params.parambyname('data').AsDateTime := DMFinanceiro.Caixa_Movimentocorrente;
    params.parambyname('conta').AsInteger := DMFinanceiro.nContaPadrao;
    open;
  end;
  C_ChequesParaRepassar.EmptyDataSet;
  nSaldoCheque:= DBCheques.TotalGeral('Valor');
  nSaldoDin := DMFinanceiro.SaldoConta( DMFinanceiro.nContaPadrao, DMProjeto.dDataSistema, '5');
...
procedure TDlgRepasse.FormsComponentBeforeClearParams(Sender: TObject);
begin
  inherited;
...
procedure TDlgRepasse.Repassar( bTodos : boolean );
...
procedure TDlgRepasse.NaoRepassar( bTodos : boolean );
var sSelecionados : string;
		i : integer;
begin
  inherited;
...
procedure TDlgRepasse.TS_SpeedButton1Click(Sender: TObject);
begin
	Repassar( false );
end;

procedure TDlgRepasse.TS_SpeedButton2Click(Sender: TObject);
begin
	NaoRepassar( false );
end;

procedure TDlgRepasse.TS_SpeedButton3Click(Sender: TObject);
begin
  inherited;
	Repassar( true );
end;

procedure TDlgRepasse.TS_SpeedButton4Click(Sender: TObject);
begin
  inherited;
	NaoRepassar( true );
end;

procedure TDlgRepasse.DBChequesDblClick(Sender: TObject);
begin
  inherited;
	Repassar( false );
end;

procedure TDlgRepasse.DBChequesParaRepassarDblClick(Sender: TObject);
begin
  inherited;
	NaoRepassar( false );
end;

procedure TDlgRepasse.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  // Adriano
...
procedure TDlgRepasse.FormCreate(Sender: TObject);
begin
  inherited;
  if not DMProjeto.DlgAutorizacao.ExecuteX( self.name,'CanRepassa') then
     Exit;
...
procedure TDlgRepasse.btContabilidadeClick(Sender: TObject);
begin
  inherited;
...
procedure TDlgRepasse.DBChequesMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TDlgRepasse.DBChequesParaRepassarMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TDlgRepasse.btComando1Click(Sender: TObject);
begin
  inherited;
 if not DMProjeto.DlgAutorizacao.ExecuteX( self.name, 'HistRepasse' ) then
   exit;
   with DMProjeto do begin
    SetParametrosForm([20,'Histórico de Repasses '+DMFinanceiro.Conta_Nome(DMFinanceiro.nContaPadrao),false]);
...
procedure TDlgRepasse.ppLabel2GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := lbCaixa.Caption;
end;

procedure TDlgRepasse.ppLabel3GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := DateToStr(DMProjeto.dDataSistema);
end;

procedure TDlgRepasse.ppLabel6GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := formatfloat('###,##0.00', nSaldoDin);
end;

procedure TDlgRepasse.ppLabel26GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := formatfloat('###,##0.00', nSaldoCheque);
end;

procedure TDlgRepasse.ppLabel7GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := formatfloat('###,##0.00', edRepassar.Value );
end;

procedure TDlgRepasse.ppLabel10GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := lbChequesParaRepassar.Caption;
end;

procedure TDlgRepasse.ppLabel12GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := formatfloat('###,##0.00', edFicarei.Value );
end;

procedure TDlgRepasse.ppLabel28GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := lbChequesNoCaixa.Caption;
end;

procedure TDlgRepasse.ppLabel20GetText(Sender: TObject; var Text: String);
begin
  inherited;
	Text := DMProjeto.sLoginName;
end;

procedure TDlgRepasse.FormsComponentRefresh(Sender: TObject);
begin
  inherited;
