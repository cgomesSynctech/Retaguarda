...
uses
  ...
  FormsComponent, jpeg, TS_Image, dxfLabel, TS_MaxPanel, TS_LookupComboBox,
  dxBar, teCtrls, TS_EffectsPanel, TS_PopupEdit;
...
type
  TDlgExtratoContas = class(TFrmModeloCadastros)
    ...
    procedure dbtDiaFormat(Sender: TObject;
  ...
  var Text: String);
    ...
    procedure ppDBText25Format(Sender: TObject; DisplayFormat: String;
      DataType: TppDataType; Value: Variant; var Text: String);
...
begin
	nSaldoAnt   := DMFinanceiro.SaldoAnteriorConta( nConta, dtDataI.date );
  Barra.Position := Barra.Position + 1;
  nSaldoAtual := DMFinanceiro.SaldoFinanceiro( nConta );
  Barra.Position := Barra.Position + 1;
  nSaldoConta := DMFinanceiro.Conta_Saldo( IntToStr(nConta),'');
  Barra.Position := Barra.Position + 1;
  nSaldoPre   := DMFinanceiro.SaldoChequePre( nConta );
  if nSaldoPre > 0 then begin
...
begin
  inherited;
	if (rdTransacional.Checked) and (clbTipoTransacao.Selecionados='') then begin
  	DlgMsg.ShowMsg(585);
    Exit;
  end;
  Barra.Position 	:= 0;
  ...
  regLimiteCredito.Visible 			:= nLimiteCredito > 0;
	ChequesACompensar.Visible 		:= (ckbACompensar.Checked);
  ChequesADepositar.Visible			:= (ckbADepositar.Checked) and (rdCronologico.Checked);
  SubChequesACompensar.Visible 	:= (ckbACompensar.Checked);
  SubChequesADepositar.Visible 	:= (ckbADepositar.Checked) and (rdTransacional.Checked);
  if ckbADepositar.Checked then begin
  	with Q_ChqDep do begin
  		close;
    	open;
      if recordcount = 0 then begin
     		ChequesADepositar.Visible := false;
      	SubChequesADepositar.Visible := false;
  ...
  Barra.Position := Barra.Position + 1;
  if ckbCartoesAReceber.Checked then begin
  	with Q_CartoesAReceber do begin
  		close;
    	parambyname('Conta').AsInteger := nConta;
    	open;
    end;
  end else begin
  	subCartoesAReceber.Visible := false;
    subCartoesAReceber2.Visible := false;
  end;
  Barra.Position := Barra.Position + 1;
  if ckbACompensar.Checked then begin
  	with Q_ChqComp do begin
  		close;
    	parambyname('conta').asInteger := nConta;
    	open;
    	if recordcount = 0 then begin
    		ChequesACompensar.Visible := false;
      	SubChequesACompensar.Visible := false;
        subCartoesAReceber.ShiftRelativeTo := regLimiteCredito;
    	end else begin
      	subCartoesAReceber.ShiftRelativeTo := ChequesACompensar;
        subCartoesAReceber2.ShiftRelativeTo := SubChequesACompensar;
      end;
    end;
  end else begin
    ChequesACompensar.Visible := false;
...
procedure TDlgExtratoContas.FormShow(Sender: TObject);
var i : integer;
begin
  inherited;
  dtDataI.Date := DMProjeto.dDataSistema - 30;
  dtDataF.Date := DMProjeto.dDataSistema;
  with DMFinanceiro.C_ContasCxBc do begin
    Close;
    Open;
...
procedure TDlgExtratoContas.dbtDiaFormat(Sender: TObject;
  DisplayFormat: String; DataType: TppDataType; Value: Variant;
  var Text: String);
begin
  inherited;
  Text := formatdatetime(copy(DMProjeto.sDateFormat,1,5),Value);
  if Value <= DMProjeto.dDataSistema then
...
begin
  inherited;
  Text := formatfloat('###,###,##0.00', nSaldoConta );
  if (nSaldoConta) < 0 then
  	TppLabel(Sender).font.color := clRed
  else
  	TppLabel(Sender).font.color := clBlue;
end;
...
procedure TDlgExtratoContas.ppDBText22Print(Sender: TObject);
begin
  inherited;
...
procedure TDlgExtratoContas.ppDBText41Format(Sender: TObject;
  DisplayFormat: String; DataType: TppDataType; Value: Variant;
  var Text: String);
begin
  inherited;
...
begin
  inherited;
	DMProjeto.CriarForm('DlgDepositosCheques',self,true);
end;

procedure TDlgExtratoContas.ppHeaderBand1BeforePrint(Sender: TObject);
begin
  inherited;
  ppHeaderBand1.Visible := not Q_Cronologico.EOF;
end;

procedure TDlgExtratoContas.ppDBText25Format(Sender: TObject;
  DisplayFormat: String; DataType: TppDataType; Value: Variant;
  var Text: String);
begin
  inherited;
...
begin
  inherited;
	if pos(LastDataObject.TableName,'Depositos,DepositosDoc,OperacoesFinanceiras,Retiradas,RetiradasDoc')>0 then begin
  	if (rdCronologico.Checked) and (Q_Cronologico.Active) then begin
      ...
      CalculaSaldos;
      ppExtratoCronologico.Publisher.Publish;
    end else if Q_Transacional.Active then begin
