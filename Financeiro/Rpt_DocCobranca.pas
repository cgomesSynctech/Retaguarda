...
begin
    ...
    with C_Titulos do
        begin
            ...
            ppMemoAceiteTitulo.Lines.Text := 'RECONHEÇO(EMOS) A EXATIDÃO DESTA DUPLICATA DE VENDA MERCANTIL NA ' +
                'IMPORTÂNCIA ACIMA PAGAREI(EMOS) À ' + DMProjeto.sNomeEmpresa + ', ' +
                'OU A SUA ORDEM NA PRAÇA E VENCIMENTO ACIMA INDICADO.';
                    ...
                    if (sDuplicataImp = 'I') then
                        ...
                        begin
                            ...
                            if (sDuplicataImp = 'S') then
                                begin
                                    // Só imprime o lado esquerdo do cabeçalho na duplicata...
                                    //DMProjeto.ImprimirCabecalho( rbrgCabecalho, '', True, True );
                                    DMProjeto.Interpreta(sCabEsquerdo);
                                    ppRichText.RichText := sCabEsquerdo;

                                    ppMemoAceite.Lines.Text := 'RECONHEÇO(EMOS) A EXATIDÃO DESTA DUPLICATA DE VENDA MERCANTIL NA ' +
                                        'IMPORTÂNCIA ACIMA PAGAREI(EMOS) À ' + DMProjeto.sNomeEmpresa + ', ' +
                                        'OU A SUA ORDEM NA PRAÇA E VENCIMENTO ACIMA INDICADO.';
                                    if sImpressaoPara = 'C' then
                                        begin
                        ...
                        begin
                            ...
                            while not C_Titulos.EOF do
                                begin
                                    ...
                                    { Felipe -  Verifica se é boleto Bradesco. Se for, pega o valor do campo DIGITONOSSONUMERO da tabela
                        ...
                        begin
                            ...
                            while not C_Titulos.EOF do
                                begin
                                    ...
                                    //                gbBoleto.Sacado.contabancaria.NomeCliente := 'synctech soluções';
                                    //                gbBoleto.Sacado.contabancaria.NumeroConta := '00004975';

                                                //identificador do endereço do Sacado
...
begin
    TZRLabel(Sender).Caption := ' No dia ' + C_TitulosVENCIMENTO.AsString + ' pagarei' +
        ' por esta única via de NOTA PROMISSORIA a firma ' + DMProjeto.sNomeEmpresa +
        ' a importância de R$ ' + FormatFloat('#,###,###,##0.00', C_TitulosVALOR.AsCurrency) +
