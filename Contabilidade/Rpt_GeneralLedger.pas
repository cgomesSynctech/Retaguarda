...
type
  ...
  private
    ...
    function Hierarquia(nConta: integer): string;
    procedure Ordenacao;
...
begin
  ...
  try
    ...
    while not C_Consulta.eof do begin
      if (Tipo <> 3) {and (pos(C_ConsultaTipo.value,'APQ') > 0)} and (nConta <> C_ConsultaConta.value) then begin
        nSaldoAnterior := DMProjeto.SaldoContaAtual(C_ConsultaConta.value,0,trunc(dtDataI.date)-1);
	      nConta := C_ConsultaConta.value;
      end;
      C_Consulta.edit;
...
//      Q_Consulta.SQL.Add('and (c.conta = '+IntToStr(Conta)+' or c.contapai = '+IntToStr(Conta)+') ')
      sWhere := sWhere + ' and (c.conta = '+IntToStr(Conta)+' or c.contapai in ('+Hierarquia(Conta)+')) ';
    end
    else if sCondicaoTipo <> '' then
      sWhere := sWhere + ' and t.tipo in ('+sCondicaoTipo+') '; //Q_Consulta.SQL.Add('and t.tipo in ('+sCondicaoTipo+') ');
...
function TRptGeneralLedger.Hierarquia(nConta: integer): string;
var xContaPai,sContas: string;
    xConta: integer;
begin
    DMProjeto.Q_Contabil.close;
    DMProjeto.Q_Contabil.SQL.text := 'select conta,contapai from contas where desativado = ''N'' and sintetico = ''S'' and contapai = :contapai';
    DMProjeto.Q_Contabil.params[0].asinteger := nConta;
    DMProjeto.Q_Contabil.open;
    sContas := IntToStr(nConta);
    while not DMProjeto.Q_Contabil.eof do begin
      xContaPai := DMProjeto.Q_Contabil.FieldByName('conta').asstring;
      if xContaPai <> '' then
        sContas := sContas +','+xContaPai;
