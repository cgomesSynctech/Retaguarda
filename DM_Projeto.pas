...
begin
    // if nCodigo <= 0 then ( teste modificado porque os ID do Força VEnda são negativos ... então sempre que pesquisava um cliente
    //que tinha sido cadastrado pelo força venda, não encontava porque o ID fica negativo 
       if nCodigo = 0 then
        DlgMsg.ShowMsg(947)
    else
        begin
...
var IdSaida, SaidaItem, Qtde, i, TipoPadrao, TabelaPadrao, Item, TipoITem, TabelaPReco,
    ...
    sItem, BaixaEstoque, BaixaEstoqueFiscal, Sigla, sComissao, sCFOP, sPossuiICMS, sPossuiISS, sPossuiIPI,
        sSERIE, sModeloDOC, Descricao, Unidade, SituacaoECF, CST, sN, sUFDestino, comando, GeraFinanceiro, sErro, statuspadraomovimento: string;
begin
    {Eliminação do DMECF.}
    try
        Imposto := 0;
        ...
        with Q_SQL do
            begin
                ...
                SQL.Text := ' select baixaestoque, baixaestoqueFiscal, tipopadrao, tabelapadrao, siglainicial, calccomissao, ' +
                    ' cfopassociado, possuiicms, possuiiss, possuiipi, serie, modelodoc, gerafinanceiro, almoxpadrao, statuspadrao ' +
                    ' from tiposmovimento ' +
                ...
                statuspadraomovimento := FieldByName('statuspadrao').AsString;
                Close;
            end;
        ...
        with Q_GerarSaida do
            begin
                ...
                SQL.Text := 'insert into Saidas (Saida, Numero, Data, Favorecido, Obs, Vendedor, ' +
                    ...
                    'Volumes, UFDestino, TipoFrete, ValorOutrasICMS, NUMEROECF, NUMEROSERIEECF, CCF, CRZ, empresa,MD5, status ) ' +
                    'values ' +
                ...
                ParamByname('status').AsString := statuspadraomovimento;
