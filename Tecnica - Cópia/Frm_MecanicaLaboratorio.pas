...
begin
    ...
    if DMMecanicaLaboratorio.C_TabelaEMBALAGEMPAPELAO.Value = 'S' then
        rb_caixapapelao.Checked := true;

    if DMMecanicaLaboratorio.C_TabelaEMBALAGEMPLASTICO.Value = 'S' then
        rb_sacoplastico.Checked := true;

    if DMMecanicaLaboratorio.C_TabelaSEMEMBALAGEM.Value = 'S' then
        rb_semembalagem.Checked := true;

    if DMMecanicaLaboratorio.C_TabelaAVARIA.Value = 'S' then
        rb_ava_sim.Checked := True
    else
        rb_ava_nao.Checked := True;

    if DMMecanicaLaboratorio.C_TabelaCOMACESSORIO.Value = 'S' then
        rb_ace_sim.Checked := True
    else
        rb_ace_nao.Checked := True;

    if DMMecanicaLaboratorio.C_TabelaESTADOPRODMANCHADO.Value = 'S' then
        cb_manchado.Checked := true
    else
        cb_manchado.Checked := false;

    if DMMecanicaLaboratorio.C_TabelaESTADOPRODARRANHADO.Value = 'S' then
        cb_arranhadoriscado.Checked := true
    else
        cb_arranhadoriscado.Checked := False;

    if DMMecanicaLaboratorio.C_TabelaESTADOPRODSUJO.Value = 'S' then
        cb_sujo.Checked := true
    else
        cb_sujo.Checked := False;

    if DMMecanicaLaboratorio.C_TabelaESTADOPRODCABOELETRICO.Value = 'S' then
        cb_caboeletrico.Checked := true
    else
        cb_caboeletrico.Checked := False;

    if DMMecanicaLaboratorio.C_TabelaESTADOPRODQUEBRADO.Value = 'S' then
