...
type
  TDMMecanica = class(TDMManutencao)
...
var
  DMMecanica: TDMMecanica;

implementation

uses DM_Projeto, Funcoes, Frm_Mecanica;

{$R *.dfm}

Procedure TDMMecanica.AtualizaAcessorios;
Begin
...
procedure TDMMecanica.GerarSerial;
Begin
  if not(C_Tabela.State in [dsEdit,dsInsert]) then
    C_Tabela.Edit;
...
Procedure TDMMecanica.PegarOSNumero;
Begin
...
procedure TDMMecanica.C_TabelaNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TDMMecanica.DataModuleCreate(Sender: TObject);
begin
  inherited;
  DMMecanica:=Self;
end;

procedure TDMMecanica.C_ServicosDetCalcFields(
  DataSet: TDataSet);
begin
  inherited;
...
procedure TDMMecanica.DMComponentGravar1_Iniciar(
...
procedure TDMMecanica.DMComponentPesquisar1_Iniciar(
...
procedure TDMMecanica.DMComponentModoInclusao1_Iniciar(
  Sender: TObject);
begin
  inherited;
  C_Equip.Close;
...
procedure TDMMecanica.DMComponentPesquisar4_Dados(
  Sender: TObject);
begin
  inherited;
  C_Equip.Close;
