...
uses
  ...
  dxExEdtr, dxEdLib, TS_CheckBox, Variants, BTOdeum, Placemnt;

type
  TFrmEstimatesAprovadas = class(TFrmModeloCadastros)
    TS_Panel2: TTS_Panel;
    TS_Shape1: TTS_Shape;
    TS_Shape3: TTS_Shape;
    TS_Label1: TTS_Label;
    ...
    dbgEstimatesSTATUS: TdxDBGridImageColumn;
    C_EstimatesDias: TIntegerField;
    dbgEstimatesDias: TdxDBGridColumn;
    C_EstimatesFONE1: TStringField;
    dbgEstimatesFone: TdxDBGridColumn;
    ...
    procedure C_EstimatesAfterScroll(DataSet: TDataSet);
  private
...
var
  FrmEstimatesAprovadas: TFrmEstimatesAprovadas;
...
procedure TFrmEstimatesAprovadas.FormShow(Sender: TObject);
var
  i : Integer;
  f : TField;
begin
  ...
  {Restringindo Vendedor}
  if not DMProjeto.LerPermissao('FrmEstimatesAprovadas', 'FatLivreVnd') then begin
    Q_Estimates.SQL.Text := replace(Q_Estimates.SQL.Text, 'XX', ' and s.Vendedor = '+IntToStr(DMProjeto.nFuncionarioLogado)+ ' ');
    C_Estimates.Open;
    lbEstadoForm.Caption :=  lbEstadoForm.Caption + C_EstimatesNomeFunc.Value;
    end
  else begin
    lbEstadoForm.Caption :=  lbEstadoForm.Caption + 'TODOS.';
    Q_Estimates.SQL.Text := replace(Q_Estimates.SQL.Text, 'XX', ' ');
    C_Estimates.Open;
  end;
  ...
  btLimpar.Visible := (DMProjeto.Parametro('ExisteCaixa') = 'S');
  btLimpar.Top := btGravar.Top + 10;

end;

procedure TFrmEstimatesAprovadas.btComando2Click(Sender: TObject);
begin
  inherited;
...
procedure TFrmEstimatesAprovadas.btCRMClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmEstimatesAprovadas.btComando1Click(Sender: TObject);
begin
  inherited;
...
procedure TFrmEstimatesAprovadas.btInvoicesClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmEstimatesAprovadas.btExtratoParcelasClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmEstimatesAprovadas.cbObsClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmEstimatesAprovadas.AtualizarNovasEstimates1Click(
  Sender: TObject);
begin
  inherited;
  C_Estimates.Close;
...
procedure TFrmEstimatesAprovadas.FormsComponentRefresh(Sender: TObject);
begin
  inherited;
...
procedure TFrmEstimatesAprovadas.btLimparClick(Sender: TObject);
var
  sPossuiTax, sCalcComissao, sBaixa, sPossuiEntrega : String;
  nTipoMov, nTipoPadrao : Integer;
begin
  inherited;

  if DlgMsg.ShowMsg(2321, [C_EstimatesNumero.Value, C_EstimatesNome.Value]) = 200 then
    exit;

  try
    with Q_SQL do begin
      Close;
      SQL.Text := 'Select TipoMovimento, PossuiTax, CALCCOMISSAO, BaixaEstoque, PossuiEntrega, TipoPadrao '+
                  'From TiposMovimento where TipoMovimento = :TM OR TipoPadrao = 2 ';
      params[0].asInteger := StrToIntDef(DMProjeto.Parametro('TipoMovParaCaixa'), 0);
      Open;

      if RecordCount > 0 then begin
        sPossuiTax := FieldByName('PossuiTax').asString;
        sCalcComissao := FieldByName('CALCCOMISSAO').asString;
        sBaixa := FieldByName('BaixaEstoque').asString;
        sPossuiEntrega := FieldByName('PossuiEntrega').asString;
        nTipoPadrao := FieldByName('TipoPadrao').asInteger;
        nTipoMov    := FieldByName('TipoMovimento').asInteger;
        end
      else begin
        Close;
        DlgMsg.ShowMsg(2320);
      ...
      {Trocando o Tipo de Movimento da Estimate}
      Close;
      SQL.Text := 'Update Saidas set Status = ''X'', PossuiTax = :PT, PossuiEntrega = :PE, '+
                  '                  CalcComissao = :CC, BaixaEstoque = :BE, TipoPadrao = :TP, '+
                  '                  TipoMovimento = :TM '+
                  'where Saida = :S and TipoPadrao = 3 and situacao = ''N'' ';
      ParamByName('S').asInteger := C_EstimatesSaida.Value;
      ParamByName('PT').asString := sPossuiTax;
      ParamByName('PE').asString := sPossuiEntrega;
      ParamByName('CC').asString := sCalcComissao;
      ParamByName('BE').asString := sBaixa;
      ParamByName('TP').asInteger := nTipoPadrao;
      ParamByName('TM').asInteger := nTipoMov;
      ExecSQL;

      {Atualizando SaidasItens para que a baixa de estoque seja efetivada - Trigger}
      Close;
      SQL.Text := 'Update SaidasItens set Importacao = null where saida = :S ';
      ParamByName('S').asInteger := C_EstimatesSaida.Value;
      ExecSQL;
...
procedure TFrmEstimatesAprovadas.btGravarClick(Sender: TObject);
begin
  inherited;

  if DlgMsg.ShowMsg(2322, [C_EstimatesNumero.Value, C_EstimatesNome.Value]) = 200 then
    exit;

  DMProjeto.SetParametrosForm( [null, 1, 'FATURAR OPERACAO', C_EstimatesSaida.Value,
                                StrToIntDef(DMProjeto.Parametro('TipoMovFatEstimate'), 0) ] );
...
procedure TFrmEstimatesAprovadas.C_EstimatesCalcFields(DataSet: TDataSet);
begin
  inherited;
  C_EstimatesItens.Value := 'Itens...';
end;

procedure TFrmEstimatesAprovadas.C_EstimatesAfterScroll(DataSet: TDataSet);
begin
  inherited;
  if C_EstimatesStatus.Value = 'V' then begin
