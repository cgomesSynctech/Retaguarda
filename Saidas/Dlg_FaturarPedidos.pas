...
uses
  ...
  TS_CalcEdit, TS_CheckBox;

type
  TDlgFaturarPedidos = class(TFrmModeloCadastros)
    ...
    C_MovimentosPCOMISSAO: TBCDField;
    ...
    C_ItensOLDQTDFATURADA: TBCDField;
...
var
  DlgFaturarPedidos: TDlgFaturarPedidos;
...
procedure TDlgFaturarPedidos.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TDlgFaturarPedidos.Q_MovimentosBeforeOpen(DataSet: TDataSet);
var
...
procedure TDlgFaturarPedidos.GridMovsTS_OnAfterSelection(
...
procedure TDlgFaturarPedidos.dbgItensTS_OnAfterSelection(
  Sender: TObject; bSelected: Boolean; QtdSel: Integer);
begin
  inherited;
...
procedure TDlgFaturarPedidos.GridMovsItensCloseUp(Sender: TObject;
...
procedure TDlgFaturarPedidos.AtualizarSelecionados;
begin
...
procedure TDlgFaturarPedidos.dbgItensEditing(Sender: TObject;
...
procedure TDlgFaturarPedidos.dbgItensEdited(Sender: TObject;
...
procedure TDlgFaturarPedidos.C_ItensicQtdMesclarValidate(
  Sender: TField);
begin
  inherited;
...
procedure TDlgFaturarPedidos.btGravarClick(Sender: TObject);
var
...
procedure TDlgFaturarPedidos.JoinItemToOperation;
  ...
  begin
    ...
    If cbPrecoCompra.Checked Then
      ...
      Else
         DM.C_ItensPreco.Value          := (C_Itens.FieldByName('custocontabil').Value + (C_Itens.FieldByName('custocontabil').Value * (dfReducao.Value/100)))
    Else
       DM.C_ItensPreco.Value          := (C_ItensPreco.Value * (1 - (dfReducao.Value/100)));
...
procedure TDlgFaturarPedidos.dbgItensTS_OnSelection(Sender: TObject;
...
procedure TDlgFaturarPedidos.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TDlgFaturarPedidos.cbPrecoCompraChange(Sender: TObject);
begin
  inherited;
  If cbPrecoCompra.Checked Then
