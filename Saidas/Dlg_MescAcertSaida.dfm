inherited DlgMescAcertSaida: TDlgMescAcertSaida
  Left = 340
  Top = 225
  Width = 750
  Height = 413
  Caption = 'Clientes / Vendas'
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnNavigator: TTS_Panel
    Left = 616
    Height = 332
    inherited btFecharCadastro: TTS_SpeedButton
      Top = 298
    end
    inherited btLimpar: TTS_SpeedButton
      Hint = 'Faz uma c'#243'pia do Movimento selecionado para a opera'#231#227'o corrente.'
      Caption = '&Copiar'
      Glyph.Data = {
        
        E7010101010101010101}
      NumGlyphs = 1
      Visible = False
    end
    inherited btGravar: TTS_SpeedButton
      Hint = 'Junta os Movimentos e Itens selecionados na opera'#231#227'o corrente.'
      Caption = 'Acertar'
      Glyph.Data = {
        
        DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
    end
    inherited TS_Bevel1: TTS_Bevel
      Visible = False
    end
  end
  inherited pnTitulo: TTS_MaxPanel
    Width = 734
    Gradient.ColorStart = 14859922
    inherited lbEstadoForm: TTS_Label
      Top = 24
      Width = 294
      Caption = 'Informe a quantidade dos itens que o cliente est'#225' acertando.'
    end
    inherited btHelp: TTS_SpeedButton
      Left = 706
    end
    inherited lbCaption: TdxfLabel
      Left = 52
      Width = 228
      AutoSize = True
      Caption = 'Acerto de Consigna'#231#245'es'
      Effect3D.ShadowedColor = 14859922
    end
  end
  inherited pnDados: TTS_Panel
    Width = 616
    Height = 332
    Color = 16116702
    object dbtForn: TTS_DBTextEffect
      Left = 6
      Top = 5
      Width = 610
      Height = 21
      DataField = 'F_NOME'
      Effects.ShadowDepth = 2
      Effects.Style = tsRaised
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      EditOffSet.x = 0
      EditOffSet.y = 0
      AllowEditOnClick = False
      EditFont.Charset = DEFAULT_CHARSET
      EditFont.Color = clWindowText
      EditFont.Height = -11
      EditFont.Name = 'MS Sans Serif'
      EditFont.Style = []
    end
    object dbgItens: TTS_QDBGrid
      Left = 66
      Top = 77
      Width = 559
      Height = 183
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'SAIDAITEM'
      SummaryGroups = <>
      SummarySeparator = '|'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = ppmGridItens
      TabOrder = 1
      Visible = False
      AutoSearchColor = clYellow
      AutoSearchTextColor = clNavy
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = []
      DataSource = C_ItensDS
      Filter.Criteria = {00000000}
      GroupPanelColor = 15461355
      HeaderColor = 15461355
      HeaderFont.Charset = DEFAULT_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -11
      HeaderFont.Name = 'Tahoma'
      HeaderFont.Style = []
      HideFocusRect = True
      HideSelection = True
      OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabs, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoHotTrack, edgoShowButtonAlways, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      OnEdited = dbgItensEdited
      OnEditing = dbgItensEditing
      Anchors = [akLeft, akTop, akRight, akBottom]
      TS_PermitirQuantidade = False
      TS_DescriptionCanChange = False
      TS_GridMenuOptions = []
      TS_AppendOnEnter = False
      RowFooterNodeFont.Charset = DEFAULT_CHARSET
      RowFooterNodeFont.Color = clWindowText
      RowFooterNodeFont.Height = -11
      RowFooterNodeFont.Name = 'Tahoma'
      RowFooterNodeFont.Style = [fsBold]
      TS_AccountFooterStyle = False
      TS_HideGroupHeader = False
      TS_AnotherColor = clWindow
      TS_ReportHeaderStyle = False
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clWindowText
      GroupNodeFont.Height = -11
      GroupNodeFont.Name = 'Tahoma'
      GroupNodeFont.Style = [fsBold]
      TS_SummaryFooterFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterFont.Color = clWindowText
      TS_SummaryFooterFont.Height = -11
      TS_SummaryFooterFont.Name = 'Tahoma'
      TS_SummaryFooterFont.Style = [fsBold]
      TS_SummaryFooterSelFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterSelFont.Color = clWindowText
      TS_SummaryFooterSelFont.Height = -11
      TS_SummaryFooterSelFont.Name = 'Tahoma'
      TS_SummaryFooterSelFont.Style = [fsBold]
      TS_ShowSelectionColumn = True
      TS_LikeString = False
      TS_SelectionColor = 12054783
      TS_SelectionFont.Charset = DEFAULT_CHARSET
      TS_SelectionFont.Color = clWindowText
      TS_SelectionFont.Height = -11
      TS_SelectionFont.Name = 'Tahoma'
      TS_SelectionFont.Style = []
      TS_OnSelection = dbgItensTS_OnSelection
      TS_OnAfterSelection = dbgItensTS_OnAfterSelection
      TS_SelectedColumn = '_icSelecionado'
      TS_ID = 0
      TS_TipoDescricao = tdVenda
      TS_SummaryFooterQtdText = 'Qtd. Geral:'
      TS_SummaryFooterQtdSelectedText = 'Qtd. Selecionada:'
      object dbgItensSEQUENCIA: TdxDBGridMaskColumn
        Caption = '#'
        Color = 15461355
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 29
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'SEQUENCIA'
      end
      object dbgItensITEM: TdxDBGridMaskColumn
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 1033
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'ITEM'
      end
      object dbgItensCodigo: TdxDBGridColumn
        Caption = 'C'#243'digo'
        Color = 15461355
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'CODIGO'
      end
      object dbgItensDESCRICAO: TdxDBGridMaskColumn
        Caption = 'Descri'#231#227'o'
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 160
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'DESCRICAO'
      end
      object dbgItensQUANTIDADE: TdxDBGridMaskColumn
        Caption = 'Qtd. Original'
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 74
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'QUANTIDADE'
      end
      object dbgItensQTDFATURADA: TdxDBGridColumn
        Caption = 'Qtd. Acertada'
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Sorted = csDown
        Width = 80
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'QTDFATURADA'
      end
      object dbgItensPRECO: TdxDBGridMaskColumn
        Caption = 'Pre'#231'o'
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 82
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'PRECO'
      end
      object dbgItensIMPORTACAO: TdxDBGridMaskColumn
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 1181
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'IMPORTACAO'
      end
      object dbgItensUSOTIPOITEM: TdxDBGridMaskColumn
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 1521
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'USOTIPOITEM'
      end
      object dbgItensSUBTOTALITEM: TdxDBGridMaskColumn
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 1883
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'SUBTOTALITEM'
      end
      object dbgItensORDEM: TdxDBGridMaskColumn
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 1033
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'ORDEM'
      end
      object dbgItensHASCHILDREN: TdxDBGridMaskColumn
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 1220
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'HASCHILDREN'
      end
      object dbgItensTIPOITEM: TdxDBGridMaskColumn
        Color = 15461355
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 1033
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'TIPOITEM'
      end
      object dbgItensicQtdMesclar: TdxDBGridColumn
        Caption = 'QTD.'
        DisableCustomizing = True
        HeaderAlignment = taCenter
        Width = 82
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'icQtdMesclar'
      end
    end
    object GridMovs: TTS_QDBGrid
      Left = 7
      Top = 41
      Width = 611
      Height = 289
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'SAIDA'
      ShowSummaryFooter = True
      SummaryGroups = <
        item
          DefaultGroup = True
          SummaryItems = <
            item
              SummaryField = 'QtdItens'
              SummaryFormat = '>QtdItens=###0'
              SummaryType = cstSum
            end>
          Name = 'Default'
        end>
      SummarySeparator = '|'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = ppmGrid
      TabOrder = 0
      Visible = False
      AutoSearchColor = clYellow
      AutoSearchTextColor = clNavy
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = []
      DataSource = C_MovimentosDS
      Filter.Criteria = {00000000}
      GroupPanelColor = 15724527
      HeaderColor = 15461355
      HeaderFont.Charset = DEFAULT_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -11
      HeaderFont.Name = 'Tahoma'
      HeaderFont.Style = []
      HideFocusRect = True
      HideSelection = True
      OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabs, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoHotTrack, edgoShowButtonAlways, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      Anchors = [akLeft, akTop, akRight, akBottom]
      TS_PermitirQuantidade = False
      TS_DescriptionCanChange = False
      TS_GridMenuOptions = [gmLayout, gmDados, gmAgrupar, gmImprimir, gmExportar]
      TS_AppendOnEnter = False
      RowFooterNodeFont.Charset = DEFAULT_CHARSET
      RowFooterNodeFont.Color = clWindowText
      RowFooterNodeFont.Height = -11
      RowFooterNodeFont.Name = 'Tahoma'
      RowFooterNodeFont.Style = [fsBold]
      TS_AccountFooterStyle = False
      TS_HideGroupHeader = False
      TS_AnotherColor = clWindow
      TS_ReportHeaderStyle = False
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clWindowText
      GroupNodeFont.Height = -11
      GroupNodeFont.Name = 'Tahoma'
      GroupNodeFont.Style = [fsBold]
      TS_SummaryFooterFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterFont.Color = clWindowText
      TS_SummaryFooterFont.Height = -11
      TS_SummaryFooterFont.Name = 'Tahoma'
      TS_SummaryFooterFont.Style = [fsBold]
      TS_SummaryFooterSelFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterSelFont.Color = clWindowText
      TS_SummaryFooterSelFont.Height = -11
      TS_SummaryFooterSelFont.Name = 'Tahoma'
      TS_SummaryFooterSelFont.Style = [fsBold]
      TS_ShowSelectionColumn = True
      TS_LikeString = False
      TS_SelectionColor = 12054783
      TS_SelectionFont.Charset = DEFAULT_CHARSET
      TS_SelectionFont.Color = clWindowText
      TS_SelectionFont.Height = -11
      TS_SelectionFont.Name = 'Tahoma'
      TS_SelectionFont.Style = []
      TS_OnAfterSelection = GridMovsTS_OnAfterSelection
      TS_SelectedColumn = '_icSelecionado'
      TS_ID = 0
      TS_TipoDescricao = tdVenda
      TS_SummaryFields.Strings = (
        'QtdItens;Sum;')
      TS_SummaryFooterQtdText = 'Qtd. Geral:'
      TS_SummaryFooterQtdSelectedText = 'Qtd. Selecionada:'
      object GridMovsDESCTIPOMOV: TdxDBGridMaskColumn
        Caption = 'Opera'#231#227'o'
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 77
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCTIPOMOV'
      end
      object GridMovsNumero: TdxDBGridColumn
        Caption = 'N'#250'mero'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Sorted = csDown
        Width = 75
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'NUMERO'
      end
      object GridMovsDATA: TdxDBGridDateColumn
        Caption = 'Data'
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 78
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATA'
      end
      object GridMovsNOME: TdxDBGridMaskColumn
        Caption = 'Cliente'
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 91
        BandIndex = 0
        RowIndex = 0
        FieldName = 'NOME'
      end
      object GridMovsPrevisao: TdxDBGridColumn
        Caption = 'Previsto para'
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 75
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DATAENTREGA'
      end
      object GridMovsTOTAL: TdxDBGridMaskColumn
        Caption = 'Total'
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 85
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TOTAL'
      end
      object GridMovsDESCTIPOPADRAO: TdxDBGridMaskColumn
        Caption = 'Tipo Opera'#231#227'o'
        DisableEditor = True
        HeaderAlignment = taCenter
        Sorted = csUp
        Visible = False
        Width = 51
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCTIPOPADRAO'
        GroupIndex = 0
      end
      object GridMovsFAVORECIDO: TdxDBGridMaskColumn
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        TabStop = False
        Visible = False
        Width = 30
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'FAVORECIDO'
      end
      object GridMovsSITUACAO: TdxDBGridMaskColumn
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        TabStop = False
        Visible = False
        Width = 26
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'SITUACAO'
      end
      object GridMovsItens: TdxDBGridPopupColumn
        Caption = 'Itens do Pedido'
        HeaderAlignment = taCenter
        Width = 157
        BandIndex = 0
        RowIndex = 0
        FieldName = 'icItens'
        PopupControl = dbgItens
        PopupFormBorderStyle = pbsSysPanel
        OnCloseUp = GridMovsItensCloseUp
      end
      object GridMovsQtdItensSel: TdxDBGridColumn
        Caption = 'Qtd.Sel.'
        HeaderAlignment = taCenter
        Width = 62
        BandIndex = 0
        RowIndex = 0
        FieldName = 'QtdItens'
        SummaryFooterType = cstSum
        SummaryFooterFormat = '###0'
      end
      object GridMovsSTATUS: TdxDBGridCheckColumn
        Alignment = taLeftJustify
        Caption = 'Fechar'
        DisableCustomizing = True
        HeaderAlignment = taCenter
        TabStop = False
        Width = 44
        BandIndex = 0
        RowIndex = 0
        DisableGrouping = True
        FieldName = 'STATUS'
        ValueChecked = 'H'
        ValueGrayed = 'L'
        ValueUnchecked = 'P'
        DisableFilter = True
      end
    end
    object gridAcerto: TTS_QDBGrid
      Left = 7
      Top = 42
      Width = 611
      Height = 283
      Bands = <
        item
          Fixed = bfLeft
        end
        item
        end>
      DefaultLayout = False
      HeaderPanelRowCount = 1
      KeyField = 'ITEM'
      SummaryGroups = <>
      SummarySeparator = '|'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = ppmAcerto
      TabOrder = 3
      AutoSearchColor = clAqua
      AutoSearchTextColor = clNavy
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'MS Sans Serif'
      BandFont.Style = []
      DataSource = C_AcertoDS
      Filter.Criteria = {00000000}
      GroupPanelColor = 15461355
      HeaderColor = 15461355
      HeaderFont.Charset = DEFAULT_CHARSET
      HeaderFont.Color = clWindowText
      HeaderFont.Height = -11
      HeaderFont.Name = 'Tahoma'
      HeaderFont.Style = []
      HideFocusRect = True
      HideSelection = True
      OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabs, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoBandHeaderWidth, edgoHotTrack, edgoShowButtonAlways, edgoUseBitmap]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clBlue
      PreviewFont.Height = -11
      PreviewFont.Name = 'MS Sans Serif'
      PreviewFont.Style = []
      OnCustomDrawCell = gridAcertoCustomDrawCell
      TS_PermitirQuantidade = False
      TS_DescriptionCanChange = False
      TS_AppendOnEnter = False
      RowFooterNodeFont.Charset = DEFAULT_CHARSET
      RowFooterNodeFont.Color = clWindowText
      RowFooterNodeFont.Height = -11
      RowFooterNodeFont.Name = 'Tahoma'
      RowFooterNodeFont.Style = [fsBold]
      TS_AccountFooterStyle = False
      TS_HideGroupHeader = False
      TS_AnotherColor = clWindow
      TS_ReportHeaderStyle = False
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clWindowText
      GroupNodeFont.Height = -11
      GroupNodeFont.Name = 'Tahoma'
      GroupNodeFont.Style = [fsBold]
      TS_SummaryFooterFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterFont.Color = clWindowText
      TS_SummaryFooterFont.Height = -11
      TS_SummaryFooterFont.Name = 'Tahoma'
      TS_SummaryFooterFont.Style = [fsBold]
      TS_SummaryFooterSelFont.Charset = DEFAULT_CHARSET
      TS_SummaryFooterSelFont.Color = clWindowText
      TS_SummaryFooterSelFont.Height = -11
      TS_SummaryFooterSelFont.Name = 'Tahoma'
      TS_SummaryFooterSelFont.Style = [fsBold]
      TS_ShowSelectionColumn = True
      TS_LikeString = False
      TS_SelectionColor = 12054783
      TS_SelectionFont.Charset = DEFAULT_CHARSET
      TS_SelectionFont.Color = clWindowText
      TS_SelectionFont.Height = -11
      TS_SelectionFont.Name = 'Tahoma'
      TS_SelectionFont.Style = []
      TS_SelectedColumn = '_icSelecionado'
      TS_ID = 0
      TS_TipoDescricao = tdVenda
      TS_SummaryFooterQtdText = 'Qtd. Geral:'
      TS_SummaryFooterQtdSelectedText = 'Qtd. Selecionada:'
      object gridAcertoTIPOMOVIMENTO: TdxDBGridMaskColumn
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 67
        BandIndex = 0
        RowIndex = 0
        FieldName = 'TIPOMOVIMENTO'
      end
      object gridAcertoFAVORECIDO: TdxDBGridMaskColumn
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 52
        BandIndex = 0
        RowIndex = 0
        FieldName = 'FAVORECIDO'
      end
      object gridAcertoNOME: TdxDBGridMaskColumn
        Caption = 'Fornecedor'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 229
        BandIndex = 1
        RowIndex = 0
        FieldName = 'NOME'
      end
      object gridAcertoITEM: TdxDBGridMaskColumn
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 46
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ITEM'
      end
      object gridAcertoCODIGO: TdxDBGridMaskColumn
        Caption = 'C'#243'digo'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 82
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CODIGO'
      end
      object gridAcertoDESCRICAO: TdxDBGridMaskColumn
        Caption = 'Item'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Sorted = csDown
        Width = 252
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCRICAO'
      end
      object gridAcertoQTD: TdxDBGridCurrencyColumn
        Caption = 'Qtd consignada'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 83
        BandIndex = 1
        RowIndex = 0
        FieldName = 'QTD'
        DisplayFormat = '0.00;-0.00'
        Nullable = False
      end
      object gridAcertoQtddevolvida: TdxDBGridColumn
        Caption = 'Qtd a acertar'
        DisableCustomizing = True
        Width = 80
        BandIndex = 1
        RowIndex = 0
        FieldName = 'Qtddevolvida'
      end
      object gridAcertoQTDDEV: TdxDBGridCurrencyColumn
        Caption = 'Qtd devolvida'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 77
        BandIndex = 1
        RowIndex = 0
        FieldName = 'QTDDEV'
        DisplayFormat = '0.00;-0.00'
        Nullable = False
      end
      object gridAcertoQTDACERT: TdxDBGridCurrencyColumn
        Caption = 'Qtd acertada'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 71
        BandIndex = 1
        RowIndex = 0
        FieldName = 'QTDACERT'
        DisplayFormat = '0.00;-0.00'
        Nullable = False
      end
      object gridAcertoUNIDADE: TdxDBGridMaskColumn
        Caption = 'Und.'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 40
        BandIndex = 0
        RowIndex = 0
        FieldName = 'UNIDADE'
      end
      object gridAcertoESTOQUE: TdxDBGridCurrencyColumn
        Caption = 'Estoque'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 57
        BandIndex = 0
        RowIndex = 0
        FieldName = 'ESTOQUE'
        DisplayFormat = '0.00;-0.00'
        Nullable = False
      end
      object gridAcertoREFERENCIA: TdxDBGridMaskColumn
        Caption = 'Refer'#234'ncia'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Width = 63
        BandIndex = 1
        RowIndex = 0
        FieldName = 'REFERENCIA'
      end
      object gridAcertoESTOQUECONSIGSAIDA: TdxDBGridCurrencyColumn
        Caption = 'Estoque Consignado'
        DisableCustomizing = True
        DisableEditor = True
        HeaderAlignment = taCenter
        Visible = False
        Width = 91
        BandIndex = 1
        RowIndex = 0
        FieldName = 'ESTOQUECONSIGSAIDA'
        DisplayFormat = '0.00;-0.00'
        Nullable = False
      end
      object gridAcertoQtdvalidar: TdxDBGridColumn
        DisableCustomizing = True
        DisableEditor = True
        Visible = False
        BandIndex = 0
        RowIndex = 0
        FieldName = 'Qtdvalidar'
      end
    end
    object pnBar: TTS_Panel
      Left = 3
      Top = 30
      Width = 611
      Height = 8
      Anchors = [akLeft, akTop, akRight]
      BevelOuter = bvLowered
      Color = 16247774
      TabOrder = 2
      Visible = False
    end
  end
  inherited DlgMsg: TDlgMsg
    Left = 410
    Top = 423
  end
  inherited LastDataObject: TTS_LastDataObject
    Left = 413
    Top = 426
  end
  inherited ppmPadrao: TTS_PopupMenu
    Left = 418
    Top = 428
  end
  inherited Beep: TBTBeeper
    Left = 84
    Top = 106
  end
  inherited FormStorage: TFormStorage
    Left = 220
    Top = 98
  end
  inherited FormsComponent: TFormsComponent
    ModuloStyle = DMProjeto.msClientes
    BarEndColor = 14859922
    CaptionShadow = 14859922
    FormColor = 16116702
    Modulo = 'Clientes / Vendas'
    Caption = 'Acerto de Consigna'#231#245'es'
    Left = 664
    Top = 203
  end
  inherited ppmHelp: TTS_PopupMenu
    Left = 413
    Top = 419
  end
  object Q_Movimentos: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BeforeOpen = Q_MovimentosBeforeOpen
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'Select '#9'e.*,'
      #9'f.Nome, t.Descricao as DescTipoMov, '
      #9'tp.Descricao as DescTipoPadrao'
      
        'From (((Saidas e INNER JOIN Favorecidos f ON f.Favorecido = e.Fa' +
        'vorecido)'
      
        '         INNER JOIN TiposMovimento t ON t.TipoMovimento = e.Tipo' +
        'Movimento)'
      
        '         INNER JOIN TiposPadrao tp ON tp.TipoPadrao = t.TipoPadr' +
        'ao)'
      ''
      
        'Where  e.Situacao = '#39'N'#39' and KK and  e.TipoPadrao in (ZZ) and '#9'e.' +
        'Favorecido in (XX)'
      ''
      ''
      '')
    UniDirectional = True
    Left = 53
    Top = 180
  end
  object P_Movimentos: TDataSetProvider
    DataSet = Q_Movimentos
    Constraints = True
    Left = 50
    Top = 221
  end
  object C_Movimentos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'P_Movimentos'
    Left = 48
    Top = 285
    object C_Movimentos_icSelecionado: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = '_icSelecionado'
    end
    object C_MovimentosicItens: TStringField
      FieldKind = fkInternalCalc
      FieldName = 'icItens'
      Size = 1000
    end
    object C_MovimentosQ_Itens: TDataSetField
      FieldName = 'Q_Itens'
    end
    object C_MovimentosQtdItens: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'QtdItens'
      DisplayFormat = '###0'
    end
    object C_MovimentosSAIDA: TIntegerField
      FieldName = 'SAIDA'
      Origin = 'SAIDAS.SAIDA'
      Required = True
    end
    object C_MovimentosNUMERO: TStringField
      FieldName = 'NUMERO'
      Origin = 'SAIDAS.NUMERO'
      Size = 12
    end
    object C_MovimentosDATA: TDateField
      FieldName = 'DATA'
      Origin = 'SAIDAS.DATA'
    end
    object C_MovimentosFAVORECIDO: TIntegerField
      FieldName = 'FAVORECIDO'
      Origin = 'SAIDAS.FAVORECIDO'
    end
    object C_MovimentosOBS: TStringField
      FieldName = 'OBS'
      Origin = 'SAIDAS.OBS'
      Size = 255
    end
    object C_MovimentosDESCONTO: TFloatField
      FieldName = 'DESCONTO'
      Origin = 'SAIDAS.DESCONTO'
    end
    object C_MovimentosTAX: TIntegerField
      FieldName = 'TAX'
      Origin = 'SAIDAS.TAX'
    end
    object C_MovimentosPERCENTUALTAX: TFloatField
      FieldName = 'PERCENTUALTAX'
      Origin = 'SAIDAS.PERCENTUALTAX'
    end
    object C_MovimentosPLANOPAGAMENTO: TIntegerField
      FieldName = 'PLANOPAGAMENTO'
      Origin = 'SAIDAS.PLANOPAGAMENTO'
    end
    object C_MovimentosVENDEDOR: TIntegerField
      FieldName = 'VENDEDOR'
      Origin = 'SAIDAS.VENDEDOR'
    end
    object C_MovimentosTOTAL: TFloatField
      FieldName = 'TOTAL'
      Origin = 'SAIDAS.TOTAL'
    end
    object C_MovimentosFRETE: TFloatField
      FieldName = 'FRETE'
      Origin = 'SAIDAS.FRETE'
    end
    object C_MovimentosTIPOENTREGA: TIntegerField
      FieldName = 'TIPOENTREGA'
      Origin = 'SAIDAS.TIPOENTREGA'
    end
    object C_MovimentosDATAENTREGA: TDateField
      FieldName = 'DATAENTREGA'
      Origin = 'SAIDAS.DATAENTREGA'
    end
    object C_MovimentosIMPORTACAO: TIntegerField
      FieldName = 'IMPORTACAO'
      Origin = 'SAIDAS.IMPORTACAO'
    end
    object C_MovimentosJUROS: TFloatField
      FieldName = 'JUROS'
      Origin = 'SAIDAS.JUROS'
    end
    object C_MovimentosOUTRASDESPESAS: TFloatField
      FieldName = 'OUTRASDESPESAS'
      Origin = 'SAIDAS.OUTRASDESPESAS'
    end
    object C_MovimentosSITUACAO: TStringField
      FieldName = 'SITUACAO'
      Origin = 'SAIDAS.SITUACAO'
      Size = 1
    end
    object C_MovimentosTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
      Origin = 'SAIDAS.TIPOMOVIMENTO'
    end
    object C_MovimentosPEDIDO_CLIENTE: TStringField
      FieldName = 'PEDIDO_CLIENTE'
      Origin = 'SAIDAS.PEDIDO_CLIENTE'
      Size = 12
    end
    object C_MovimentosPOSSUITAX: TStringField
      FieldName = 'POSSUITAX'
      Origin = 'SAIDAS.POSSUITAX'
      Size = 1
    end
    object C_MovimentosBAIXAESTOQUE: TStringField
      FieldName = 'BAIXAESTOQUE'
      Origin = 'SAIDAS.BAIXAESTOQUE'
      Size = 1
    end
    object C_MovimentosCALCCOMISSAO: TStringField
      FieldName = 'CALCCOMISSAO'
      Origin = 'SAIDAS.CALCCOMISSAO'
      Size = 1
    end
    object C_MovimentosPOSSUIENTREGA: TStringField
      FieldName = 'POSSUIENTREGA'
      Origin = 'SAIDAS.POSSUIENTREGA'
      Size = 1
    end
    object C_MovimentosCAMPO01: TStringField
      FieldName = 'CAMPO01'
      Origin = 'SAIDAS.CAMPO01'
      Size = 50
    end
    object C_MovimentosCAMPO02: TStringField
      FieldName = 'CAMPO02'
      Origin = 'SAIDAS.CAMPO02'
      Size = 50
    end
    object C_MovimentosCAMPO03: TStringField
      FieldName = 'CAMPO03'
      Origin = 'SAIDAS.CAMPO03'
      Size = 50
    end
    object C_MovimentosCAMPO04: TStringField
      FieldName = 'CAMPO04'
      Origin = 'SAIDAS.CAMPO04'
      Size = 50
    end
    object C_MovimentosIMPRESSO: TStringField
      FieldName = 'IMPRESSO'
      Origin = 'SAIDAS.IMPRESSO'
      Size = 1
    end
    object C_MovimentosCAIXA: TIntegerField
      FieldName = 'CAIXA'
      Origin = 'SAIDAS.CAIXA'
    end
    object C_MovimentosIMPOSTO: TFloatField
      FieldName = 'IMPOSTO'
      Origin = 'SAIDAS.IMPOSTO'
    end
    object C_MovimentosIMPOSTO_INCLUSO: TFloatField
      FieldName = 'IMPOSTO_INCLUSO'
      Origin = 'SAIDAS.IMPOSTO_INCLUSO'
    end
    object C_MovimentosPCOMISSAO: TFloatField
      FieldName = 'PCOMISSAO'
      Origin = 'SAIDAS.PCOMISSAO'
    end
    object C_MovimentosLOCALENTREGA: TIntegerField
      FieldName = 'LOCALENTREGA'
      Origin = 'SAIDAS.LOCALENTREGA'
    end
    object C_MovimentosTABELAPADRAO: TIntegerField
      FieldName = 'TABELAPADRAO'
      Origin = 'SAIDAS.TABELAPADRAO'
    end
    object C_MovimentosTIPOPADRAO: TIntegerField
      FieldName = 'TIPOPADRAO'
      Origin = 'SAIDAS.TIPOPADRAO'
    end
    object C_MovimentosDATACANCELAMENTO: TDateField
      FieldName = 'DATACANCELAMENTO'
      Origin = 'SAIDAS.DATACANCELAMENTO'
    end
    object C_MovimentosFUNCCANCELAMENTO: TIntegerField
      FieldName = 'FUNCCANCELAMENTO'
      Origin = 'SAIDAS.FUNCCANCELAMENTO'
    end
    object C_MovimentosMOTIVOCANCELAMENTO: TStringField
      FieldName = 'MOTIVOCANCELAMENTO'
      Origin = 'SAIDAS.MOTIVOCANCELAMENTO'
      Size = 80
    end
    object C_MovimentosPJUROS: TFloatField
      FieldName = 'PJUROS'
      Origin = 'SAIDAS.PJUROS'
    end
    object C_MovimentosVALORCOMISSAO: TFloatField
      FieldName = 'VALORCOMISSAO'
      Origin = 'SAIDAS.VALORCOMISSAO'
    end
    object C_MovimentosTOTALITENS: TFloatField
      FieldName = 'TOTALITENS'
      Origin = 'SAIDAS.TOTALITENS'
    end
    object C_MovimentosTOTALITENS123: TFloatField
      FieldName = 'TOTALITENS123'
      Origin = 'SAIDAS.TOTALITENS123'
    end
    object C_MovimentosBASEIMPOSTO: TFloatField
      FieldName = 'BASEIMPOSTO'
      Origin = 'SAIDAS.BASEIMPOSTO'
    end
    object C_MovimentosBASEINCLUSO: TFloatField
      FieldName = 'BASEINCLUSO'
      Origin = 'SAIDAS.BASEINCLUSO'
    end
    object C_MovimentosDESCONTOITENS: TFloatField
      FieldName = 'DESCONTOITENS'
      Origin = 'SAIDAS.DESCONTOITENS'
    end
    object C_MovimentosVALIDADE: TDateField
      FieldName = 'VALIDADE'
      Origin = 'SAIDAS.VALIDADE'
    end
    object C_MovimentosCONDICOESPGTO: TStringField
      FieldName = 'CONDICOESPGTO'
      Origin = 'SAIDAS.CONDICOESPGTO'
      Size = 255
    end
    object C_MovimentosDATAENTREGUE: TDateField
      FieldName = 'DATAENTREGUE'
      Origin = 'SAIDAS.DATAENTREGUE'
    end
    object C_MovimentosSTATUS: TStringField
      FieldName = 'STATUS'
      Origin = 'SAIDAS.STATUS'
      Size = 1
    end
    object C_MovimentosTIPOIMPRESSAO: TStringField
      FieldName = 'TIPOIMPRESSAO'
      Origin = 'SAIDAS.TIPOIMPRESSAO'
      Size = 1
    end
    object C_MovimentosSTATUSANTERIOR: TStringField
      FieldName = 'STATUSANTERIOR'
      Origin = 'SAIDAS.STATUSANTERIOR'
      Size = 1
    end
    object C_MovimentosDTMODIFICACAO: TDateTimeField
      FieldName = 'DTMODIFICACAO'
      Origin = 'SAIDAS.DTMODIFICACAO'
    end
    object C_MovimentosTERMOCONTRATO: TStringField
      FieldName = 'TERMOCONTRATO'
      Origin = 'SAIDAS.TERMOCONTRATO'
      Size = 500
    end
    object C_MovimentosALMOXPADRAO: TIntegerField
      FieldName = 'ALMOXPADRAO'
      Origin = 'SAIDAS.ALMOXPADRAO'
    end
    object C_MovimentosCFOPNOTA: TStringField
      FieldName = 'CFOPNOTA'
      Origin = 'SAIDAS.CFOPNOTA'
      Size = 4
    end
    object C_MovimentosSERIENOTA: TStringField
      FieldName = 'SERIENOTA'
      Origin = 'SAIDAS.SERIENOTA'
      Size = 3
    end
    object C_MovimentosUFDESTINO: TStringField
      FieldName = 'UFDESTINO'
      Origin = 'SAIDAS.UFDESTINO'
      Size = 2
    end
    object C_MovimentosTIPOFRETE: TStringField
      FieldName = 'TIPOFRETE'
      Origin = 'SAIDAS.TIPOFRETE'
      Size = 1
    end
    object C_MovimentosTRANSPORTADORA: TIntegerField
      FieldName = 'TRANSPORTADORA'
      Origin = 'SAIDAS.TRANSPORTADORA'
    end
    object C_MovimentosDESCESPECIE: TStringField
      FieldName = 'DESCESPECIE'
      Origin = 'SAIDAS.DESCESPECIE'
      Size = 15
    end
    object C_MovimentosDESCMARCA: TStringField
      FieldName = 'DESCMARCA'
      Origin = 'SAIDAS.DESCMARCA'
      Size = 15
    end
    object C_MovimentosDESCNUMERO: TStringField
      FieldName = 'DESCNUMERO'
      Origin = 'SAIDAS.DESCNUMERO'
      Size = 10
    end
    object C_MovimentosINDICADOPELOFAV: TIntegerField
      FieldName = 'INDICADOPELOFAV'
      Origin = 'SAIDAS.INDICADOPELOFAV'
    end
    object C_MovimentosPOSSUIICMS: TStringField
      FieldName = 'POSSUIICMS'
      Origin = 'SAIDAS.POSSUIICMS'
      Size = 1
    end
    object C_MovimentosPOSSUIIPI: TStringField
      FieldName = 'POSSUIIPI'
      Origin = 'SAIDAS.POSSUIIPI'
      Size = 1
    end
    object C_MovimentosPOSSUIISS: TStringField
      FieldName = 'POSSUIISS'
      Origin = 'SAIDAS.POSSUIISS'
      Size = 1
    end
    object C_MovimentosCONTABILIZAICMS: TStringField
      FieldName = 'CONTABILIZAICMS'
      Origin = 'SAIDAS.CONTABILIZAICMS'
      Size = 1
    end
    object C_MovimentosMODELO: TStringField
      FieldName = 'MODELO'
      Origin = 'SAIDAS.MODELO'
      Size = 2
    end
    object C_MovimentosPDV: TIntegerField
      FieldName = 'PDV'
      Origin = 'SAIDAS.PDV'
    end
    object C_MovimentosFONTE: TStringField
      FieldName = 'FONTE'
      Origin = 'SAIDAS.FONTE'
      Size = 1
    end
    object C_MovimentosPLACAVEIC: TStringField
      FieldName = 'PLACAVEIC'
      Origin = 'SAIDAS.PLACAVEIC'
      Size = 10
    end
    object C_MovimentosATRATAR: TStringField
      FieldName = 'ATRATAR'
      Origin = 'SAIDAS.ATRATAR'
      Size = 255
    end
    object C_MovimentosINTRODUCAO: TStringField
      FieldName = 'INTRODUCAO'
      Origin = 'SAIDAS.INTRODUCAO'
      Size = 255
    end
    object C_MovimentosPRAZOENTREGA: TStringField
      FieldName = 'PRAZOENTREGA'
      Origin = 'SAIDAS.PRAZOENTREGA'
      Size = 40
    end
    object C_MovimentosGARANTIA: TStringField
      FieldName = 'GARANTIA'
      Origin = 'SAIDAS.GARANTIA'
      Size = 50
    end
    object C_MovimentosSEESTOQUE: TStringField
      FieldName = 'SEESTOQUE'
      Origin = 'SAIDAS.SEESTOQUE'
      Size = 1
    end
    object C_MovimentosPROCESSO: TStringField
      FieldName = 'PROCESSO'
      Origin = 'SAIDAS.PROCESSO'
      Size = 30
    end
    object C_MovimentosCARTACONVITE: TStringField
      FieldName = 'CARTACONVITE'
      Origin = 'SAIDAS.CARTACONVITE'
      Size = 30
    end
    object C_MovimentosTOMADAPRECOS: TStringField
      FieldName = 'TOMADAPRECOS'
      Origin = 'SAIDAS.TOMADAPRECOS'
      Size = 30
    end
    object C_MovimentosDATAABERTURA: TStringField
      FieldName = 'DATAABERTURA'
      Origin = 'SAIDAS.DATAABERTURA'
      Size = 30
    end
    object C_MovimentosEXPLICACOES: TStringField
      FieldName = 'EXPLICACOES'
      Origin = 'SAIDAS.EXPLICACOES'
      Size = 4000
    end
    object C_MovimentosSUBSTTRIB: TStringField
      FieldName = 'SUBSTTRIB'
      Origin = 'SAIDAS.SUBSTTRIB'
      Size = 1
    end
    object C_MovimentosCONDICOESPAGTO: TStringField
      FieldName = 'CONDICOESPAGTO'
      Origin = 'SAIDAS.CONDICOESPAGTO'
      Size = 255
    end
    object C_MovimentosCOBRANCA: TIntegerField
      FieldName = 'COBRANCA'
      Origin = 'SAIDAS.COBRANCA'
    end
    object C_MovimentosCARTEIRA: TIntegerField
      FieldName = 'CARTEIRA'
      Origin = 'SAIDAS.CARTEIRA'
    end
    object C_MovimentosMEDICO: TIntegerField
      FieldName = 'MEDICO'
      Origin = 'SAIDAS.MEDICO'
    end
    object C_MovimentosNF_CUPOM: TStringField
      FieldName = 'NF_CUPOM'
      Origin = 'SAIDAS.NF_CUPOM'
      Size = 12
    end
    object C_MovimentosNOME: TStringField
      FieldName = 'NOME'
      Origin = 'FAVORECIDOS.NOME'
      Size = 50
    end
    object C_MovimentosDESCTIPOMOV: TStringField
      FieldName = 'DESCTIPOMOV'
      Origin = 'TIPOSMOVIMENTO.DESCTIPOMOV'
      Size = 30
    end
    object C_MovimentosDESCTIPOPADRAO: TStringField
      FieldName = 'DESCTIPOPADRAO'
      Origin = 'TIPOSPADRAO.DESCTIPOPADRAO'
    end
    object C_MovimentosTOTALPGTOS: TFloatField
      FieldName = 'TOTALPGTOS'
      Origin = 'SAIDAS.TOTALPGTOS'
    end
    object C_MovimentosREDUCOESBASE: TFloatField
      FieldName = 'REDUCOESBASE'
      Origin = 'SAIDAS.REDUCOESBASE'
    end
    object C_MovimentosACRESCIMOSBASE: TFloatField
      FieldName = 'ACRESCIMOSBASE'
      Origin = 'SAIDAS.ACRESCIMOSBASE'
    end
    object C_MovimentosVOLUMES: TFloatField
      FieldName = 'VOLUMES'
      Origin = 'SAIDAS.VOLUMES'
    end
    object C_MovimentosBASECALCICMS: TFloatField
      FieldName = 'BASECALCICMS'
      Origin = 'SAIDAS.BASECALCICMS'
    end
    object C_MovimentosVALORICMS: TFloatField
      FieldName = 'VALORICMS'
      Origin = 'SAIDAS.VALORICMS'
    end
    object C_MovimentosVALORISS: TFloatField
      FieldName = 'VALORISS'
      Origin = 'SAIDAS.VALORISS'
    end
    object C_MovimentosVALORIPI: TFloatField
      FieldName = 'VALORIPI'
      Origin = 'SAIDAS.VALORIPI'
    end
    object C_MovimentosPESOBRUTO: TFloatField
      FieldName = 'PESOBRUTO'
      Origin = 'SAIDAS.PESOBRUTO'
    end
    object C_MovimentosPESOLIQUIDO: TFloatField
      FieldName = 'PESOLIQUIDO'
      Origin = 'SAIDAS.PESOLIQUIDO'
    end
    object C_MovimentosALIQICMSVENDA: TFloatField
      FieldName = 'ALIQICMSVENDA'
      Origin = 'SAIDAS.ALIQICMSVENDA'
    end
    object C_MovimentosTOTALSERVICOS: TFloatField
      FieldName = 'TOTALSERVICOS'
      Origin = 'SAIDAS.TOTALSERVICOS'
    end
    object C_MovimentosVALORISENTAS: TFloatField
      FieldName = 'VALORISENTAS'
      Origin = 'SAIDAS.VALORISENTAS'
    end
    object C_MovimentosVALOROUTRASICMS: TFloatField
      FieldName = 'VALOROUTRASICMS'
      Origin = 'SAIDAS.VALOROUTRASICMS'
    end
    object C_MovimentosVALORISENTASIPI: TFloatField
      FieldName = 'VALORISENTASIPI'
      Origin = 'SAIDAS.VALORISENTASIPI'
    end
    object C_MovimentosVALOROUTRASIPI: TFloatField
      FieldName = 'VALOROUTRASIPI'
      Origin = 'SAIDAS.VALOROUTRASIPI'
    end
    object C_MovimentosPJUROSNEGOCIADO: TFloatField
      FieldName = 'PJUROSNEGOCIADO'
      Origin = 'SAIDAS.PJUROSNEGOCIADO'
    end
    object C_MovimentosTOTALPRODUTOS: TFloatField
      FieldName = 'TOTALPRODUTOS'
      Origin = 'SAIDAS.TOTALPRODUTOS'
    end
    object C_MovimentosVALORIPIITENS: TFloatField
      FieldName = 'VALORIPIITENS'
      Origin = 'SAIDAS.VALORIPIITENS'
    end
    object C_MovimentosBASECALCICMSITENS: TFloatField
      FieldName = 'BASECALCICMSITENS'
      Origin = 'SAIDAS.BASECALCICMSITENS'
    end
    object C_MovimentosVALORICMSITENS: TFloatField
      FieldName = 'VALORICMSITENS'
      Origin = 'SAIDAS.VALORICMSITENS'
    end
    object C_MovimentosBASECALCSUBSTITENS: TFloatField
      FieldName = 'BASECALCSUBSTITENS'
      Origin = 'SAIDAS.BASECALCSUBSTITENS'
    end
    object C_MovimentosBASECALCSUBST: TFloatField
      FieldName = 'BASECALCSUBST'
      Origin = 'SAIDAS.BASECALCSUBST'
    end
    object C_MovimentosVALORICMSSUBST: TFloatField
      FieldName = 'VALORICMSSUBST'
      Origin = 'SAIDAS.VALORICMSSUBST'
    end
    object C_MovimentosVALORSEGURO: TFloatField
      FieldName = 'VALORSEGURO'
      Origin = 'SAIDAS.VALORSEGURO'
    end
    object C_MovimentosLICITACAOICMS: TFloatField
      FieldName = 'LICITACAOICMS'
      Origin = 'SAIDAS.LICITACAOICMS'
    end
  end
  object C_MovimentosDS: TDataSource
    DataSet = C_Movimentos
    Left = 128
    Top = 285
  end
  object ppmGrid: TTS_PopupMenu
    Left = 667
    Top = 263
  end
  object Q_Itens: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    DataSource = Q_MovimentosDS
    SQL.Strings = (
      
        'Select ei.*, i.TipoItem, i.Codigo, ei.QtdFaturada as OldQtdReceb' +
        'ida'
      
        'from SAIDAS S INNER JOIN SaidasItens ei ON S.SAIDA = EI.SAIDA  L' +
        'EFT JOIN Itens i ON ei.Item = i.Item'
      
        'Where ei.Saida = :SAIDA and ei.Status in ('#39'P'#39', '#39'L'#39', '#39'F'#39') and s.s' +
        'ituacao = '#39'N'#39
      'order by ei.Ordem')
    Left = 272
    Top = 172
    ParamData = <
      item
        DataType = ftInteger
        Name = 'SAIDA'
        ParamType = ptUnknown
        Size = 4
      end>
  end
  object Q_MovimentosDS: TDataSource
    DataSet = Q_Movimentos
    Left = 129
    Top = 235
  end
  object C_Itens: TClientDataSet
    Aggregates = <>
    DataSetField = C_MovimentosQ_Itens
    Params = <>
    Left = 242
    Top = 233
    object C_Itens_icSelecionado: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = '_icSelecionado'
    end
    object C_ItensicQtdMesclar: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'icQtdMesclar'
      OnValidate = C_ItensicQtdMesclarValidate
      DisplayFormat = '##,##0.###'
    end
    object C_ItensSAIDAITEM: TIntegerField
      FieldName = 'SAIDAITEM'
      Required = True
    end
    object C_ItensSAIDA: TIntegerField
      FieldName = 'SAIDA'
    end
    object C_ItensSEQUENCIA: TIntegerField
      FieldName = 'SEQUENCIA'
    end
    object C_ItensITEM: TIntegerField
      FieldName = 'ITEM'
    end
    object C_ItensDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 255
    end
    object C_ItensPRECO: TFloatField
      FieldName = 'PRECO'
    end
    object C_ItensQUANTIDADE: TFloatField
      FieldName = 'QUANTIDADE'
    end
    object C_ItensTAXAVEL: TStringField
      FieldName = 'TAXAVEL'
      Size = 1
    end
    object C_ItensIMPORTACAO: TIntegerField
      FieldName = 'IMPORTACAO'
    end
    object C_ItensCONTAVENDA: TIntegerField
      FieldName = 'CONTAVENDA'
    end
    object C_ItensTABELAPRECO: TIntegerField
      FieldName = 'TABELAPRECO'
    end
    object C_ItensUSOTIPOITEM: TStringField
      FieldName = 'USOTIPOITEM'
      Size = 15
    end
    object C_ItensPRECOTABELA: TFloatField
      FieldName = 'PRECOTABELA'
    end
    object C_ItensORDEM: TIntegerField
      FieldName = 'ORDEM'
    end
    object C_ItensCUSTOMEDIO: TFloatField
      FieldName = 'CUSTOMEDIO'
    end
    object C_ItensCUSTOCONTABIL: TFloatField
      FieldName = 'CUSTOCONTABIL'
    end
    object C_ItensHASCHILDREN: TStringField
      FieldName = 'HASCHILDREN'
      Size = 1
    end
    object C_ItensPRODUCAO: TIntegerField
      FieldName = 'PRODUCAO'
    end
    object C_ItensFUNCPRODUCAO: TIntegerField
      FieldName = 'FUNCPRODUCAO'
    end
    object C_ItensCOLUNA1: TStringField
      FieldName = 'COLUNA1'
    end
    object C_ItensCOLUNA2: TStringField
      FieldName = 'COLUNA2'
    end
    object C_ItensCOLUNA3: TStringField
      FieldName = 'COLUNA3'
    end
    object C_ItensCOLUNA4: TStringField
      FieldName = 'COLUNA4'
    end
    object C_ItensMAXDESCONTO: TFloatField
      FieldName = 'MAXDESCONTO'
    end
    object C_ItensDESPESA: TIntegerField
      FieldName = 'DESPESA'
    end
    object C_ItensENTRADAITEM: TIntegerField
      FieldName = 'ENTRADAITEM'
    end
    object C_ItensCONTRATOREF: TIntegerField
      FieldName = 'CONTRATOREF'
    end
    object C_ItensMESCLADO: TStringField
      FieldName = 'MESCLADO'
      Size = 1
    end
    object C_ItensSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 1
    end
    object C_ItensNUMEROLOTE: TStringField
      FieldName = 'NUMEROLOTE'
      Size = 12
    end
    object C_ItensUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Size = 6
    end
    object C_ItensFATOR: TFloatField
      FieldName = 'FATOR'
    end
    object C_ItensDESCONTO: TFloatField
      FieldName = 'DESCONTO'
    end
    object C_ItensDESCRICAOCOMPLEMENTAR: TStringField
      FieldName = 'DESCRICAOCOMPLEMENTAR'
      Size = 255
    end
    object C_ItensPROMOCAO: TStringField
      FieldName = 'PROMOCAO'
      Size = 1
    end
    object C_ItensBAIXAESTOQUE: TStringField
      FieldName = 'BAIXAESTOQUE'
      Size = 1
    end
    object C_ItensALMOXARIFADO: TIntegerField
      FieldName = 'ALMOXARIFADO'
    end
    object C_ItensORDEMMONTAGEM: TStringField
      FieldName = 'ORDEMMONTAGEM'
      Size = 1
    end
    object C_ItensCST: TStringField
      FieldName = 'CST'
      Size = 2
    end
    object C_ItensSITUACAOECF: TStringField
      FieldName = 'SITUACAOECF'
      Size = 1
    end
    object C_ItensCFOPNOTA: TStringField
      FieldName = 'CFOPNOTA'
      Size = 3
    end
    object C_ItensNUMEROITEM: TStringField
      FieldName = 'NUMEROITEM'
      Size = 5
    end
    object C_ItensREGISTRO: TStringField
      FieldName = 'REGISTRO'
      Size = 15
    end
    object C_ItensTIPOITEM: TIntegerField
      FieldName = 'TIPOITEM'
    end
    object C_ItensCODIGO: TStringField
      FieldName = 'CODIGO'
      Size = 13
    end
    object C_ItensSUBTOTALITEM: TFloatField
      FieldName = 'SUBTOTALITEM'
    end
    object C_ItensRATEIODESCONTO: TFloatField
      FieldName = 'RATEIODESCONTO'
    end
    object C_ItensQTDFATURADA: TFloatField
      FieldName = 'QTDFATURADA'
    end
    object C_ItensQTDMONTAGEM: TFloatField
      FieldName = 'QTDMONTAGEM'
    end
    object C_ItensN_USOTIPOITEM: TFloatField
      FieldName = 'N_USOTIPOITEM'
    end
    object C_ItensPRECOSEMPROMOCAO: TFloatField
      FieldName = 'PRECOSEMPROMOCAO'
    end
    object C_ItensBASECALCICMSPROD: TFloatField
      FieldName = 'BASECALCICMSPROD'
    end
    object C_ItensIPI: TFloatField
      FieldName = 'IPI'
    end
    object C_ItensALIQICMS: TFloatField
      FieldName = 'ALIQICMS'
    end
    object C_ItensREDUCAOCST: TFloatField
      FieldName = 'REDUCAOCST'
    end
    object C_ItensBASECALCSUBSTPROD: TFloatField
      FieldName = 'BASECALCSUBSTPROD'
    end
    object C_ItensVALORIPIPROD: TFloatField
      FieldName = 'VALORIPIPROD'
    end
    object C_ItensTVA: TFloatField
      FieldName = 'TVA'
    end
    object C_ItensVALORICMSPROD: TFloatField
      FieldName = 'VALORICMSPROD'
    end
    object C_ItensVALORISENTASPROD: TFloatField
      FieldName = 'VALORISENTASPROD'
    end
    object C_ItensVALORICMSSUBSTPROD: TFloatField
      FieldName = 'VALORICMSSUBSTPROD'
    end
    object C_ItensPDESCONTO: TFloatField
      FieldName = 'PDESCONTO'
    end
    object C_ItensQTDDEVOLVIDO: TFloatField
      FieldName = 'QTDDEVOLVIDO'
    end
    object C_ItensQTDACERTADO: TFloatField
      FieldName = 'QTDACERTADO'
    end
    object C_ItensOLDQTDRECEBIDA: TFloatField
      FieldName = 'OLDQTDRECEBIDA'
    end
  end
  object C_ItensDS: TDataSource
    DataSet = C_Itens
    Left = 232
    Top = 285
  end
  object ppmGridItens: TTS_PopupMenu
    Left = 511
    Top = 171
  end
  object Q_Filhos: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      'select si.*, i.codigo, i.descricao '
      'from saidasitensfilhos si, itens i '
      'where si.item = i.item and si.saidaitem = :IDITEM')
    Left = 341
    Top = 181
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDITEM'
        ParamType = ptUnknown
      end>
  end
  object P_Acerto: TDataSetProvider
    DataSet = Q_Acerto
    Constraints = True
    Left = 141
    Top = 151
  end
  object C_Acerto: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'P_Acerto'
    Left = 144
    Top = 103
    object C_Acerto_icSelecionado: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = '_icSelecionado'
    end
    object C_AcertoQtddevolvida: TCurrencyField
      FieldKind = fkInternalCalc
      FieldName = 'Qtddevolvida'
      OnValidate = C_AcertoQtddevolvidaValidate
    end
    object C_AcertoQtdvalidar: TCurrencyField
      FieldKind = fkInternalCalc
      FieldName = 'Qtdvalidar'
    end
    object C_AcertoTIPOMOVIMENTO: TIntegerField
      FieldName = 'TIPOMOVIMENTO'
    end
    object C_AcertoFAVORECIDO: TIntegerField
      FieldName = 'FAVORECIDO'
    end
    object C_AcertoNOME: TStringField
      FieldName = 'NOME'
      Size = 50
    end
    object C_AcertoITEM: TIntegerField
      FieldName = 'ITEM'
    end
    object C_AcertoCODIGO: TStringField
      FieldName = 'CODIGO'
      Size = 13
    end
    object C_AcertoDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object C_AcertoUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Size = 6
    end
    object C_AcertoREFERENCIA: TStringField
      FieldName = 'REFERENCIA'
      Size = 25
    end
    object C_AcertoEMPRESA: TIntegerField
      FieldName = 'EMPRESA'
    end
    object C_AcertoPDV: TIntegerField
      FieldName = 'PDV'
    end
    object C_AcertoESTOQUECONSIGSAIDA: TBCDField
      FieldName = 'ESTOQUECONSIGSAIDA'
      Precision = 18
      Size = 3
    end
    object C_AcertoESTOQUE: TBCDField
      FieldName = 'ESTOQUE'
      Precision = 18
      Size = 3
    end
    object C_AcertoQTD: TFloatField
      FieldName = 'QTD'
    end
    object C_AcertoQTDDEV: TFloatField
      FieldName = 'QTDDEV'
    end
    object C_AcertoQTDACERT: TFloatField
      FieldName = 'QTDACERT'
    end
  end
  object C_AcertoDS: TDataSource
    DataSet = C_Acerto
    Left = 280
    Top = 117
  end
  object Q_Acerto: TIBQuery
    Database = DMProjeto.DB_Projeto
    Transaction = DMProjeto.IBT_Projeto
    BufferChunks = 1000
    CachedUpdates = False
    SQL.Strings = (
      
        'select e.tipomovimento, e.favorecido, f.nome, ei.item, i.codigo,' +
        ' i.descricao,'
      'i.unidade, i.referencia, i.estoqueconsigsaida, i.estoque,'
      'sum(ei.quantidade) as qtd,'
      'sum(ei.qtddevolvido) as qtddev, sum(ei.qtdacertado) as qtdacert,'
      'ei.empresa, ei.pdv'
      'from saidas e'
      'left join saidasitens ei on e.saida = ei.saida'
      'left join itens i on ei.item = i.item'
      'left join favorecidos f on e.favorecido = f.favorecido'
      'where ei.status in ('#39'L'#39','#39'F'#39','#39'P'#39') and e.tipomovimento = 11 '
      'and  e.favorecido in (XX)  and e.situacao = '#39'N'#39' '
      'group by e.tipomovimento, e.favorecido,f.nome,ei.item,i.codigo,'
      
        'i.descricao, i.unidade,i.referencia,i.estoqueconsigsaida,i.estoq' +
        'ue,'
      'ei.empresa, ei.pdv')
    Left = 25
    Top = 125
  end
  object ppmAcerto: TTS_PopupMenu
    Left = 457
    Top = 230
  end
end
