...
type
    TDlgMesclarCupons = class(TFrmModeloCadastros)
        ...
        procedure dfCupomKeyPress(Sender: TObject; var Key: Char);
    private
...
var
    DlgMesclarCupons: TDlgMesclarCupons;
...
procedure TDlgMesclarCupons.FormShow(Sender: TObject);
begin
    inherited;
    ...
    sSQL := Q_Movimentos.SQL.text;
end;

procedure TDlgMesclarCupons.ProcessaCupons;
begin
    inherited;
    Screen.Cursor := crHourGlass;
    try
        Q_Movimentos.SQL.text := sSQL;
        if dfCupom.text <> '' then
            Q_Movimentos.SQL.Add('and s.numero = ''' + dfCupom.text + '''');
                        ...
                        while not DM.C_Mesclagens.EOF do
                            begin
                                if C_Movimentos.Locate('SAIDA', DM.C_Mesclagens.FieldByName('SAIDA_ORIGINAL').asInteger, []) and
                                    C_Itens.Locate('SAIDAITEM', DM.C_Mesclagens.FieldByName('SAIDAITEM_ORIGINAL').asInteger, []) then
                                    begin
                                        bDeleteItens := true;
...
procedure TDlgMesclarCupons.Q_MovimentosBeforeOpen(DataSet: TDataSet);
var
    sSaidasOriginais: string;
begin
    inherited;
    {Substituindo Tipo Padrao e Favorecido}
    {Felipe (06/08/2015) - Adicionado condicao de modelo 65 para importacao de NFCE para tela de Nota Fiscal p/ Cupom}
    if not DM.bAlteracao then
        Q_Movimentos.SQL.Text := replace(Q_Movimentos.SQL.Text, 'KK', ' (s.serienota = ''ECF'' or s.modelo = ''65'') ')
    else
        begin
            sSaidasOriginais := '';
            with DM.Q_SQL do
            ...
            if sSaidasOriginais <> '' then
                Q_Movimentos.SQL.Text := replace(Q_Movimentos.SQL.Text, 'KK', ' ( (s.Serienota = ''ECF'' or s.modelo = ''65'') or s.Saida in (' + sSaidasOriginais + ') ) ')
            else
                Q_Movimentos.SQL.Text := replace(Q_Movimentos.SQL.Text, 'KK', ' (s.Serienota = ''ECF'' or s.modelo = ''65'') ');
...
procedure TDlgMesclarCupons.GridMovsTS_OnAfterSelection(
...
procedure TDlgMesclarCupons.dbgItensTS_OnAfterSelection(
    Sender: TObject; bSelected: Boolean; QtdSel: Integer);
begin
    inherited;
...
procedure TDlgMesclarCupons.GridMovsItensCloseUp(Sender: TObject;
...
procedure TDlgMesclarCupons.AtualizarSelecionados;
begin
...
procedure TDlgMesclarCupons.dbgItensEditing(Sender: TObject;
...
procedure TDlgMesclarCupons.dbgItensEdited(Sender: TObject;
...
procedure TDlgMesclarCupons.C_ItensicQtdMesclarValidate(
    Sender: TField);
begin
    inherited;
    if (C_ItensItem.Value > 0) and (C_ItensTipoItem.Value in [1, 2, 3]) and
        (C_ItensQuantidade.Value > 0) and (Sender.asFloat > 0) and
        (Sender.asFloat - (C_ItensQuantidade.Value - C_ItensQtdFaturada.Value) > 0.01) then
        begin
            DlgMsg.ShowMsg(2189);
...
procedure TDlgMesclarCupons.btGravarClick(Sender: TObject);
var
    m, i: Integer;
...
begin
    ...
    with DM do
        begin
            ...
            C_TabelaNF_CUPOM.AsVariant :=  C_Movimentos.FieldByName('Numero').Value;
            if pos(DMProjeto.Parametro('MsgRefCupom') + ' ' + C_MovimentosNumero.value, C_TabelaOBS.value) = 0 then
                C_TabelaOBS.value := C_TabelaOBS.value + ' ' + DMProjeto.Parametro('MsgRefCupom') + ' ' + C_MovimentosNumero.value;
        end;
        ...
        if GridMovs.nQuantidadeSelecionada = 1 then
            with DM do
                begin
                    ...
                    Q_SQL.Params[1].AsInteger := C_MovimentosEMPRESA.Value;
                    Q_SQL.Params[2].AsInteger := C_MovimentosPDV.Value;
                    Q_SQL.Open;
...
procedure TDlgMesclarCupons.JoinItemToOperation;
        ...
        try
            ...
            while not DM.C_Itens.EOF do
                begin
                    ...
                    if (nMatchFilhos = nFilhos) and (nFilhos = nFilhosExistente) then
                        begin
                            if (C_ItensPreco.Value = DM.C_ItensPreco.Value) and (C_ItensDESCRICAO.Value = DM.C_ItensDESCRICAO.Value) and
                                ...
                                (C_ItensAlmoxarifado.Value = DM.C_ItensAlmoxarifado.Value) then
                                begin
    ...
    begin
        ...
        DM.C_ItensAliqICMS.Value := 0.00; //C_ItensAliqICMS.Value;
...
begin
    ...
    try
        ...
        if ItemExatamenteIgual(sNovoFlagBaixaEstoque) then
            begin //Já posiciona DM.C_Itens no item que será adicionado
                ...
                with DM.C_Mesclagens do
                    begin
                        ...
                        FieldByName('PDV').AsInteger := C_ItensPDV.Value;
...
procedure TDlgMesclarCupons.dbgItensTS_OnSelection(Sender: TObject;
...
procedure TDlgMesclarCupons.dfCupomKeyPress(Sender: TObject;
    var Key: Char);
begin
    inherited;
    if key = #13 then
