...
begin
    try
        ...
        {Quantidade para alimentar o gráfico}
        with DMProjeto.Q_SQL do
            begin
                nLinhasSelect := Grid.nQuantidadeSelecionada;
                {Cabeçalho da Nota com Emitente e Cliente}
                Close;
                Sql.Text :=
                    ' Select ' +
                    's.saida, ' +
                                        ...
                                        if FieldByName('HasPisCofins').AsString = 'S' then
                                            begin
                                                ...
                                                case FieldByName('cstpiscofins').Value of
                                                    ...
                                                    3:
                                                        begin
                                                            {Para PIS/CoFINS por Quantidade, Não iremos implementar agora.}
                                                        end;
                                                    4, 6, 7, 8, 9:
                                                        begin
        ...
        Application.MessageBox('Erro ao Realizar a Exportação', 'Erro', mb_ok);
    end;
