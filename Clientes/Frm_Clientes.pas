    ...
    with DMClientes do
        begin
            ...
            if C_ConsultasPreCONSULTA.Value = 9 then
                ...
                begin // Clientes que não estão comprando
...
begin
    ...
    if ((DMClientes.C_Tabela.FieldByName('Favorecido').AsInteger = 0) or
        ...
        begin
            DlgMsg.ShowMsg(50, ['O Consumidor Final não Pode ser Alterado !!!']);
            Exit;
        end;
...
begin
    ...
    if not (key in ['0'..'9', #8]) then
        begin
            DlgMsg.ShowMsg(50, ['Para o campo Fone 1, utilizar somente números!']);
            key := #0;
        end;
end;
...
begin
    ...
    if key in ['&', '@'] then
        begin
            DlgMsg.ShowMsg(50, ['Para o campo Razão Social, não utilizar os seguintes caracteres:' +
                #13 + '@' +
                    #13 + '&']);
            key := #0;
        end;
...
begin
    ...
    if Length(cepCliente) <> 8 then
        Application.MessageBox('O CEP digitado não é válido, favor verificar.', 'Aviso', MB_ICONEXCLAMATION);
...
begin
  inherited;
    ...
    { Felipe - Atribuindo informações aos campos do relatório (02/03/2016) }
