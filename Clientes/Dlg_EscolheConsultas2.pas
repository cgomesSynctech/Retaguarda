...
uses
  ...
  TS_Label, IBDatabase, FormsComponent, jpeg, TS_Image, dxfLabel, dxCntner,
  dxEditor, dxEdLib, dxExEdtr, TS_DateTimePicker, teCtrls,
  TS_EffectsPanel, TS_PopupMenu, TS_ComboBox, TS_CheckBox, TransEff,
    ...
    with FrmClientes do begin
      if C_ConsultasCONSULTA.Value=9 then begin // Aniversariantes
        ...
        if cmbPeriodo.ItemIndex = 0 then begin
        	Condicao := '(extract(day from f.datanasc)='+formatdatetime('d',DMProjeto.dDataSistema)+' '+
                      'and extract(month from f.datanasc)='+formatdatetime('m',DMProjeto.dDataSistema)+') ';
        end else if cmbPeriodo.ItemIndex = 1 then begin
          Condicao := '(extract(day from f.datanasc)>='+formatdatetime('d',DMProjeto.dDataSistema)+' '+
                      'and extract(month from f.datanasc)>='+formatdatetime('m',DMProjeto.dDataSistema)+' '+
           						'and extract(day from f.datanasc)<='+formatdatetime('d',DMProjeto.dDataSistema+7)+' '+
                      'and extract(month from f.datanasc)<='+formatdatetime('m',DMProjeto.dDataSistema+7)+') ';
        end else if cmbPeriodo.ItemIndex = 2 then begin
        	Condicao := '(extract(month from f.datanasc)='+formatdatetime('m ',DMProjeto.dDataSistema)+') ';
        end else begin
        	Condicao := '(extract(month from f.datanasc)='+formatdatetime('m ',DateAdd('MM',1,DMProjeto.dDataSistema))+') ';
        end;
